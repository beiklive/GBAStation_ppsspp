...
void Path::Init(std::string_view str) {
  if (str.empty()) {
    ...
    // Content URIs on non scoped-storage (and possibly other cases?) can
    // contain "raw:/" URIs inside. This happens when picking the Download
    // folder using the folder browser on Android 9. Here's an example:
    ...
    //
    // Since this is a legacy use case, I think it's safe enough to just detect
    // this and flip it to a NATIVE url and hope for the best.
...
void Path::operator/=(std::string_view subdir) { *this = *this / subdir; }
...
Path Path::WithReplacedExtension(const std::string &oldExtension,
                                 const std::string &newExtension) const {
  if (type_ == PathType::CONTENT_URI) {
    AndroidContentURI uri(path_);
    return Path(
        uri.WithReplacedExtension(oldExtension, newExtension).ToString());
  }
...
Path Path::WithReplacedExtension(const std::string &newExtension) const {
  ...
  std::string newPath =
      path_.substr(0, path_.size() - extension.size()) + newExtension;
  return Path(newPath);
}
...
bool Path::FilePathContainsNoCase(std::string_view needle) const {
  ...
  auto found = std::search(haystack.begin(), haystack.end(), needle.begin(),
                           needle.end(), pred);
...
bool Path::StartsWithGlobalAndNotEqual(const Path &other) const {
  ...
  if (type_ == PathType::CONTENT_URI) {
    ...
    return CountChar(aLast, '/') != CountChar(bLast, '/') &&
           startsWith(aLast, bLast);
  }
...
Path Path::GetRootVolume() const {
  ...
  if (path_[0] == '/' && path_[1] == '/') {
    ...
    if ((path_[2] == '.' || path_[2] == '?') && path_[3] == '/') {
      // Device path, or "\\.\UNC" path, skip the dot and consider the device
      // the root.
      next = 4;
    }
  ...
  // Handle libnx device paths like "sdmc:/path" or "romfs:/path"
  // The format is "device:/path" where device is something like sdmc, romfs,
  // etc.
  size_t colonPos = path_.find(':');
  if (colonPos != std::string::npos && colonPos < path_.size() - 1 &&
      path_[colonPos + 1] == '/') {
    // Found a "device:/" pattern - return "device:/" as the root
    std::string root = path_.substr(0, colonPos + 2);
    printf("DEBUG: GetRootVolume('%s') -> '%s'\n", path_.c_str(), root.c_str());
    return Path(root); // Include the colon and slash
  }
...
bool Path::IsAbsolute() const {
  ...
  if (path_.empty())
    ...
    // Check for device:/
    size_t colon = path_.find(':');
    if (colon != std::string::npos && colon + 1 < path_.size() &&
        path_[colon + 1] == '/')
      return true;
#if PPSSPP_PLATFORM(WINDOWS)
...
bool Path::ComputePathTo(const Path &other, std::string &path) const {
  ...
  if (!other.StartsWith(*this)) {
    ...
    printf("DEBUG: ComputePathTo('%s' -> '%s') FAILED: StartsWith=false\n",
           path_.c_str(), other.path_.c_str());
    return false;
  }
  ...
  if (type_ == PathType::CONTENT_URI) {
    ...
    if (start > 0 && path_.back() != '/') {
      start++; // account for slash
    }
    path = other.path_.substr(start);
    printf("DEBUG: ComputePathTo('%s' -> '%s') success. Diff='%s'\n",
           path_.c_str(), other.path_.c_str(), path.c_str());
    return true;
  }
}
...
bool FixPathCase(const Path &realBasePath, std::string &path,
                 FixPathCaseBehavior behavior) {
#if _WIN32
  return true;
#else
  ...
  while (start < len) {
    ...
    if (i > start) {
      ...
      if (FixFilenameCase(fullPath, component) == false) {
        ...
        return (behavior == FPC_PARTIAL_ALLOWED ||
                (behavior == FPC_PATH_MUST_EXIST && i >= len));
      }
