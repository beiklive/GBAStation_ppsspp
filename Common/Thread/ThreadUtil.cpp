...
// TODO: Many other platforms also support TLS, in fact probably nearly all that
// we support these days.
...
static void InitializeSetThreadDescription() {
  if (g_pSetThreadDescription != nullptr ||
      g_failedToFindSetThreadDescription) {
    return;
  }
  ...
  g_pSetThreadDescription = reinterpret_cast<TSetThreadDescription>(
...
void AssertCurrentThreadName(const char *threadName) {
  ...
  if (curThreadName && strcmp(curThreadName, threadName) != 0) {
    ERROR_LOG(Log::System, "Thread name assert failed: Expected %s, was %s",
              threadName, curThreadName);
    _dbg_assert_msg_(false, "Thread name assert failed: Expected %s, was %s",
                     threadName, curThreadName);
  }
#endif
}
