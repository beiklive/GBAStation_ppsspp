...
#if PPSSPP_PLATFORM(SWITCH)
extern "C" {
VKAPI_ATTR PFN_vkVoidFunction VKAPI_CALL SwitchStaticVkGetInstanceProcAddr(VkInstance instance, const char *pName) __asm__("vk_icdGetInstanceProcAddr");
VKAPI_ATTR VkResult VKAPI_CALL SwitchStaticVkNegotiateLoaderICDInterfaceVersion(uint32_t *pVersion) __asm__("vk_icdNegotiateLoaderICDInterfaceVersion");
}

static void SwitchVulkanDebugLog(const char *fmt, ...) {
...
static VKAPI_ATTR VkResult VKAPI_CALL SwitchEnumerateInstanceLayerProperties(uint32_t *pPropertyCount, VkLayerProperties * /*pProperties*/) {
	if (!pPropertyCount) {
...
static VkInstance g_switchVulkanInstance = VK_NULL_HANDLE;

static VKAPI_ATTR PFN_vkVoidFunction VKAPI_CALL SwitchLoaderGetDeviceProcAddr(VkDevice /*device*/, const char *pName) {
	if (!pName) {
		return nullptr;
	}
	PFN_vkVoidFunction func = SwitchStaticVkGetInstanceProcAddr(g_switchVulkanInstance, pName);
	if (!func) {
		func = SwitchStaticVkGetInstanceProcAddr(VK_NULL_HANDLE, pName);
	}
	return func;
}

static VKAPI_ATTR PFN_vkVoidFunction VKAPI_CALL SwitchLoaderGetInstanceProcAddr(VkInstance instance, const char *pName) {
	if (!pName) {
		return nullptr;
	}
	if (strcmp(pName, "vkGetInstanceProcAddr") == 0) {
		return reinterpret_cast<PFN_vkVoidFunction>(&SwitchLoaderGetInstanceProcAddr);
	}
	if (strcmp(pName, "vkGetDeviceProcAddr") == 0) {
		return reinterpret_cast<PFN_vkVoidFunction>(&SwitchLoaderGetDeviceProcAddr);
	}
	if (strcmp(pName, "vkEnumerateInstanceLayerProperties") == 0) {
		return reinterpret_cast<PFN_vkVoidFunction>(&SwitchEnumerateInstanceLayerProperties);
	}
	return SwitchStaticVkGetInstanceProcAddr(instance, pName);
}
#endif
...
bool VulkanLoad(std::string *errorStr) {
	...
	SwitchStaticVkNegotiateLoaderICDInterfaceVersion(&icdVersion);
	SwitchVulkanDebugLog("VulkanLoad: switchVK ICD negotiated version=%u", icdVersion);
	if (vkCreateInstance && vkGetInstanceProcAddr && vkGetDeviceProcAddr &&
		...
		SwitchVulkanDebugLog("VulkanLoad: base functions already loaded");
		return true;
	}

	vkGetInstanceProcAddr = reinterpret_cast<PFN_vkGetInstanceProcAddr>(&SwitchLoaderGetInstanceProcAddr);
	if (!vkGetInstanceProcAddr) {
		*errorStr = "Failed to load vkGetInstanceProcAddr";
		SwitchVulkanDebugLog("VulkanLoad: %s", errorStr->c_str());
		return false;
	}
	...
	if (vkCreateInstance && vkGetInstanceProcAddr && vkGetDeviceProcAddr &&
		vkEnumerateInstanceExtensionProperties && vkEnumerateInstanceLayerProperties) {
		INFO_LOG(Log::G3D, "VulkanLoad: Switch ICD base functions loaded (icd=%u).", icdVersion);
		SwitchVulkanDebugLog("VulkanLoad: Switch ICD base functions loaded create=%p gipa=%p gdpa=%p enumExt=%p enumLayer=%p",
			vkCreateInstance, vkGetInstanceProcAddr, vkGetDeviceProcAddr,
			vkEnumerateInstanceExtensionProperties, vkEnumerateInstanceLayerProperties);
		return true;
	}

	char details[256]{};
	snprintf(details, sizeof(details), "Failed to load Vulkan base functions create=%p gipa=%p gdpa=%p enumExt=%p enumLayer=%p icd=%u",
		vkCreateInstance, vkGetInstanceProcAddr, vkGetDeviceProcAddr,
		vkEnumerateInstanceExtensionProperties, vkEnumerateInstanceLayerProperties, icdVersion);
	*errorStr = details;
	ERROR_LOG(Log::G3D, "VulkanLoad: %s create=%p gipa=%p gdpa=%p enumExt=%p enumLayer=%p icd=%u",
		"Failed to load Vulkan base functions", vkCreateInstance, vkGetInstanceProcAddr, vkGetDeviceProcAddr,
		vkEnumerateInstanceExtensionProperties, vkEnumerateInstanceLayerProperties, icdVersion);
	SwitchVulkanDebugLog("VulkanLoad: %s", errorStr->c_str());
	return false;
#else
...
#if !PPSSPP_PLATFORM(IOS_APP_STORE)
#if PPSSPP_PLATFORM(SWITCH)
	g_switchVulkanInstance = instance;
#endif
