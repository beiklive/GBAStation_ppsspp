...
namespace Libretro
{
  ...
  static void VsyncSwapIntervalDetect()
  {
    ...
    else
    {
      ...
      swapRemainder = (swapRemainder < 0.0f) ? -swapRemainder : swapRemainder;
    }
        ...
        (swapInteger != vsyncSwapInterval))
    {
      vsyncSwapIntervalCounter =
          (swapInteger == vsyncSwapIntervalLast) ? (vsyncSwapIntervalCounter + 1) : 0;
...
static void check_variables(CoreParameter &coreParam)
{
  ...
  if (gpu && (g_Config.iTexScalingType != iTexScalingType_prev || g_Config.iTexScalingLevel != iTexScalingLevel_prev || g_Config.sTextureShaderName != sTextureShaderName_prev))
  {
...
void retro_init(void)
{
  ...
  // retro_base_dir /= "PPSSPP"; // Tiicu: We want to use the system dir directly (sdmc:/tiicu/system/psp)
  ...
  // Replicate InitSysDirectories() logic from Core/System.cpp
  // This is normally only compiled for Windows, so libretro doesn't have it.
  //
  // GetSysDirectory(DIRECTORY_PSP) returns: pspDirectory
  // where pspDirectory = memStickDirectory / "PSP" (unless basename is "PSP")

  // Determine pspDirectory exactly as GetSysDirectory does
  std::string lastComponent = retro_save_dir.GetFilename();
  bool isCasePSP = (strcasecmp(lastComponent.c_str(), "PSP") == 0);

  Path pspDirectory;
  if (isCasePSP) {
    // If the memstick dir IS the PSP folder, use it directly
    pspDirectory = retro_save_dir;

  } else {
    // Otherwise, add /PSP
    pspDirectory = retro_save_dir / "PSP";
  }

  // Create directories using mkdir() directly with error checking
  // Order matters: parent directories must exist first!
  auto createDir = [](const Path &dir) {
  ...
  // Create standard PSP directories (like InitSysDirectories does)

  // 1. PSP directory (if not already the save dir)
  if (!isCasePSP) {
    createDir(pspDirectory);
  }

  // 2. Standard subdirectories
  createDir(pspDirectory / "COMMON");
  createDir(pspDirectory / "GAME");
  createDir(pspDirectory / "SAVEDATA");
  createDir(pspDirectory / "SAVESTATE");
  createDir(pspDirectory / "SYSTEM");
  createDir(pspDirectory / "SYSTEM" / "CACHE");
  createDir(pspDirectory / "PPSSPP_STATE");

  // Note: ms0: is mounted by pspFileSystem in __IoInit() (Core/HLE/sceIo.cpp).
...
bool retro_unserialize(const void *data, size_t size)
{
  ...
  SaveState::SaveStart state;
  bool retVal = CChunkFileReader::LoadPtr((u8 *)data, state, &errorString) == CChunkFileReader::ERROR_NONE;
