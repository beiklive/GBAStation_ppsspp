#include "GBAStationCoreConfig.h"
...
void CoreConfig::CreateDefaultConfig() const {
	...
	if (out.good()) {
		out << defaultJson_;
		LogMessage(log_, "GBAStation %s config default created path=%s", coreName_.c_str(), configPath_.c_str());
	} else {
		LogMessage(log_, "GBAStation %s config default create failed path=%s", coreName_.c_str(), configPath_.c_str());
	}
}
...
void CoreConfig::Load() {
	...
	if (config.is_discarded()) {
		LogMessage(log_, "GBAStation %s config corrupt, recreating path=%s", coreName_.c_str(), configPath_.c_str());
		in.close();
	...
	LogMessage(log_, "GBAStation %s config loaded path=%s options=%u", coreName_.c_str(), configPath_.c_str(), (unsigned)options_.size());
}
...
void CoreConfig::Save() const {
	...
	if (out.good()) {
		out << config.dump(4);
		LogMessage(log_, "GBAStation %s config saved path=%s options=%u", coreName_.c_str(), configPath_.c_str(), (unsigned)options_.size());
	} else {
		LogMessage(log_, "GBAStation %s config save failed path=%s", coreName_.c_str(), configPath_.c_str());
	}
}
