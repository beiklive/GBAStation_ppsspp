...
constexpr const char *kAssetInstallMarker = "sdmc:/GBAStation/PSP/.GBAStation_assets_version";
constexpr const char *kAssetInstallVersion = "ppsspp-GBAStation-assets-v3";
constexpr size_t kCopyBufferSize = 64 * 1024;

constexpr std::array<const char *, 4> kRomfsOnlyGBAStationAssets = {{
	"bolt.svg",
	"loader.svg",
	"ra.svg",
	"trophy.mp3",
...
constexpr std::array<const char *, 5> kRequiredAssetSentinelFiles = {{
	"sdmc:/GBAStation/PSP/flash0/font/ltn0.pgf",
	"sdmc:/GBAStation/PSP/vfpu/vfpu_sqrt_lut.dat",
	"sdmc:/GBAStation/PSP/lang/en_US.ini",
	"sdmc:/GBAStation/PSP/ppge_atlas.zim",
	"sdmc:/GBAStation/PSP/compat.ini",
}};
...
bool IsRomfsOnlyGBAStationAsset(const std::string &source, const char *name) {
	if (source != kSourceAssetsRoot) {
		return false;
	}
	for (const char *GBAStationAsset : kRomfsOnlyGBAStationAssets) {
		if (!std::strcmp(name, GBAStationAsset)) {
			return true;
		}
	}
	return false;
...
bool CopyDirectoryRecursive(const std::string &source, const std::string &destination, std::vector<unsigned char> *buffer, const LogCallback &log) {
	...
	while ((entry = readdir(dir)) != nullptr) {
		...
		if (IsRomfsOnlyGBAStationAsset(source, entry->d_name)) {
			continue;
		}
...
void RemoveStaleGBAStationAssetsFromPpssppRoot(const LogCallback &log) {
	for (const char *asset : kRomfsOnlyGBAStationAssets) {
		const std::string path = JoinPath(Paths::PpssppDataRoot, asset);
		if (remove(path.c_str()) == 0) {
			LogMessage(log, "asset install removed stale GBAStation asset %s", path.c_str());
		}
	}
}
...
bool InstallPpssppAssets(LogCallback log) {
	...
	RemoveStaleGBAStationAssetsFromPpssppRoot(log);
	if (!WriteMarker(log)) {
		return false;
	}
