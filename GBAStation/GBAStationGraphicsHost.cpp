...
class GBAStationVulkanGraphicsContext final : public GraphicsContext {
public:
	~GBAStationVulkanGraphicsContext() override {
		Shutdown();
	}

	bool InitFromRenderThread(std::string *errorMessage) override {
		auto fail = [&](const std::string &message) {
			ERROR_LOG(Log::G3D, "GBAStationVulkanGraphicsContext init failed: %s", message.c_str());
			if (errorMessage) {
		...
		INFO_LOG(Log::G3D, "GBAStationVulkanGraphicsContext::InitFromRenderThread begin");
		...
		INFO_LOG(Log::G3D, "GBAStation Vulkan swapchain initialized: %dx%d", vulkan_->GetBackbufferWidth(), vulkan_->GetBackbufferHeight());
		...
		INFO_LOG(Log::G3D, "GBAStationVulkanGraphicsContext::InitFromRenderThread complete");
		return true;
	}
	...
	void Resize() override {
		...
		INFO_LOG(Log::G3D, "GBAStationVulkanGraphicsContext::Resize old=%dx%d", vulkan_->GetBackbufferWidth(), vulkan_->GetBackbufferHeight());
		draw_->HandleEvent(Draw::Event::LOST_BACKBUFFER, vulkan_->GetBackbufferWidth(), vulkan_->GetBackbufferHeight());
		vulkan_->DestroySwapchain();
		...
		if (vulkan_->InitSwapchain(presentMode)) {
			draw_->HandleEvent(Draw::Event::GOT_BACKBUFFER, vulkan_->GetBackbufferWidth(), vulkan_->GetBackbufferHeight());
			INFO_LOG(Log::G3D, "GBAStationVulkanGraphicsContext::Resize new=%dx%d", vulkan_->GetBackbufferWidth(), vulkan_->GetBackbufferHeight());
		} else {
			ERROR_LOG(Log::G3D, "GBAStationVulkanGraphicsContext::Resize failed: %s", vulkan_->InitError().c_str());
		}
	}
	...
	void Poll() override {
		...
		if (renderManager_->NeedsSwapchainRecreate()) {
			INFO_LOG(Log::G3D, "GBAStationVulkanGraphicsContext::Poll requested swapchain recreate");
			Resize();
		}
	}
...
namespace {
GraphicsContext* g_GBAStation_graphics_context = nullptr;
}
...
bool InitializeGBAStationGraphicsHost(std::string *error_message, GraphicsContext **ctx, GPUCore core) {
	...
	*ctx = nullptr;
	if (core != GPUCORE_VULKAN) {
		if (error_message) {
			*error_message = "Only Vulkan rendering is supported by the GBAStation PPSSPP host";
		}
		return false;
	}
	...
	static GBAStationVulkanGraphicsContext graphicsContext;
	GraphicsContext *context = &graphicsContext;
	if (!context->InitFromRenderThread(error_message)) {
		return false;
	}

	g_GBAStation_graphics_context = context;
	*ctx = context;
	return true;
}

void ShutdownGBAStationGraphicsHost() {
	if (!g_GBAStation_graphics_context) {
		return;
	}

	g_GBAStation_graphics_context->Shutdown();
	g_GBAStation_graphics_context = nullptr;
}

bool GBAStationGraphicsHost::Init(std::string *error_message, GraphicsContext **ctx, GPUCore core) {
	return InitializeGBAStationGraphicsHost(error_message, ctx, core);
}

void GBAStationGraphicsHost::Shutdown() {
