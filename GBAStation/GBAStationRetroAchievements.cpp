...
#include "GBAStation/GBAStationTranslationManager.h"
...
GBAStationRetroAchievements g_retroAchievements;
GBAStationRetroAchievements *g_activeRetroAchievements = nullptr;
...
void GBAStationRetroAchievements::Initialize(LogCallback log) {
	log_ = std::move(log);
	Shutdown();
	...
	if (!username_.empty() && !token_.empty()) {
		...
		rc_client_begin_login_with_token(client_, username_.c_str(), token_.c_str(),
			[](int result, const char *errorMessage, rc_client_t *, void *userdata) {
				auto *self = static_cast<GBAStationRetroAchievements *>(userdata);
				if (!self) {
					return;
				}
				if (result == RC_OK) {
...
void GBAStationRetroAchievements::LoadConfig() {
	enabled_ = false;
	hardcore_ = false;
	username_.clear();
	password_.clear();
...
void GBAStationRetroAchievements::SaveToken(const std::string &token) {
	if (token.empty()) {
		return;
	}
...
void GBAStationRetroAchievements::LoginWithPassword() {
...
void GBAStationRetroAchievements::SetGame(const std::string &path) {
	if (!path.empty()) {
		gamePath_ = path;
	}
	IdentifyGame();
...
void GBAStationRetroAchievements::IdentifyGame() {
...
void GBAStationRetroAchievements::UnloadGame() {
	gamePath_.clear();
	identifying_ = false;
...
void GBAStationRetroAchievements::FrameUpdate() {
...
void GBAStationRetroAchievements::Idle() {
...
void GBAStationRetroAchievements::Shutdown() {
	++requestGeneration_;
	UnloadGame();
...
bool GBAStationRetroAchievements::HardcoreModeActive() const {
	return enabled_ && client_ && rc_client_get_hardcore_enabled(client_) != 0 && rc_client_get_user_info(client_) != nullptr;
}

bool GBAStationRetroAchievements::WarnIfHardcoreModeActive(bool isSaveStateAction) {
...
void GBAStationRetroAchievements::PreloadBadges() {
	if (!client_) {
		return;
	}
...
void GBAStationRetroAchievements::PushNotification(const std::string &title, const std::string &description, const std::string &badgeName, float delaySeconds) {
...
void GBAStationRetroAchievements::DownloadBadge(const std::string &badgeName) {
...
void GBAStationRetroAchievements::PumpBadgeDownloads() {
...
Draw::Texture *GBAStationRetroAchievements::GetBadgeTexture(Draw::DrawContext *draw, const std::string &badgeName) {
	if (!draw) {
		return nullptr;
	}
	const std::string cleanBadgeName = CleanBadgeName(badgeName);
...
void GBAStationRetroAchievements::ReleaseBadgeTextures() {
...
void GBAStationRetroAchievements::QueueServerCall(const char *url, const char *postData, void *callback, void *callbackData) {
	if (!url || !callback) {
		return;
	}
...
void GBAStationRetroAchievements::HandleEvent(const rc_client_event_t *event) {
	if (!event) {
		return;
	}
...
void GBAStationRetroAchievements::LogRCMessage(const char *message) {
