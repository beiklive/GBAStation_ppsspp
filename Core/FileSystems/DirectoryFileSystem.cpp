...
std::vector<PSPFileInfo> DirectoryFileSystem::GetDirListing(const std::string &path, bool *exists)
{
  ...
  for (auto &file : files)
  {
    ...
    if (hideISOFiles)
    {
      if (endsWithNoCase(entry.name, ".cso") || endsWithNoCase(entry.name, ".iso") || endsWithNoCase(entry.name, ".chd"))
      { // chd not really necessary, but let's hide them too.
...
size_t VFSFileSystem::SeekFile(u32 handle, s32 position, FileMove type)
{
  ...
  if (iter != entries.end())
  {
    ...
    {
    case FILEMOVE_BEGIN:
      iter->second.seekPos = position;
      break;
    case FILEMOVE_CURRENT:
      iter->second.seekPos += position;
      break;
    case FILEMOVE_END:
      iter->second.seekPos = iter->second.size + position;
      break;
    }
