...
static bool IsProbablyExpression(std::string_view input) {
  // Heuristic: If it's a single-line statement without assignment or keywords,
...
static void print(const std::string &message) { g_lua.Print(message); }
...
// TODO: We should probably disallow or at least discourage raw read/writes and
// instead only support read/writes that refer to the name of a memory region.
static int r32(int address) {
  if (Memory::IsValid4AlignedAddress(address)) {
    return Memory::Read_U32(address);
  } else {
    g_lua.Print(LogLineType::Error,
                StringFromFormat("r32: bad address %08x", address));
    return 0;
  }
}
...
static void w32(int address, int value) {
  if (Memory::IsValid4AlignedAddress(address)) {
    Memory::Write_U32(
        value, address); // NOTE: These are backwards for historical reasons.
  } else {
    g_lua.Print(LogLineType::Error,
                StringFromFormat("w32: bad address %08x trying to write %08x",
                                 address, value));
  }
}
...
void LuaContext::Shutdown() { lua_.reset(); }
    ...
    if (result.valid()) {
      for (const sol::stack_proxy &item : result) {
        ...
        case sol::type::number: {
          ...
          lines_.push_back(LuaLogLine{LogLineType::Integer,
                                      StringFromFormat("%08x (%d)", num, num),
                                      item.get<int>()});
          break;
        }
        case sol::type::string: {
          ...
          lines_.push_back(
              LuaLogLine{LogLineType::String, item.get<std::string>()});
          break;
        }
        default:
          break;
