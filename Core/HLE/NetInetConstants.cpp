...
int convertSocketTypePSP2Host(int type) {
  // FIXME: Masked with 0x0F since there might be additional flags mixed in
  // socket type that need to be converted too
  ...
  return hleLogError(Log::sceNet, type, "Unknown Socket Type") &
         PSP_NET_INET_SOCK_TYPE_MASK;
}
...
int convertSocketProtoPSP2Host(int protocol) {
  ...
  case PSP_NET_INET_IPPROTO_UNSPEC:
    return PSP_NET_INET_IPPROTO_UNSPEC; // 0 only valid if there is only 1
                                        // protocol available for a particular
                                        // domain/family and type?
...
std::string inetSocketProto2str(int protocol) {
  switch (protocol) {
  case PSP_NET_INET_IPPROTO_UNSPEC:
    return "IPPROTO_UNSPEC (DEFAULT?)"; // defaulted to IPPROTO_TCP for
                                        // SOCK_STREAM and IPPROTO_UDP for
                                        // SOCK_DGRAM
  case PSP_NET_INET_IPPROTO_ICMP:
    return "IPPROTO_ICMP";
  case PSP_NET_INET_IPPROTO_IGMP:
...
int convertCMsgTypePSP2Host(int type, int level) {
  ...
  return hleLogError(Log::sceNet, type, "Unknown CMSG_TYPE (Level = %08x)",
                     level);
}
...
int convertSockoptNamePSP2Host(int optname, int level) {
  ...
  return hleLogError(Log::sceNet, optname,
                     "Unknown or unsupported PSP's SockOpt Name (Level = %08x)",
                     level);
}
...
int convertSockoptNameHost2PSP(int optname, int level) {
  ...
  return hleLogError(Log::sceNet, optname,
                     "Unknown Host's SockOpt Name (Level = %08x)", level);
}
...
int convertInetErrnoHost2PSP(int error) {
  ...
  if (error != 0)
    return hleLogError(Log::sceNet, error, "Unknown Host Error Number (%d)",
                       error);
  return error;
}
...
int convertInetErrno2PSPError(int error) {
  ...
  case ERROR_INET_ENETRESET:
    return SCE_KERNEL_ERROR_ERRNO_CONNECTION_RESET;
  case ERROR_INET_EISCONN:
    return SCE_KERNEL_ERROR_ERRNO_ALREADY; // SCE_KERNEL_ERROR_ERRNO_IS_ALREADY_CONNECTED;
                                           // // UNO only check for 0x80010077
                                           // and 0x80010078
  case ERROR_INET_ENOTCONN:
  ...
  if (error != 0)
    return hleLogError(Log::sceNet, error, "Unknown PSP Error Number (%d)",
                       error);
  return error;
}
