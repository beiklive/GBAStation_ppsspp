...
const InfraDNSConfig &GetInfraDNSConfig() { return g_infraDNSConfig; }
...
std::string InfraDNSConfig::ToString() const {
  ...
  w.C("State: ");
  switch (state) {
  case InfraGameState::NotWorking:
    w.C("Not working").endl();
    break;
  case InfraGameState::Working:
    w.C("Working").endl();
    break;
  case InfraGameState::Unknown:
    w.C("Unknown").endl();
    break;
  }
...
bool NetworkWarnUserIfOnlineAndCantSavestate() {
  if (IsNetworkConnected() && !g_Config.bAllowSavestateWhileConnected) {
    ...
    g_OSD.Show(OSDType::MESSAGE_INFO,
               nw->T("Save states are not available when online"), 3.0f,
               "saveonline");
    return true;
  } else {
    return false;
...
bool NetworkWarnUserIfOnlineAndCantSpeed() {
  if (IsNetworkConnected() && !g_Config.bAllowSpeedControlWhileConnected) {
    auto nw = GetI18NCategory(I18NCat::NETWORKING);
    g_OSD.Show(OSDType::MESSAGE_INFO,
               nw->T("Speed controls are not available when online"), 3.0f,
               "speedonline");
    return true;
  } else {
    return false;
...
void AfterApctlMipsCall::DoState(PointerWrap &p) {
  ...
  if (!s)
    return;
  // Just in case there are "s" corruption in the future where s.ver is a
  // negative number
  if (s >= 1) {
    Do(p, handlerID);
    Do(p, oldState);
...
void AfterApctlMipsCall::run(MipsCall &call) {
  ...
  DEBUG_LOG(Log::sceNet,
            "AfterApctlMipsCall::run "
            "[ID=%i][OldState=%d][NewState=%d][Event=%d][Error=%d][ArgsPtr=%"
            "08x] [cbId: %u][retV0: %08x]",
            handlerID, oldState, newState, event, error, argsAddr, call.cbId,
            v0);
  // call.setReturnValue(v0);
}

void AfterApctlMipsCall::SetData(int HandlerID, int OldState, int NewState,
                                 int Event, int Error, u32_le ArgsAddr) {
  handlerID = HandlerID;
  oldState = OldState;
  newState = NewState;
  event = Event;
...
bool LoadDNSForGameID(std::string_view gameID, std::string_view jsonStr,
                      InfraDNSConfig *dns) {
  ...
  *dns = {};
  ...
  if (def) {
    ...
    dns->connectAdHocForGrouping =
        def.getBool("connect_adhoc_for_grouping", false);
  }
  ...
  for (const JsonNode *iter : games->value) {
    ...
    dns->connectAdHocForGrouping = game.getBool("connect_adhoc_for_grouping",
                                                dns->connectAdHocForGrouping);
  ...
  NOTICE_LOG(Log::sceNet, "Loaded DNS config from JSON: %s",
             dns->ToString().c_str());
  return true;
}
...
bool LoadAutoDNS(std::string_view json) {
  ...
  if (!dyn_dns.empty()) {
    ...
    INFO_LOG(Log::sceNet, "DynDNS requested, trying to resolve '%s'...",
             dyn_dns.c_str());
    addrinfo *resolved = nullptr;
    std::string err;
    if (!net::DNSResolve(dyn_dns, "", &resolved, err)) {
      ERROR_LOG(Log::sceNet, "Error resolving, falling back to '%s'",
                dns.c_str());
    } else if (resolved) {
      bool found = false;
      for (auto ptr = resolved; ptr && !found; ptr = ptr->ai_next) {
        ...
        case AF_INET: {
          ...
          if (inet_ntop(ptr->ai_family,
                        ...
                        ipstr, sizeof(ipstr)) != 0) {
            INFO_LOG(Log::sceNet,
                     "Successfully resolved '%s' to '%s', overriding DNS.",
                     dyn_dns.c_str(), ipstr);
            if (g_infraDNSConfig.dns != ipstr) {
              INFO_LOG(Log::sceNet,
                       "Replacing specified DNS IP %s with dyndns %s!",
                       g_infraDNSConfig.dns.c_str(), ipstr);
              g_infraDNSConfig.dns = ipstr;
              // If dyndns is working, we do not need the fixed lookups. So
              // let's kick them.
              INFO_LOG(Log::sceNet, "Clearing fixed DNS lookups.");
              g_infraDNSConfig.fixedDNS.clear();
            } else {
              INFO_LOG(Log::sceNet, "DynDNS: %s already up to date",
                       g_infraDNSConfig.dns.c_str());
            }
            found = true;
          }
          break;
...
static const std::string_view jsonUrl =
    "http://metadata.ppsspp.org/infra-dns.json";
...
void StartInfraJsonDownload() {
  ...
  if (g_infraDL) {
    WARN_LOG(
        Log::sceNet,
        "json is already being downloaded. Still, starting a new download.");
  }

  if (!g_Config.bDontDownloadInfraJson) {
    const char *const acceptMime =
        "application/json, text/*; q=0.9, */*; q=0.8";
    g_infraDL = g_DownloadManager.StartDownload(
        jsonUrl, Path(), http::RequestFlags::Cached24H, acceptMime);
  }
}
...
bool PollInfraJsonDownload(std::string *jsonOutput) {
  ...
  if (g_Config.bDontDownloadInfraJson) {
    NOTICE_LOG(Log::sceNet,
               "As specified by the ini setting DontDownloadInfraJson, using "
               "infra-dns.json directly from /assets");
    size_t jsonSize = 0;
    std::unique_ptr<uint8_t[]> jsonStr(
        g_VFS.ReadFile("infra-dns.json", &jsonSize));
    if (!jsonStr) {
      jsonOutput->clear();
      return true; // A clear output but returning true means something vent
                   // very wrong.
    }
  ...
  if (infraDL->Failed()) {
    // First, fall back to cache if it exists. Could build this functionality
    // into the download manager but it would be a bit awkward.
    std::string json;
    if (File::ReadBinaryFileToString(g_DownloadManager.UrlToCachePath(jsonUrl),
                                     &json) &&
        !json.empty()) {
      WARN_LOG(
          Log::sceNet,
          "Failed to download infra-dns.json, falling back to cached file");
      *jsonOutput = json;
      LoadAutoDNS(*jsonOutput);
      return true;
    }

    // If it doesn't, let's just grab the assets file. Because later this will
    // mean that we didn't even get the cached copy.
    size_t jsonSize = 0;
    std::unique_ptr<uint8_t[]> jsonStr(
        g_VFS.ReadFile("infra-dns.json", &jsonSize));
    if (!jsonStr) {
...
std::string ProcessHostnameWithInfraDNS(const std::string &hostname) {
  ...
  // Resolve any aliases. First check the ini file, then check the hardcoded DNS
  // config.
  ...
  if (aliasIter != g_Config.mHostToAlias.end()) {
    ...
    INFO_LOG(Log::sceNet, "%s - Resolved alias %s from hostname %s",
             __FUNCTION__, alias.c_str(), hostname.c_str());
  ...
  if (g_Config.bInfrastructureAutoDNS) {
    ...
    if (fixedDNSIter != GetInfraDNSConfig().fixedDNS.end()) {
      ...
      INFO_LOG(Log::sceNet,
               "%s - Resolved IP %s from fixed DNS lookup with '%s'",
               __FUNCTION__, domainIP.c_str(), resolvedHostname.c_str());
      resolvedHostname = domainIP;
    }
  }

  // Check if hostname is already an IPv4 address, if so we do not need further
  // lookup. This usually happens after the mHostToAlias or fixedDNSIter
  // lookups, which effectively both are hardcoded DNS.
  ...
  if (net::DirectDNSLookupIPV4(dnsServer.c_str(), resolvedHostname.c_str(),
                               &resolvedAddr)) {
    char temp[32];
    inet_ntop(AF_INET, &resolvedAddr, temp, sizeof(temp));
    INFO_LOG(Log::sceNet, "Direct lookup of '%s' from '%s' succeeded: %s",
             resolvedHostname.c_str(), dnsServer.c_str(), temp);
    return std::string(temp);
  }

  WARN_LOG(
      Log::sceNet,
      "Direct DNS lookup of '%s' at DNS server '%s' failed. Will try OS DNS...",
      resolvedHostname.c_str(), dnsServer.c_str());
  return resolvedHostname;
}
...
void InitLocalhostIP() {
  ...
  isLocalServer = (!strcasecmp(serverStr.c_str(), "localhost") ||
                   serverStr.find("127.") == 0);
}
...
static void __ApctlState(u64 userdata, int cyclesLate) {
  ...
  if (waitID == 0 || error != 0) {
    WARN_LOG(Log::sceNet,
             "sceNetApctl State WaitID(%i) on Thread(%i) already woken up? "
             "(error: %08x)",
             uid, threadID, error);
    return;
  }
  ...
  DEBUG_LOG(Log::sceNet,
            "Returning (WaitID: %d, error: %08x) Result (%08x) of sceNetApctl "
            "- Event: %d, State: %d",
            waitID, error, (int)result, event, netApctlState);
}

// Used to change Apctl State after a delay and before executing callback
...
u32_le __CreateHLELoop(u32_le *loopAddr, const char *sceFuncName,
                       const char *hleFuncName, const char *tagName) {
  ...
  u32_le threadHackAddress = kernelMemory.Alloc(
      blockSize, false,
      tagName); // blockSize will be rounded to 256 granularity
  Memory::Memcpy(threadHackAddress, loopAddr,
                 sizeof(u32_le) * 3); // This area will be cleared again after
                                      // loading an old savestate :(
...
void __NetCallbackInit() {
  ...
  dummyThreadHackAddr =
      __CreateHLELoop(dummyThreadCode, "sceNetAdhoc", "__NetTriggerCallbacks",
                      "dummythreadhack");
  matchingThreadHackAddr =
      __CreateHLELoop(matchingThreadCode, "sceNetAdhocMatching",
                      "__NetMatchingCallbacks", "matchingThreadHack");
  apctlThreadHackAddr = __CreateHLELoop(
      apctlThreadCode, "sceNetApctl", "__NetApctlCallbacks", "apctlThreadHack");

  // Newer one should be placed last to prevent callbacks going to the wrong
  // after action after loading from old save state
  actionAfterMatchingMipsCall =
      __KernelRegisterActionType(AfterMatchingMipsCall::Create);
  actionAfterAdhocMipsCall =
      __KernelRegisterActionType(AfterAdhocMipsCall::Create);
  actionAfterApctlMipsCall =
      __KernelRegisterActionType(AfterApctlMipsCall::Create);
}
...
void __NetInit() {
  ...
  g_adhocServerIP.in.sin_port =
      htons(SERVER_PORT); // 27312 // Maybe read this from config too
  ...
  SceNetEtherAddr mac;
  getLocalMac(&mac);
  INFO_LOG(Log::sceNet, "LocalHost IP will be %s [%s]",
           ip2str(g_localhostIP.in.sin_addr).c_str(), mac2str(&mac).c_str());

  // For libretro we don't have a better place. On other platforms, we just
  // init/shutdown it with the rest of the emu (NativeInit / NativeShutdown).
#ifdef __LIBRETRO__
...
static void __UpdateApctlHandlers(u32 oldState, u32 newState, u32 flag,
                                  u32 error) {
...
void netValidateLoopMemory() {
  // Allocate Memory if it wasn't valid/allocated after loaded from old
  // SaveState
  if (!apctlThreadHackAddr ||
      (apctlThreadHackAddr &&
    ...
    apctlThreadHackAddr =
        kernelMemory.Alloc(blockSize, false, "apctlThreadHack");
    if (apctlThreadHackAddr)
      Memory::Memcpy(apctlThreadHackAddr, apctlThreadCode,
                     sizeof(apctlThreadCode));
  }
}
...
void __NetDoState(PointerWrap &p) {
  ...
  if (s >= 4) {
    ...
    if (actionAfterApctlMipsCall != -1) {
      __KernelRestoreActionType(actionAfterApctlMipsCall,
                                AfterApctlMipsCall::Create);
    }
  ...
  CoreTiming::RestoreRegisterEvent(apctlStateEvent, "__ApctlState",
                                   __ApctlState);
  if (s >= 6) {
    Do(p, netApctlInfoId);
    Do(p, netApctlInfo);
  ...
  if (p.mode == p.MODE_READ) {
    // Let's not change "Inited" value when Loading SaveState in the middle of
    // multiplayer to prevent memory & port leaks
    ...
    apctlEvents.clear();
    // Discard created resolvers for now (since i'm not sure whether the
    // information in the struct is sufficient or not, and we don't support
    // multi-threading yet anyway)
    __NetResolverShutdown();
  }
}
...
void __NetApctlCallbacks() {
  ...
  // We are temporarily borrowing APctl thread for NpAuth callbacks for testing
  // to simulate authentication
  if (NpAuthProcessEvents()) {
    delayus = (adhocEventDelay + adhocExtraDelay);
  }

  // Temporarily borrowing APctl thread for NpMatching2 callbacks for testing
  // purpose
  if (NpMatching2ProcessEvents()) {
    delayus = (adhocEventDelay + adhocExtraDelay);
  }

  // How AP works probably like this: Game use sceNetApctl function ->
  // sceNetApctl let the hardware know and do their's thing and have a new State
  // -> Let the game know the resulting State through Event on their handler
  if (!apctlEvents.empty()) {
    ...
    if (event == PSP_NET_APCTL_EVENT_CONNECT_REQUEST ||
    ...
    // Need to make sure netApctlState is updated before calling the callback's
    // mipscall so the game can GetState()/GetInfo() within their handler's
    // subroutine and make use the new State/Info Should we update NewState &
    // Error accordingly to Event before executing the mipscall ? sceNetApctl*
    // functions might want to set the error value tho, so we probably should
    // leave it untouched, right?
    // error = 0;
    switch (event) {
    case PSP_NET_APCTL_EVENT_CONNECT_REQUEST:
      newState = PSP_NET_APCTL_STATE_JOINING; // Should we set the State to
                                              // PSP_NET_APCTL_STATE_DISCONNECTED
                                              // if there was error?
      if (error != 0)
        apctlEvents.push_front({oldState, PSP_NET_APCTL_STATE_DISCONNECTED,
                                PSP_NET_APCTL_EVENT_ERROR, error});
      else
        apctlEvents.push_front(
            {oldState, newState, PSP_NET_APCTL_EVENT_ESTABLISHED,
             0}); // Should we use PSP_NET_APCTL_EVENT_EAP_AUTH if securityType
                  // is not NONE?
      break;

    case PSP_NET_APCTL_EVENT_ESTABLISHED:
      newState = PSP_NET_APCTL_STATE_GETTING_IP;
      // FIXME: Official prx seems to return ERROR 0x80410280 on the next event
      // when using invalid connection profile to Connect?
      if (error != 0)
        apctlEvents.push_front({oldState, PSP_NET_APCTL_STATE_DISCONNECTED,
                                PSP_NET_APCTL_EVENT_ERROR, error});
      else
        apctlEvents.push_front(
            {oldState, newState, PSP_NET_APCTL_EVENT_GET_IP, 0});
      break;
    ...
    case PSP_NET_APCTL_EVENT_DISCONNECT_REQUEST:
      ...
      delayus = adhocDefaultDelay /
                2; // FIXME: Similar to Adhocctl Disconnect, we probably need to
                   // change the state within a frame-time (or less to be safer)
      break;
    ...
    case PSP_NET_APCTL_EVENT_SCAN_REQUEST:
      ...
      if (error != 0)
        ...
        apctlEvents.push_front(
            {oldState, newState, PSP_NET_APCTL_EVENT_SCAN_COMPLETE, 0});
      break;
    ...
    case PSP_NET_APCTL_EVENT_SCAN_COMPLETE:
      ...
      if (error == 0)
        apctlEvents.push_front(
            {oldState, newState, PSP_NET_APCTL_EVENT_SCAN_STOP, 0});
      break;
    ...
    case PSP_NET_APCTL_EVENT_EAP_AUTH: // Is this suppose to happen between
                                       // JOINING and ESTABLISHED ?
      newState = PSP_NET_APCTL_STATE_EAP_AUTH;
      if (error != 0)
        apctlEvents.push_front({oldState, PSP_NET_APCTL_STATE_DISCONNECTED,
                                PSP_NET_APCTL_EVENT_ERROR, error});
      else
        apctlEvents.push_front({oldState, newState,
                                PSP_NET_APCTL_EVENT_KEY_EXCHANGE,
                                0}); // not sure if KEY_EXCHANGE is the next
                                     // step after AUTH or not tho
      break;

    case PSP_NET_APCTL_EVENT_KEY_EXCHANGE: // Is this suppose to happen between
                                           // JOINING and ESTABLISHED ?
      ...
      if (error != 0)
        ...
        apctlEvents.push_front(
            {oldState, newState, PSP_NET_APCTL_EVENT_ESTABLISHED, 0});
      break;
    ...
    case PSP_NET_APCTL_EVENT_RECONNECT:
      ...
      if (error != 0)
        ...
        apctlEvents.push_front(
            {oldState, newState, PSP_NET_APCTL_EVENT_CONNECT_REQUEST, 0});
      break;
    ...
    // Since 0 is a valid index to types_ we use -1 to detects if it was loaded
    // from an old save state
    if (actionAfterApctlMipsCall < 0) {
      actionAfterApctlMipsCall =
          __KernelRegisterActionType(AfterApctlMipsCall::Create);
    }

    // Run mipscall. Should we skipped executing the mipscall if oldState ==
    // newState?
    for (std::map<int, ApctlHandler>::iterator it = apctlHandlers.begin();
         it != apctlHandlers.end(); ++it) {
      DEBUG_LOG(Log::sceNet,
                "ApctlCallback "
                "[ID=%i][OldState=%d][NewState=%d][Event=%d][Error=%08x]["
                "ArgsPtr=%08x]",
                it->first, oldState, newState, event, error,
                it->second.argument);
      args.data[4] = it->second.argument;
      AfterApctlMipsCall *after =
          (AfterApctlMipsCall *)__KernelCreateAction(actionAfterApctlMipsCall);
      after->SetData(it->first, oldState, newState, event, error,
                     it->second.argument);
      hleEnqueueCall(it->second.entryPoint, 5, args.data, after);
    }
    // Similar to Adhocctl, new State might need to be set after delayed, right
    // before executing the mipscall (ie. simulated beforeAction)
    ScheduleApctlState(event, newState, delayus, "apctl callback state");
    hleNoLogVoid();
    return;
  }

  // Must be delayed long enough whenever there is a pending callback to make
  // sure previous callback & it's afterAction are fully executed
...
u32 Net_Term() {
  // May also need to Terminate netAdhocctl and netAdhoc to free some resources
  // & threads, since the game (ie. GTA:VCS, Wipeout Pulse, etc) might not have
  // called them before calling sceNetTerm and causing them to behave strangely
  // on the next sceNetInit & sceNetAdhocInit
  NetAdhocctl_Term();
...
/*
Parameters:
        poolsize	- Memory pool size (appears to be for the whole of the
networking library). calloutprio	- Priority of the SceNetCallout thread.
        calloutstack	- Stack size of the SceNetCallout thread (defaults to
4096 on non 1.5 firmware regardless of what value is passed). netintrprio
- Priority of the SceNetNetintr thread. netintrstack	- Stack size of the
SceNetNetintr thread (defaults to 4096 on non 1.5 firmware regardless of what
value is passed).
*/
static int sceNetInit(u32 poolSize, u32 calloutPri, u32 calloutStack,
                      u32 netinitPri, u32 netinitStack) {
  // TODO: Create Network Threads using given priority & stack
  // TODO: The correct behavior is actually to allocate more and leak the other
  // threads/pool. But we reset here for historic reasons (GTA:VCS potentially
  // triggers this.)
  if (g_netInited) {
    // This cleanup attempt might not worked when SaveState were loaded in the
    // middle of multiplayer game and re-entering multiplayer, thus causing
    // memory leaks & wasting binded ports. Maybe we shouldn't save/load
    // "Inited" vars on SaveState?
    Net_Term();
  }

  if (poolSize == 0) {
    return hleLogError(Log::sceNet, SCE_KERNEL_ERROR_ILLEGAL_MEMSIZE,
                       "invalid pool size");
  } else if (calloutPri < 0x08 || calloutPri > 0x77) {
    return hleLogError(Log::sceNet, SCE_KERNEL_ERROR_ILLEGAL_PRIORITY,
                       "invalid callout thread priority");
  } else if (netinitPri < 0x08 || netinitPri > 0x77) {
    return hleLogError(Log::sceNet, SCE_KERNEL_ERROR_ILLEGAL_PRIORITY,
                       "invalid init thread priority");
  }

  // TODO: Should also start the threads, probably?  For now, let's just
  // allocate.
  // TODO: Respect the stack size if firmware set to 1.50?
  u32 stackSize = 4096;
  netThread1Addr = AllocUser(stackSize, true, "netstack1");
  if (netThread1Addr == 0) {
    return hleLogError(Log::sceNet, SCE_KERNEL_ERROR_NO_MEMORY,
  ...
  if (netPoolAddr == 0) {
    ...
    FreeUser(netThread2Addr);
    return hleLogError(Log::sceNet, SCE_KERNEL_ERROR_NO_MEMORY,
                       "unable to allocate pool");
  }

  INFO_LOG(Log::sceNet,
           "sceNetInit(poolsize=%d, calloutpri=%i, calloutstack=%d, "
           "netintrpri=%i, netintrstack=%d) at %08x",
           poolSize, calloutPri, calloutStack, netinitPri, netinitStack,
           currentMIPS->pc);

  netMallocStat.pool =
      poolSize -
      0x20; // On Vantage Master Portable this is slightly (32 bytes) smaller
            // than the poolSize arg when tested with JPCSP + prx files
  netMallocStat.maximum = 0x4050; // Dummy maximum foot print
  netMallocStat.free =
      netMallocStat
          .pool; // Dummy free size, we should set this high enough to prevent
                 // any issue (ie. Vantage Master Portable), this is probably
                 // the only field being checked by games?
...
// Normal usage: sceKernelDeleteThread followed by sceNetFreeThreadInfo with the
// same threadID as argument
...
static u32 sceWlanGetEtherAddr(u32 addrAddr) {
  ...
  if (PPSSPP_ID > 1) {
    ...
    // Making sure the 1st 2-bits on the 1st byte of OUI are zero to prevent
    // issue with some games (ie. Gran Turismo)
    addr[0] &= 0xfc;
  } else {
    // Read MAC Address from config
    if (!ParseMacAddress(g_Config.sMACAddress, addr)) {
      ERROR_LOG(Log::sceNet, "Error parsing mac address %s",
                g_Config.sMACAddress.c_str());
...
static u32 sceNetGetLocalEtherAddr(u32 addrAddr) {
  // FIXME: Return 0x80410180 (pspnet[_core] error code?) before successful
  // attempt to Create/Connect/Join a Group? (ie. adhocctlCurrentMode ==
  // ADHOCCTL_MODE_NONE)
...
static void sceNetEtherNtostr(u32 macPtr, u32 bufferPtr) {
  DEBUG_LOG(Log::sceNet, "sceNetEtherNtostr(%08x, %08x) at %08x", macPtr,
            bufferPtr, currentMIPS->pc);
...
static void sceNetEtherStrton(u32 bufferPtr, u32 macPtr) {
  ...
  if (Memory::IsValidAddress(bufferPtr) && Memory::IsValidAddress(macPtr)) {
    ...
    VERBOSE_LOG(Log::sceNet, "sceNetEtherStrton - [%s]",
                mac2str((SceNetEtherAddr *)Memory::GetPointer(macPtr)).c_str());
...
void NetApctl_InitDefaultInfo() {
  memset(&netApctlInfo, 0, sizeof(netApctlInfo));
  // Set dummy/fake parameters, assuming this is the currently selected Network
  // Configuration profile
  // FIXME: Some of these info supposed to be taken from netConfInfo
  int validConfId = std::max(
      1,
      netApctlInfoId); // Should be: sceUtilityGetNetParamLatestID(validConfId);
  truncate_cpy(netApctlInfo.name, sizeof(netApctlInfo.name),
               defaultNetConfigName + std::to_string(validConfId));
  truncate_cpy(netApctlInfo.ssid, sizeof(netApctlInfo.ssid), defaultNetSSID);
  // Default IP Address
  char ipstr[INET_ADDRSTRLEN] = "0.0.0.0"; // Driver 76 needs a dot formatted IP
                                           // instead of a zeroed buffer
  ...
  truncate_cpy(netApctlInfo.secondaryDns, sizeof(netApctlInfo.secondaryDns),
               ipstr);
...
void NetApctl_InitInfo(int confId) {
  memset(&netApctlInfo, 0, sizeof(netApctlInfo));
  // Set dummy/fake values, some of these (ie. IP set to Auto) probably not
  // suppose to have valid info before connected to an AP, right?
  // FIXME: Some of these info supposed to be taken from netConfInfo
  truncate_cpy(netApctlInfo.name, sizeof(netApctlInfo.name),
               defaultNetConfigName + std::to_string(confId));
  truncate_cpy(netApctlInfo.ssid, sizeof(netApctlInfo.ssid), defaultNetSSID);
  memcpy(netApctlInfo.bssid, "\1\1\2\2\3\3",
         sizeof(netApctlInfo.bssid)); // fake AP's mac address
  ...
  if (netApctlInfo.channel == PSP_SYSTEMPARAM_ADHOC_CHANNEL_AUTOMATIC)
    netApctlInfo.channel = defaultWlanChannel;
  // Get Local IP Address
  sockaddr_in sockAddr;
  getLocalIp(&sockAddr); // This will be valid IP, we probably not suppose to
                         // have a valid IP before connected to any AP, right?
  char ipstr[INET_ADDRSTRLEN] =
      "127.0.0.1"; // Patapon 3 seems to try to get current IP using
                   // ApctlGetInfo() right after ApctlInit(), what kind of IP
                   // should we use as default before ApctlConnect()? it
                   // shouldn't be a valid IP, right?
  inet_ntop(AF_INET, &sockAddr.sin_addr, ipstr, sizeof(ipstr));
  truncate_cpy(netApctlInfo.ip, sizeof(netApctlInfo.ip), ipstr);
  // Change the last number to 1 to indicate a common dns server/internet
  // gateway
  ...
  // Games (for example, Wipeout Pulse) often use this to perform their own DNS
  // lookups through UDP, so we need to pass in the configured server. The
  // reason we need autoconfig is that private Servers may need to use custom
  // DNS Server - and most games are now best played on private servers (only a
  // few official ones remain, if any).
  if (g_Config.bInfrastructureAutoDNS) {
    INFO_LOG(Log::sceNet, "Responding to game query with AutoDNS address: %s",
             g_infraDNSConfig.dns.c_str());
    truncate_cpy(netApctlInfo.primaryDns, sizeof(netApctlInfo.primaryDns),
                 g_infraDNSConfig.dns);
  } else {
    INFO_LOG(Log::sceNet,
             "Responding to game query with manual DNS address: %s",
             g_Config.sInfrastructureDNSServer.c_str());
    truncate_cpy(netApctlInfo.primaryDns, sizeof(netApctlInfo.primaryDns),
  ...
  truncate_cpy(netApctlInfo.secondaryDns, sizeof(netApctlInfo.secondaryDns),
               "0.0.0.0"); // Fireteam Bravo 2 seems to try to use secondary DNS
                           // too if it's not 0.0.0.0

  truncate_cpy(netApctlInfo.subNetMask, sizeof(netApctlInfo.subNetMask),
               "255.255.255.0");
}
...
static int sceNetApctlInit(int stackSize, int initPriority) {
  ...
  apctlThreadID = __KernelCreateThread(
      "ApctlThread", __KernelGetCurThreadModuleId(), apctlThreadHackAddr,
      initPriority, stackSize, PSP_THREAD_ATTR_USER, 0, true);
  if (apctlThreadID > 0) {
  ...
  SceNetAdhocctlAdhocId *prodCode =
      (SceNetAdhocctlAdhocId *)Memory::GetCharPointer(apctlProdCodeAddr);
  if (prodCode) {
    memset(prodCode, 0, structsz);
    // TODO: Use a 9-characters product id instead of disc id (ie. not
    // null-terminated(VT_UTF8_SPE) and shouldn't be variable size?)
    std::string discID = g_paramSFO.GetDiscID();
    prodCode->type =
        1; // VT_UTF8 since we're using DiscID instead of product id
    memcpy(prodCode->data, discID.c_str(),
           std::min(ADHOCCTL_ADHOCID_LEN, (int)discID.size()));
  }

  // Actually connect sceNetAdhocctl. TODO: Can we move this to
  // sceNetApctlConnect? Would be good because a lot of games call Init but not
  // Connect on startup.
  if (g_infraDNSConfig.connectAdHocForGrouping) {
    hleCall(sceNetAdhocctl, int, sceNetAdhocctlInit, stackSize, initPriority,
            apctlProdCodeAddr);
  }
...
int NetApctl_Term() {
  // Note: Since we're borrowing AdhocServer for Grouping purpose, we should
  // cleanup too Should we check the g_infraDNSConfig.connectAdHocForGrouping
  // flag here?
  if (g_netApctlInited) {
  ...
  if (apctlThreadID != 0) {
    __KernelStopThread(apctlThreadID, SCE_KERNEL_ERROR_THREAD_TERMINATED,
                       "ApctlThread stopped");
    __KernelDeleteThread(apctlThreadID, SCE_KERNEL_ERROR_THREAD_TERMINATED,
                         "ApctlThread deleted");
    apctlThreadID = 0;
  }
...
static int sceNetApctlGetInfo(int code, u32 pInfoAddr) {
  DEBUG_LOG(Log::sceNet, "sceNetApctlGetInfo(%i, %08x) at %08x", code,
            pInfoAddr, currentMIPS->pc);
  ...
  case PSP_NET_APCTL_INFO_PROFILE_NAME:
    ...
    Memory::MemcpyUnchecked(pInfoAddr, netApctlInfo.name,
                            APCTL_PROFILENAME_MAXLEN);
    NotifyMemInfo(MemBlockFlags::WRITE, pInfoAddr, APCTL_PROFILENAME_MAXLEN,
                  "NetApctlGetInfo");
  ...
  case PSP_NET_APCTL_INFO_BSSID:
    ...
    DEBUG_LOG(Log::sceNet, "ApctlInfo - BSSID: %s",
              mac2str((SceNetEtherAddr *)&netApctlInfo.bssid).c_str());
    NotifyMemInfo(MemBlockFlags::WRITE, pInfoAddr, ETHER_ADDR_LEN,
                  "NetApctlGetInfo");
    break;
  case PSP_NET_APCTL_INFO_SSID:
    ...
    NotifyMemInfo(MemBlockFlags::WRITE, pInfoAddr, APCTL_SSID_MAXLEN,
                  "NetApctlGetInfo");
  ...
  case PSP_NET_APCTL_INFO_IP:
    ...
    NotifyMemInfo(MemBlockFlags::WRITE, pInfoAddr, APCTL_IPADDR_MAXLEN,
                  "NetApctlGetInfo");
  ...
  case PSP_NET_APCTL_INFO_SUBNETMASK:
    if (!Memory::IsValidRange(pInfoAddr, APCTL_IPADDR_MAXLEN))
      return hleLogError(Log::sceNet, -1, "apctl invalid arg");
    Memory::MemcpyUnchecked(pInfoAddr, netApctlInfo.subNetMask,
                            APCTL_IPADDR_MAXLEN);
    NotifyMemInfo(MemBlockFlags::WRITE, pInfoAddr, APCTL_IPADDR_MAXLEN,
                  "NetApctlGetInfo");
    DEBUG_LOG(Log::sceNet, "ApctlInfo - SubNet Mask: %s",
              netApctlInfo.subNetMask);
    break;
  case PSP_NET_APCTL_INFO_GATEWAY:
    if (!Memory::IsValidRange(pInfoAddr, APCTL_IPADDR_MAXLEN))
      return hleLogError(Log::sceNet, -1, "apctl invalid arg");
    Memory::MemcpyUnchecked(pInfoAddr, netApctlInfo.gateway,
                            APCTL_IPADDR_MAXLEN);
  ...
  case PSP_NET_APCTL_INFO_PRIMDNS:
    if (!Memory::IsValidRange(pInfoAddr, APCTL_IPADDR_MAXLEN))
      return hleLogError(Log::sceNet, -1, "apctl invalid arg");
    Memory::MemcpyUnchecked(pInfoAddr, netApctlInfo.primaryDns,
                            APCTL_IPADDR_MAXLEN);
    NotifyMemInfo(MemBlockFlags::WRITE, pInfoAddr, APCTL_IPADDR_MAXLEN,
                  "NetApctlGetInfo");
    DEBUG_LOG(Log::sceNet, "ApctlInfo - Primary DNS: %s",
              netApctlInfo.primaryDns);
    break;
  case PSP_NET_APCTL_INFO_SECDNS:
    if (!Memory::IsValidRange(pInfoAddr, APCTL_IPADDR_MAXLEN))
      return hleLogError(Log::sceNet, -1, "apctl invalid arg");
    Memory::MemcpyUnchecked(pInfoAddr, netApctlInfo.secondaryDns,
                            APCTL_IPADDR_MAXLEN);
    NotifyMemInfo(MemBlockFlags::WRITE, pInfoAddr, APCTL_IPADDR_MAXLEN,
                  "NetApctlGetInfo");
    DEBUG_LOG(Log::sceNet, "ApctlInfo - Secondary DNS: %s",
              netApctlInfo.secondaryDns);
    break;
  ...
  case PSP_NET_APCTL_INFO_PROXY_URL:
    ...
    NotifyMemInfo(MemBlockFlags::WRITE, pInfoAddr, APCTL_URL_MAXLEN,
                  "NetApctlGetInfo");
  ...
  case PSP_NET_APCTL_INFO_WIFISP:
    ...
    break;
  default:
    return hleLogError(Log::sceNet, SCE_NET_APCTL_ERROR_INVALID_CODE,
                       "apctl invalid code");
  }
...
static int NetApctl_AddHandler(u32 handlerPtr, u32 handlerArg) {
  ...
  if (!foundHandler && Memory::IsValidAddress(handlerPtr)) {
    if (apctlHandlers.size() >= MAX_APCTL_HANDLERS) {
      ERROR_LOG(Log::sceNet, "Failed to Add handler(%x, %x): Too many handlers",
                handlerPtr, handlerArg);
      retval =
          SCE_NET_ADHOCCTL_ERROR_TOO_MANY_HANDLERS; // TODO: What's the proper
                                                    // error code for Apctl's
                                                    // TOO_MANY_HANDLERS?
      return retval;
    }
    apctlHandlers[retval] = handler;
    INFO_LOG(Log::sceNet, "Added Apctl handler(%x, %x): %d", handlerPtr,
             handlerArg, retval);
  } else {
    ERROR_LOG(Log::sceNet, "Existing Apctl handler(%x, %x)", handlerPtr,
              handlerArg);
  }
...
int sceNetApctlConnect(int confId) {
  ...
  // Is this confId is the index to the scanning's result data or
  // sceNetApctlGetBSSDescIDListUser result?
  netApctlInfoId = confId;

  // Note: We're borrowing AdhocServer for Grouping purpose, so we can simulate
  // Broadcast over the internet just like Adhoc's pro-online implementation
  int ret = 0;
  if (g_infraDNSConfig.connectAdHocForGrouping) {
  ...
  if (netApctlState == PSP_NET_APCTL_STATE_DISCONNECTED) {
    __UpdateApctlHandlers(0, PSP_NET_APCTL_STATE_JOINING,
  ...
  // user-triggered connection attempt from causing events to piles up
  return hleLogInfo(Log::sceNet, 0, "connect = %i", ret);
}

int sceNetApctlDisconnect() {
  // Like its 'sister' function sceNetAdhocctlDisconnect, we need to alert Apctl
  // handlers that a disconnect took place or else games like Phantasy Star
  // Portable 2 will hang at certain points (e.g. returning to the main menu
  // after trying to connect to PSN). Note: Since we're borrowing AdhocServer
  // for Grouping purpose, we should disconnect too
  if (g_infraDNSConfig.connectAdHocForGrouping) {
  ...
  apctlEvents.clear();
  __UpdateApctlHandlers(netApctlState, PSP_NET_APCTL_STATE_DISCONNECTED,
                        PSP_NET_APCTL_EVENT_DISCONNECT_REQUEST, 0);
  // TODO: Blocks current thread and wait for a state change, but the state
  // should probably need to be changed within 1 frame-time (~16ms)
  return hleLogInfo(Log::sceNet, 0);
}

int NetApctl_GetState() { return netApctlState; }
...
static int sceNetApctlGetState(u32 pStateAddr) {
  // if (!netApctlInited) return hleLogError(Log::sceNet,
  // SCE_NET_APCTL_ERROR_NOT_IN_BSS, "apctl not in bss");
  ...
  return hleLogError(
      Log::sceNet, -1,
      "apctl invalid arg"); // 0x8002013A or ERROR_NET_WLAN_INVALID_ARG ?
}
...
int NetApctl_ScanUser() {
  ...
  __UpdateApctlHandlers(0, PSP_NET_APCTL_STATE_SCANNING,
                        PSP_NET_APCTL_EVENT_SCAN_REQUEST, 0);
  return hleLogInfo(Log::sceNet, 0);
}
...
int NetApctl_GetBSSDescIDListUser(u32 sizeAddr, u32 bufAddr) {
  ...
  // Faking 4 entries, games like MGS:PW Recruit will need to have a different
  // AP for each entry
  int entries = 4;
  ...
  if (bufAddr != 0 && Memory::IsValidAddress(sizeAddr)) {
    ...
    for (int i = 0; i < entries; i++) {
      ...
      DEBUG_LOG(Log::sceNet, "%s writing ID#%d to %08x", __FUNCTION__, i,
                bufAddr + offset);
...
int NetApctl_GetBSSDescEntryUser(int entryId, int infoId, u32 resultAddr) {
  ...
  case PSP_NET_APCTL_DESC_IBSS: // IBSS, 6 bytes
    if (entryId == 0)
      Memory::Memcpy(resultAddr, netApctlInfo.bssid, sizeof(netApctlInfo.bssid),
                     "GetBSSDescEntryUser");
    else {
      ...
      // issue with some games (ie. Gran Turismo)
      dummyMAC[0] &= 0xfc;
      Memory::Memcpy(resultAddr, dummyMAC, sizeof(dummyMAC),
                     "GetBSSDescEntryUser");
    }
    break;
  case PSP_NET_APCTL_DESC_SSID_NAME:
    // Return 32 bytes
    if (entryId == 0)
      Memory::Memcpy(resultAddr, netApctlInfo.ssid, sizeof(netApctlInfo.ssid),
                     "GetBSSDescEntryUser");
    else {
      Memory::Memcpy(resultAddr, dummySSID, sizeof(dummySSID),
                     "GetBSSDescEntryUser");
    }
    break;
  ...
  case PSP_NET_APCTL_DESC_CHANNEL:
    ...
    if (entryId == 0)
      ...
      // Generate channel for testing purposes, not even sure whether this is
      // channel or not, MGS:PW seems to treat the data as u8
  ...
  case PSP_NET_APCTL_DESC_SIGNAL_STRENGTH:
    ...
    if (entryId == 0)
      ...
      // Randomize signal strength between 1%~99% since games like MGS:PW are
      // using signal strength to determine the strength of the recruit
      Memory::Write_U8((int)(((float)rand() / (float)RAND_MAX) * 99.0 + 1.0),
                       resultAddr);
    }
    break;
...
static int sceNetApctlGetBSSDescEntryUser(int entryId, int infoId,
                                          u32 resultAddr) {
  WARN_LOG(Log::sceNet, "UNTESTED %s(%i, %i, %08x)", __FUNCTION__, entryId,
           infoId, resultAddr);
  return NetApctl_GetBSSDescEntryUser(entryId, infoId, resultAddr);
}

static int sceNetApctlScanSSID2() { return NetApctl_ScanUser(); }

/**************
 * Arg1 = output buffer size being filled? (initially the same with Arg3 ?)
 * Arg2 = output buffer? (linked list where the 1st 32-bit is the next address?
 *followed by entry Id? ie. 8-bytes per entry?) Arg3 = max buffer size? (ie.
 *0x100 ?) Arg4 = input flag? (initially 0/1 ?)
 ***************/
static int sceNetApctlGetBSSDescIDList2(u32 Arg1, u32 Arg2, u32 Arg3,
                                        u32 Arg4) {
 ...
 * Arg2 = input field type within the entry desc (ie.
 *PSP_NET_APCTL_DESC_SSID_NAME ?) Arg3 = output buffer for retrieved entry data?
 *(max size = 32 bytes? ie. APCTL_SSID_MAXLEN ? or similar to
 *SceNetApctlInfoInternal union ?)
 ***************/
static int sceNetApctlGetBSSDescEntry2(int entryId, int infoId,
                                       u32 resultAddr) {
  WARN_LOG(Log::sceNet, "UNTESTED %s(%i, %i, %08x) at %08x", __FUNCTION__,
           entryId, infoId, resultAddr, currentMIPS->pc);
  return NetApctl_GetBSSDescEntryUser(entryId, infoId, resultAddr);
}

static int sceNetApctlAddInternalHandler(u32 handlerPtr, u32 handlerArg) {
  ERROR_LOG(Log::sceNet, "UNIMPL %s(%08x, %08x)", __FUNCTION__, handlerPtr,
            handlerArg);
...
static int sceNetApctlScan() { return NetApctl_ScanUser(); }

static int sceNetApctlGetBSSDescIDList(u32 sizeAddr, u32 bufAddr) {
  ERROR_LOG(Log::sceNet, "UNIMPL %s(%08x, %08x)", __FUNCTION__, sizeAddr,
            bufAddr);
...
static int sceNetApctlGetBSSDescEntry(int entryId, int infoId, u32 resultAddr) {
  ERROR_LOG(Log::sceNet, "UNIMPL %s(%i, %i, %08x)", __FUNCTION__, entryId,
            infoId, resultAddr);
...
static int sceNetApctl_lib2_C20A144C(int connIndex, u32 ps3MacAddressPtr) {
  ERROR_LOG(Log::sceNet, "UNIMPL %s(%i, %08x)", __FUNCTION__, connIndex,
            ps3MacAddressPtr);
  // Simple forward, don't need to use hleCall
...
static int sceNetUpnpStart() { return hleLogError(Log::sceNet, 0, "UNIMPL"); }

static int sceNetUpnpStop() { return hleLogError(Log::sceNet, 0, "UNIMPL"); }

static int sceNetUpnpTerm() { return hleLogError(Log::sceNet, 0, "UNIMPL"); }
...
const HLEFunction sceNet[] = {
    ...
    {0X0BF0A3AE, &WrapU_U<sceNetGetLocalEtherAddr>, "sceNetGetLocalEtherAddr",
     'x', "x"},
    {0X50647530, &WrapI_I<sceNetFreeThreadinfo>, "sceNetFreeThreadinfo", 'i',
     "i"},
    {0XCC393E48, &WrapI_U<sceNetGetMallocStat>, "sceNetGetMallocStat", 'i',
     "p"},
...
const HLEFunction sceNetApctl[] = {
    ...
    {0X24FE91A1, &WrapI_V<sceNetApctlDisconnect>, "sceNetApctlDisconnect", 'i',
     ""},
    {0X5DEAC81B, &WrapI_U<sceNetApctlGetState>, "sceNetApctlGetState", 'i',
     "x"},
    {0X8ABADD51, &WrapU_UU<sceNetApctlAddHandler>, "sceNetApctlAddHandler", 'x',
     "xx"},
    {0XE2F91F9B, &WrapI_II<sceNetApctlInit>, "sceNetApctlInit", 'i', "ii"},
    {0X5963991B, &WrapI_U<sceNetApctlDelHandler>, "sceNetApctlDelHandler", 'i',
     "x"},
    {0XB3EDD0EC, &WrapI_V<sceNetApctlTerm>, "sceNetApctlTerm", 'i', ""},
    {0X2BEFDF23, &WrapI_IU<sceNetApctlGetInfo>, "sceNetApctlGetInfo", 'i',
     "ix"},
    ...
    {0XF25A5006, &WrapI_UUUU<sceNetApctlGetBSSDescIDList2>,
     "sceNetApctlGetBSSDescIDList2", 'i', "xxxx"},
    {0X2935C45B, &WrapI_IIU<sceNetApctlGetBSSDescEntry2>,
     "sceNetApctlGetBSSDescEntry2", 'i', "iix"},
    {0X04776994, &WrapI_IIU<sceNetApctlGetBSSDescEntryUser>,
     "sceNetApctlGetBSSDescEntryUser", 'i', "iix"},
    {0X6BDDCB8C, &WrapI_UU<sceNetApctlGetBSSDescIDListUser>,
     "sceNetApctlGetBSSDescIDListUser", 'i', "xx"},
    {0X7CFAB990, &WrapI_UU<sceNetApctlAddInternalHandler>,
     "sceNetApctlAddInternalHandler", 'i', "xx"},
    {0XE11BAFAB, &WrapI_U<sceNetApctlDelInternalHandler>,
     "sceNetApctlDelInternalHandler", 'i', "x"},
    {0XA7BB73DF, &WrapI_UU<sceNetApctl_A7BB73DF>, "sceNetApctl_A7BB73DF", 'i',
     "xx"},
    {0X6F5D2981, &WrapI_U<sceNetApctl_6F5D2981>, "sceNetApctl_6F5D2981", 'i',
     "x"},
    {0X69745F0A, &WrapI_I<sceNetApctl_lib2_69745F0A>,
     "sceNetApctl_lib2_69745F0A", 'i', "i"},
    {0X4C19731F, &WrapI_IU<sceNetApctl_lib2_4C19731F>,
     "sceNetApctl_lib2_4C19731F", 'i', "ix"},
    {0XB3CF6849, &WrapI_V<sceNetApctlScan>, "sceNetApctlScan", 'i', ""},
    {0X0C7FFA5C, &WrapI_UU<sceNetApctlGetBSSDescIDList>,
     "sceNetApctlGetBSSDescIDList", 'i', "xx"},
    {0X96BEB231, &WrapI_IIU<sceNetApctlGetBSSDescEntry>,
     "sceNetApctlGetBSSDescEntry", 'i', "iix"},
    {0XC20A144C, &WrapI_IU<sceNetApctl_lib2_C20A144C>,
     "sceNetApctl_lib2_C20A144C", 'i', "ix"},
...
const HLEFunction sceWlanDrv[] = {
    {0XD7763699, &WrapU_V<sceWlanGetSwitchState>, "sceWlanGetSwitchState", 'x',
     ""},
    {0X0C622081, &WrapU_U<sceWlanGetEtherAddr>, "sceWlanGetEtherAddr", 'x',
     "x"},
...
void Register_sceNetIfhandle() {
  RegisterHLEModule("sceNetIfhandle", ARRAY_SIZE(sceNetIfhandle),
                    sceNetIfhandle);
}