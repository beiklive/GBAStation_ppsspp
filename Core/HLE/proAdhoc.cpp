...
// NOTE: This does not need to be managed by the socket manager - not exposed to
// the game.
...
int defaultWlanChannel =
    PSP_SYSTEMPARAM_ADHOC_CHANNEL_11; // Don't put 0(Auto) here, it needed to be
                                      // a valid/actual channel number
...
bool isMacMatch(const SceNetEtherAddr *addr1, const SceNetEtherAddr *addr2) {
  // Ignoring the 1st byte since there are games (ie. Gran Turismo) who tamper
  // with the 1st byte of OUI to change the unicast/multicast bit
  return (memcmp(((const char *)addr1) + 1, ((const char *)addr2) + 1,
                 ETHER_ADDR_LEN - 1) == 0);
}
...
bool isPTPPortInUse(uint16_t port, bool forListen, SceNetEtherAddr *dstmac,
                    uint16_t dstport) {
  // Iterate Sockets
  for (int i = 0; i < MAX_SOCKET; i++) {
    auto sock = adhocSockets[i];
    if (sock != NULL && sock->type == SOCK_PTP)
      ...
      // to Listen or Open the same PTP port twice (unless destination mac or
      // port are different).
      if (sock->data.ptp.lport == port &&
          ((forListen && sock->data.ptp.state == ADHOC_PTP_STATE_LISTEN) ||
           (!forListen && sock->data.ptp.state != ADHOC_PTP_STATE_LISTEN &&
            sock->data.ptp.pport == dstport && dstmac != nullptr &&
            isMacMatch(&sock->data.ptp.paddr, dstmac)))) {
        return true;
      }
  }
  // Unused Port
...
std::string ip2str(in_addr in, bool maskPublicIP) {
  ...
  if (maskPublicIP && !isPrivateIP(in.s_addr))
    ...
    snprintf(str, sizeof(str), "%u.%u.%u.%u", ipptr[0], ipptr[1], ipptr[2],
             ipptr[3]);
...
addMember(SceNetAdhocMatchingContext *context, SceNetEtherAddr *mac) {
  ...
  // Already existed
  if (peer != NULL) {
    WARN_LOG(Log::sceNet, "Member Peer Already Existed! Updating [%s]",
             mac2str(mac).c_str());
    peer->state = 0;
    peer->sending = 0;
  ...
  // Member is not added yet
    ...
    peer = (SceNetAdhocMatchingMemberInternal *)malloc(
        sizeof(SceNetAdhocMatchingMemberInternal));
    if (peer != NULL) {
...
void addFriend(SceNetAdhocctlConnectPacketS2C *packet) {
  ...
  if (peer != NULL) {
    ...
    WARN_LOG(Log::sceNet, "Friend Peer Already Existed! Updating [%s][%s][%s]",
             ...
             packet->name.data); // inet_ntoa(*(in_addr*)&packet->ip)
    peer->nickname = packet->name;
    peer->mac_addr = packet->mac;
    peer->ip_addr = packet->ip;
    // Calculate final IP-specific Port Offset
    peer->port_offset =
        ((isOriPort && !isPrivateIP(peer->ip_addr)) ? 0 : portOffset);
    // Update TimeStamp
...
// fd is a host socket
int IsSocketReady(int fd, bool readfd, bool writefd, int *errorcode,
                  int timeoutUS) {
  ...
  // Note: select will flags an unconnected TCP socket (ie. a freshly created
  // socket without connecting first, or when connect failed with ECONNREFUSED
  // on linux) as writeable/readable, thus can't be used to tell whether the
  // connection has established or not.
  int ret = select(fd + 1, readfd ? &readfds : nullptr,
                   writefd ? &writefds : nullptr, nullptr, &tval);
  if (errorcode != nullptr)
...
// If they have O_NONBLOCK, use the POSIX way to do it. On POSIX sockets Error
// code would be EINPROGRESS instead of EAGAIN
...
// Otherwise, use the old way of doing it (UNIX way). On UNIX sockets Error code
// would be EAGAIN instead of EINPROGRESS
/*#else
        if (nonblocking) {
                // Change to Non - Blocking Mode
...
int countAvailableNetworks(const bool excludeSelf) {
  ...
  // Count Groups
  for (; group != NULL && (!excludeSelf || !isLocalMAC(&group->bssid.mac_addr));
       group = group->next)
    count++;
  ...
  for (; peer != NULL; peer = peer->next) {
    ...
    if (peer->ip_addr == ip) {
      ...
      */
      ...
      INFO_LOG(Log::sceNet, "Removing Friend Peer %s [%s]",
               mac2str(&peer->mac_addr).c_str(),
               ip2str(*(struct in_addr *)&tmpip)
                   .c_str()); // inet_ntoa(*(in_addr*)&peer->ip_addr)

      // Free Memory
      // free(peer);
      // peer = NULL;
      // Instead of removing it from the list we'll make it timed out since most
      // Matching games are moving group and may still need the peer data thus
      // not recognizing it as Unknown peer
 ...
 */
SceNetAdhocMatchingMemberInternal *
findOutgoingRequest(SceNetAdhocMatchingContext *context) {
  // Iterate Peer List for Matching Target
  SceNetAdhocMatchingMemberInternal *peer = context->peerlist;
  for (; peer != NULL; peer = peer->next) {
    // Found Peer in List
    if (peer->state == PSP_ADHOC_MATCHING_PEER_OUTGOING_REQUEST)
      return peer;
  }
 ...
 */
void postAcceptAddSiblings(SceNetAdhocMatchingContext *context,
                           int siblingcount, SceNetEtherAddr *siblings) {
  // Cast Sibling MAC Array to uint8_t
  // PSP CPU has a problem with non-4-byte aligned Pointer Access.
  // As the buffer of "siblings" isn't properly aligned I don't want to risk a
  // crash.
  uint8_t *siblings_u8 = (uint8_t *)siblings;

  peerlock.lock();
  // Iterate Siblings. Reversed so these siblings are added into peerlist in the
  // same order with the peerlist on host/parent side
  for (int i = siblingcount - 1; i >= 0; i--) {
    SceNetEtherAddr *mac =
        (SceNetEtherAddr *)(siblings_u8 + sizeof(SceNetEtherAddr) * i);
    ...
    if (peer != NULL) {
      ...
      // Allocate Memory
      SceNetAdhocMatchingMemberInternal *sibling =
          (SceNetAdhocMatchingMemberInternal *)malloc(
              sizeof(SceNetAdhocMatchingMemberInternal));
      ...
      if (sibling != NULL) {
        ...
        sibling->lastping =
            CoreTiming::GetGlobalTimeUsScaled(); // time_now_d()*1000000.0;

        // Link Peer
        sibling->next = context->peerlist;
        context->peerlist = sibling;

        // Spawn Established Event. FIXME: ESTABLISHED event should only be
        // triggered for Parent/P2P peer?
        // spawnLocalEvent(context, PSP_ADHOC_MATCHING_EVENT_ESTABLISHED,
        // &sibling->mac, 0, NULL);

        INFO_LOG(Log::sceNet, "Accepting Sibling Peer %s",
                 mac2str(&sibling->mac).c_str());
      }
    }
  }
  peerlock.unlock();
 ...
 * @return Internal Peer Reference or... NULL
 */
SceNetAdhocMatchingMemberInternal *findPeer(SceNetAdhocMatchingContext *context,
                                            SceNetEtherAddr *mac) {
  if (mac == NULL)
    return NULL;
 ...
 */
SceNetAdhocMatchingMemberInternal *
findParent(SceNetAdhocMatchingContext *context) {
  // Iterate Peer List for Matching Target
  SceNetAdhocMatchingMemberInternal *peer = context->peerlist;
  for (; peer != NULL; peer = peer->next) {
    // Found Peer in List
...
SceNetAdhocMatchingMemberInternal *findP2P(SceNetAdhocMatchingContext *context,
  ...
  for (; peer != NULL; peer = peer->next) {
    ...
    if (!excludeTimedout || peer->lastping != 0)
      // Found Peer in List
      if (peer->state == PSP_ADHOC_MATCHING_PEER_P2P)
        return peer;
  }
 ...
 */
void deletePeer(SceNetAdhocMatchingContext *context,
                SceNetAdhocMatchingMemberInternal *&peer) {
  // Valid Arguments
  if (context != NULL && peer != NULL) {
    ...
    if (item != NULL) {
      ...
      // Beginning Item
        ...
        context->peerlist = item->next;

      INFO_LOG(Log::sceNet, "Removing Member Peer %s",
               mac2str(&peer->mac).c_str());
    }
 ...
 */
void linkEVMessage(SceNetAdhocMatchingContext *context,
 ...
 * @param message Thread Message Pointer
 */
void linkIOMessage(SceNetAdhocMatchingContext *context,
                   ThreadMessage *message) {
  // Lock Access
 ...
 */
void sendGenericMessage(SceNetAdhocMatchingContext *context, int stack,
                        SceNetEtherAddr *mac, int opcode, int optlen,
                        const void *opt) {
  ...
  if (memory != NULL) {
    ...
    if (stack == PSP_ADHOC_MATCHING_EVENT_STACK)
      linkEVMessage(context, header);

    // Link Thread Message to Input Stack
    else
      linkIOMessage(context, header);
 ...
 * @param opt Optional Data
 */
void sendAcceptMessage(SceNetAdhocMatchingContext *context,
                       SceNetAdhocMatchingMemberInternal *peer, int optlen,
                       const void *opt) {
  // Send Accept Message
  sendGenericMessage(context, PSP_ADHOC_MATCHING_INPUT_STACK, &peer->mac,
                     PSP_ADHOC_MATCHING_PACKET_ACCEPT, optlen, opt);
}
 ...
 * @param opt Optional Data
 */
void sendJoinRequest(SceNetAdhocMatchingContext *context,
                     SceNetAdhocMatchingMemberInternal *peer, int optlen,
                     const void *opt) {
  // Send Join Message
  sendGenericMessage(context, PSP_ADHOC_MATCHING_INPUT_STACK, &peer->mac,
                     PSP_ADHOC_MATCHING_PACKET_JOIN, optlen, opt);
}
 ...
 * @param opt Optional Data
 */
void sendCancelMessage(SceNetAdhocMatchingContext *context,
                       SceNetAdhocMatchingMemberInternal *peer, int optlen,
                       const void *opt) {
  // Send Cancel Message
  sendGenericMessage(context, PSP_ADHOC_MATCHING_INPUT_STACK, &peer->mac,
                     PSP_ADHOC_MATCHING_PACKET_CANCEL, optlen, opt);
}
 ...
 * @param data Data
 */
void sendBulkData(SceNetAdhocMatchingContext *context,
                  SceNetAdhocMatchingMemberInternal *peer, int datalen,
                  const void *data) {
  // Send Bulk Data Message
  sendGenericMessage(context, PSP_ADHOC_MATCHING_INPUT_STACK, &peer->mac,
                     PSP_ADHOC_MATCHING_PACKET_BULK, datalen, data);
}
 ...
 * @param peer Target Peer
 */
void abortBulkTransfer(SceNetAdhocMatchingContext *context,
                       SceNetAdhocMatchingMemberInternal *peer) {
  // Send Bulk Data Abort Message
  sendGenericMessage(context, PSP_ADHOC_MATCHING_INPUT_STACK, &peer->mac,
                     PSP_ADHOC_MATCHING_PACKET_BULK_ABORT, 0, NULL);
}
 ...
 * @param peer New Kid
 */
void sendBirthMessage(SceNetAdhocMatchingContext *context,
                      SceNetAdhocMatchingMemberInternal *peer) {
  // Send Birth Message
  sendGenericMessage(context, PSP_ADHOC_MATCHING_INPUT_STACK, &peer->mac,
                     PSP_ADHOC_MATCHING_PACKET_BIRTH, 0, NULL);
}
 ...
 */
void sendDeathMessage(SceNetAdhocMatchingContext *context,
                      SceNetAdhocMatchingMemberInternal *peer) {
  // Send Death Message
  sendGenericMessage(context, PSP_ADHOC_MATCHING_INPUT_STACK, &peer->mac,
                     PSP_ADHOC_MATCHING_PACKET_DEATH, 0, NULL);
}
...
uint32_t countConnectedPeers(SceNetAdhocMatchingContext *context,
  ...
  // Child Mode
    ...
    if (findParent(context) != NULL) {
      ...
      count += countChildren(context, excludeTimedout) +
               1; // Since count is already started from 1, Do we need to +1
                  // here? Ys vs. Sora no Kiseki seems to show wrong number of
                  // players without +1 here
    }
  }
 ...
 * @param opt Optional Data
 */
void spawnLocalEvent(SceNetAdhocMatchingContext *context, int event,
                     SceNetEtherAddr *mac, int optlen, void *opt) {
  // Spawn Local Event
  sendGenericMessage(context, PSP_ADHOC_MATCHING_EVENT_STACK, mac, event,
                     optlen, opt);
}
...
void handleTimeout(SceNetAdhocMatchingContext *context) {
  ...
  while (peer != NULL && contexts != NULL && coreState != CORE_POWERDOWN) {
    ...
    // Timeout! Apparently the latest GetGlobalTimeUsScaled (ie. now) have a
    // possibility to be smaller than previous GetGlobalTimeUsScaled (ie.
    // lastping) thus resulting a negative number when subtracted :(
    if (peer->state != 0 && static_cast<s64>(now - peer->lastping) >
                                static_cast<s64>(context->timeout)) {
      // Spawn Timeout Event. FIXME: Should we allow TIMEOUT Event to intervene
      // joining process of Parent-Child too just like P2P Mode? (ie. Crazy Taxi
      // uses P2P Mode)
      if ((context->mode == PSP_ADHOC_MATCHING_MODE_CHILD &&
           peer->state == PSP_ADHOC_MATCHING_PEER_PARENT) ||
          (context->mode == PSP_ADHOC_MATCHING_MODE_PARENT &&
           peer->state == PSP_ADHOC_MATCHING_PEER_CHILD) ||
        ...
        // FIXME: TIMEOUT event should only be triggered on Parent/P2P mode and
        // for Parent/P2P peer?
        spawnLocalEvent(context, PSP_ADHOC_MATCHING_EVENT_TIMEOUT, &peer->mac,
                        0, NULL);

        INFO_LOG(Log::sceNet,
                 "TimedOut Member Peer %s (%lld - %lld = %lld > %lld us)",
                 mac2str(&peer->mac).c_str(), now, peer->lastping,
                 (now - peer->lastping), context->timeout);
...
void clearPeerList(SceNetAdhocMatchingContext *context) {
  ...
  while (peer != NULL) {
    // Grab Next Pointer
    context->peerlist =
        peer->next; // SceNetAdhocMatchingMemberInternal * next = peer->next;

    // Delete Peer
    free(peer); // deletePeer(context, peer);
    // Instead of removing peer immediately, We should give a little time before
    // removing the peer and let it timed out? just in case the game is in the
    // middle of communicating with the peer on another thread so it won't
    // recognize it as Unknown peer
    // peer->lastping = CoreTiming::GetGlobalTimeUsScaled();
...
// It seems After Actions being called in reverse order of Mipscall order (ie.
// MipsCall order of ACCEPT(6)->ESTABLISH(7) getting AfterAction order of
// ESTABLISH(7)->ACCEPT(6)
void AfterMatchingMipsCall::run(MipsCall &call) {
  ...
  if (__IsInInterrupt())
    ERROR_LOG(Log::sceNet,
              "AfterMatchingMipsCall::run [ID=%i][Event=%d] is Returning "
              "Inside an Interrupt!",
              contextID, EventID);
  // SetMatchingInCallback(context, false);
  DEBUG_LOG(Log::sceNet,
            "AfterMatchingMipsCall::run [ID=%i][Event=%d][%s] [cbId: "
            "%u][retV0: %08x]",
            contextID, EventID,
            mac2str((SceNetEtherAddr *)Memory::GetPointer(bufAddr)).c_str(),
            call.cbId, v0);
  if (Memory::IsValidAddress(bufAddr))
    userMemory.Free(bufAddr);
  // call.setReturnValue(v0);
}

void AfterMatchingMipsCall::SetData(int ContextID, int eventId,
                                    u32_le BufAddr) {
  contextID = ContextID;
  EventID = eventId;
  bufAddr = BufAddr;
...
void AfterAdhocMipsCall::run(MipsCall &call) {
  ...
  if (__IsInInterrupt())
    ERROR_LOG(Log::sceNet,
              "AfterAdhocMipsCall::run [ID=%i][Event=%d] is Returning Inside "
              "an Interrupt!",
              HandlerID, EventID);
  SetAdhocctlInCallback(false);
  isAdhocctlBusy = false;
  DEBUG_LOG(Log::sceNet,
            "AfterAdhocMipsCall::run [ID=%i][Event=%d] [cbId: %u][retV0: %08x]",
            HandlerID, EventID, call.cbId, v0);
  // call.setReturnValue(v0);
}
...
// Make sure MIPS calls have been fully executed before the next
...
void sendChat(const std::string &chatString) {
  ...
  // TODO check network inited, check send success or not, chatlog.pushback
  // error on failed send, pushback error on not connected
  if (friendFinderRunning) {
    // Send Chat to Server
    if (!chatString.empty()) {
      // maximum char allowed is 64 character for compability with original
      // server (pro.coldbird.net)
      std::string message = chatString.substr(
          0, 60); // 64 return chat variable corrupted is it out of memory?
      strcpy(chat.message, message.c_str());
      // Send Chat Messages
      if (IsSocketReady((int)metasocket, false, true) > 0) {
        int chatResult = (int)send((int)metasocket, (const char *)&chat,
                                   sizeof(chat), MSG_NOSIGNAL);
    ...
    chatLog.push_back(std::string(
        n->T("You're in Offline Mode, go to lobby or online hall")));
    chatMessageGeneration++;
  }
}
...
// TODO: We should probably change this thread into PSPThread (or merging it
// into the existing AdhocThread PSPThread) as there are too many global vars
// being used here which also being used within some HLEs
int friendFinder() {
  ...
  // #ifdef __SWITCH__
  //   FILE *fp = fopen("sdmc:/tiicu/debug/debug_core.txt", "a");
  //   if (fp) {
  //     fprintf(fp, "[FriendFinder] Thread started. Server: %s\n",
  ...
  // #ifdef __SWITCH__
  //   fp = fopen("sdmc:/tiicu/debug/debug_core.txt", "a");
  //   if (fp) {
  //     fprintf(fp, "[FriendFinder] Resolving DNS...\n");
  //     fclose(fp);
  //   }
  // #endif

  if (g_Config.bEnableWlan &&
      !net::DNSResolve(g_Config.sProAdhocServer, "", &resolved, err)) {
    ERROR_LOG(Log::sceNet, "DNS Error Resolving %s\n",
              g_Config.sProAdhocServer.c_str());
    g_OSD.Show(OSDType::MESSAGE_ERROR,
               std::string(n->T("DNS Error Resolving")) +
  ...
  //   fp = fopen("sdmc:/tiicu/debug/debug_core.txt", "a");
  //   if (fp) {
  //     fprintf(fp, "[FriendFinder] DNS Resolved (or skipped/failed).\n");
  //     fclose(fp);
  //   }
  // #endif
  ...
  //   fp = fopen("sdmc:/tiicu/debug/debug_core.txt", "a");
  //   if (fp) {
  //     fprintf(fp, "[FriendFinder] Entering loop.\n");
  //     fclose(fp);
  //   }
  // #endif
  while (friendFinderRunning) {
    ...
    if (metasocket == (int)INVALID_SOCKET && netAdhocctlInited &&
      ...
      if (g_Config.bEnableWlan) {
        ...
        if (initNetwork(&product_code) == 0) {
          ...
          // At this point we are most-likely not in a Group within the Adhoc
          // Server, so we should probably reset AdhocctlState
    ...
    if (g_adhocServerConnected) {
      // Ping Server
      now = time_now_d() *
            1000000.0; // Use time_now_d()*1000000.0 instead of
                       // CoreTiming::GetGlobalTimeUsScaled() if the game gets
                       // disconnected from AdhocServer too soon when FPS wasn't
                       // stable
      // original code : ((sceKernelGetSystemTimeWide() - lastping) >=
      // ADHOCCTL_PING_TIMEOUT)
      if (static_cast<s64>(now - lastping) >=
          PSP_ADHOCCTL_PING_TIMEOUT) { // We may need to use lower interval to
                                       // prevent getting timeout at Pro Adhoc
                                       // Server through internet
        // Prepare Packet
        uint8_t opcode = OPCODE_PING;

        // Send Ping to Server, may failed with socket error 10054/10053 if
        // someone else with the same IP already connected to AdHoc Server (the
        // server might need to be modified to differentiate MAC instead of IP)
        if (IsSocketReady((int)metasocket, false, true) > 0) {
          int iResult = (int)send((int)metasocket, (const char *)&opcode, 1,
                                  MSG_NOSIGNAL);
          ...
          if (iResult == SOCKET_ERROR) {
            ERROR_LOG(
                Log::sceNet,
                "FriendFinder: Socket Error (%i) when sending OPCODE_PING",
                error);
            if (error != EAGAIN && error != EWOULDBLOCK) {
              ...
              metasocket = (int)INVALID_SOCKET;
              g_OSD.Show(OSDType::MESSAGE_ERROR,
                         std::string(n->T("Disconnected from AdhocServer")) +
                             " (" + std::string(n->T("Error")) + ": " +
                             std::to_string(error) + ")");
              // Mark all friends as timedout since we won't be able to detects
              // disconnected friends anymore without being connected to Adhoc
              // Server
              peerlock.lock();
            ...
            // Update Ping Time
            lastping = now;
            VERBOSE_LOG(Log::sceNet, "FriendFinder: Sending OPCODE_PING (%llu)",
      ...
      if (IsSocketReady((int)metasocket, true, false) > 0) {
        int received = (int)recv((int)metasocket, (char *)(rx + rxpos),
                                 sizeof(rx) - rxpos, MSG_NOSIGNAL);
        ...
        if (received > 0) {
          ...
          INFO_LOG(Log::sceNet, "Received %d Bytes of Data from Adhoc Server",
                   received);
        }
      }

      // Calculate EnterGameMode Timeout to prevent waiting forever for
      // disconnected players
      if (isAdhocctlBusy && adhocctlState == ADHOCCTL_STATE_DISCONNECTED &&
          adhocctlCurrentMode == ADHOCCTL_MODE_GAMEMODE &&
          netAdhocGameModeEntered &&
          static_cast<s64>(now - adhocctlStartTime) >
              netAdhocEnterGameModeTimeout) {
        netAdhocGameModeEntered = false;
      ...
      if (rxpos > 0) {
        ...
        if (rx[0] == OPCODE_CONNECT_BSSID) {
          ...
          if (rxpos >= (int)sizeof(SceNetAdhocctlConnectBSSIDPacketS2C)) {
            // Cast Packet
            SceNetAdhocctlConnectBSSIDPacketS2C *packet =
                (SceNetAdhocctlConnectBSSIDPacketS2C *)rx;

            INFO_LOG(Log::sceNet,
                     "FriendFinder: Incoming OPCODE_CONNECT_BSSID [%s]",
                     mac2str(&packet->mac).c_str());
            // Update Group BSSID
            parameter.bssid.mac_addr =
                packet->mac; // This packet seems to contains Adhoc Group
                             // Creator's BSSID (similar to AP's BSSID) so it
                             // shouldn't get mixed up with local MAC address.
                             // Note: On JPCSP + prx files params.bssid is
                             // hardcoded to "Jpcsp\0" and doesn't match to any
                             // of player's mac

            // From JPCSP: Some games have problems when the
            // PSP_ADHOCCTL_EVENT_CONNECTED is sent too quickly after connecting
            // to a network. The connection will be set CONNECTED with a small
            // delay (200ms or 200us?) Notify Event Handlers
            if (adhocctlCurrentMode == ADHOCCTL_MODE_GAMEMODE) {
              ...
              if (std::find_if(gameModeMacs.begin(), gameModeMacs.end(),
                ...
                // Arrange the order to be consistent on all players (Host on
                // top), Starting from our self the rest of new players will be
                // added to the back
                gameModeMacs.push_back(localMac);

                // FIXME: OPCODE_CONNECT_BSSID only triggered once, but the
                // timing of ADHOCCTL_EVENT_GAME notification could be too soon,
                // since there could be more players that need to join before
                // the event should be notified
                if (netAdhocGameModeEntered &&
                    gameModeMacs.size() >= requiredGameModeMacs.size()) {
                  notifyAdhocctlHandlers(ADHOCCTL_EVENT_GAME, 0);
                }
              } else
                WARN_LOG(Log::sceNet,
                         "GameMode SelfMember [%s] Already Existed!",
                         mac2str(&localMac).c_str());
            } else {
            ...
            // Move RX Buffer
            memmove(rx, rx + sizeof(SceNetAdhocctlConnectBSSIDPacketS2C),
                    sizeof(rx) - sizeof(SceNetAdhocctlConnectBSSIDPacketS2C));
        ...
        // Chat Packet
          ...
          if (rxpos >= (int)sizeof(SceNetAdhocctlChatPacketS2C)) {
            ...
            SceNetAdhocctlChatPacketS2C *packet =
                (SceNetAdhocctlChatPacketS2C *)rx;
            ...
            NOTICE_LOG(Log::sceNet, "Received chat message %s",
                       packet->base.message);
            std::string incoming = "";
            ...
            // Move RX Buffer
            memmove(rx, rx + sizeof(SceNetAdhocctlChatPacketS2C),
                    sizeof(rx) - sizeof(SceNetAdhocctlChatPacketS2C));
        ...
        // Connect Packet
          ...
          if (rxpos >= (int)sizeof(SceNetAdhocctlConnectPacketS2C)) {
            ...
            SceNetAdhocctlConnectPacketS2C *packet =
                (SceNetAdhocctlConnectPacketS2C *)rx;
            ...
            INFO_LOG(Log::sceNet,
                     "FriendFinder: Incoming OPCODE_CONNECT [%s][%s][%s]",
                     mac2str(&packet->mac).c_str(),
                     ip2str(*(in_addr *)&ipaddr).c_str(), packet->name.data);

            // Add User
            addFriend(packet);

            // Make sure GameMode participants are all joined (including self
            // MAC)
            if (adhocctlCurrentMode == ADHOCCTL_MODE_GAMEMODE) {
              if (std::find_if(gameModeMacs.begin(), gameModeMacs.end(),
                ...
                // Arrange the order to be consistent on all players (Host on
                // top), Existing players are sent in reverse by AdhocServer
                SceNetEtherAddr localMac;
                getLocalMac(&localMac);
                ...
                // Starting from our self the rest of new players will be added
                // to the back
                ...
                // From JPCSP: Join complete when all the required MACs have
                // joined
                if (netAdhocGameModeEntered &&
                    requiredGameModeMacs.size() > 0 &&
                    gameModeMacs.size() == requiredGameModeMacs.size()) {
                  // TODO: Should we replace gameModeMacs contents with
                  // requiredGameModeMacs contents to make sure they are in the
                  // same order with macs from
                  // sceNetAdhocctlCreateEnterGameMode? But may not be
                  // consistent with the list on client side!
                  // gameModeMacs = requiredGameModeMacs;
                  notifyAdhocctlHandlers(ADHOCCTL_EVENT_GAME, 0);
                }
              } else
                WARN_LOG(Log::sceNet, "GameMode Member [%s] Already Existed!",
                         mac2str(&packet->mac).c_str());
            }
            ...
            // Move RX Buffer
            memmove(rx, rx + sizeof(SceNetAdhocctlConnectPacketS2C),
                    sizeof(rx) - sizeof(SceNetAdhocctlConnectPacketS2C));
        ...
        // Disconnect Packet
          ...
          if (rxpos >= (int)sizeof(SceNetAdhocctlDisconnectPacketS2C)) {
            ...
            SceNetAdhocctlDisconnectPacketS2C *packet =
                (SceNetAdhocctlDisconnectPacketS2C *)rx;
            ...
            // Delete User by IP, should delete by MAC since IP can be shared
            // (behind NAT) isn't?
            ...
            // Move RX Buffer
            memmove(rx, rx + sizeof(SceNetAdhocctlDisconnectPacketS2C),
                    sizeof(rx) - sizeof(SceNetAdhocctlDisconnectPacketS2C));
        ...
        // Scan Packet
          ...
          if (rxpos >= (int)sizeof(SceNetAdhocctlScanPacketS2C)) {
            ...
            SceNetAdhocctlScanPacketS2C *packet =
                (SceNetAdhocctlScanPacketS2C *)rx;
            ...
            SceNetAdhocctlScanInfo *group = (SceNetAdhocctlScanInfo *)malloc(
                sizeof(SceNetAdhocctlScanInfo));

            // Allocated Structure Data
            if (group != NULL) {
              // Clear Memory, should this be done only when allocating new
              // group?
              ...
              // Since 0 is not a valid active channel we fake the channel for
              // Automatic Channel (JPCSP use 11 as default). Ridge Racer 2 will
              // ignore any groups with channel 0 or that doesn't matched with
              // channel value returned from sceUtilityGetSystemParamInt (which
              // mean sceUtilityGetSystemParamInt must not return channel 0 when
              // connected to a network?)
              group->channel =
                  parameter.channel; //(parameter.channel ==
                                     // PSP_SYSTEMPARAM_ADHOC_CHANNEL_AUTOMATIC)
                                     //? defaultWlanChannel : parameter.channel;
              // This Mode should be a valid mode (>=0), probably should be sent
              // by AdhocServer since there are 2 possibilities (Normal and
              // GameMode). Air Conflicts - Aces Of World War 2 (which use
              // GameMode) seems to relies on this Mode value.
              group->mode = std::max(
                  ADHOCCTL_MODE_NORMAL,
                  adhocctlCurrentMode); // default to ADHOCCTL_MODE_NORMAL
            ...
            // Move RX Buffer
            memmove(rx, rx + sizeof(SceNetAdhocctlScanPacketS2C),
                    sizeof(rx) - sizeof(SceNetAdhocctlScanPacketS2C));
    ...
    sleep_ms(10, "pro-adhoc-poll-2"); // Using 1ms for faster response just like
                                      // AdhocServer?

    // Don't do anything if it's paused, otherwise the log will be flooded
    while (Core_IsStepping() && coreState != CORE_POWERDOWN &&
           friendFinderRunning)
  ...
  // Prevent the games from having trouble to reInitiate Adhoc (the next NetInit
  // -> PdpCreate after NetTerm)
  adhocctlState = ADHOCCTL_STATE_DISCONNECTED;
...
int getLocalIp(sockaddr_in *SocketAddress) {
  ...
  if (metasocket != (int)INVALID_SOCKET) {
    ...
    int ret =
        getsockname((int)metasocket, (struct sockaddr *)&localAddr, &addrLen);
    // Note: Sometimes metasocket still contains a valid socket fd right after
    // failed to connect to AdhocServer on a different thread, thus ended with
    // 0.0.0.0 here
...
// getifaddrs first appeared in glibc 2.3, On Android officially supported since
// __ANDROID_API__ >= 24
#if (defined(_IFADDRS_H_) || (__GLIBC__ > 2) ||                                \
     (__GLIBC__ == 2 && __GLIBC_MINOR__ >= 3) || (__ANDROID_API__ >= 24))
  ...
  if (sock != SOCKET_ERROR) {
    const char *kGoogleDnsIp =
        "8.8.8.8"; // Needs to be an IP string so it can be resolved as fast as
                   // possible to IP, doesn't need to be reachable
    ...
    int err =
        connect(sock, (struct sockaddr *)&serv,
                sizeof(serv)); // connect should succeed even with SOCK_DGRAM
    if (err != SOCKET_ERROR) {
      ...
      if (err != SOCKET_ERROR) {
        SocketAddress->sin_addr =
            name.sin_addr; // May be we should cache this so it doesn't need to
                           // use connect all the time, or even better cache it
                           // when connecting to adhoc server to get an accurate
                           // IP
        closesocket(sock);
        return 0;
      }
...
static std::vector<std::pair<uint32_t, uint32_t>> InitPrivateIPRanges() {
  ...
  if (1 == inet_pton(AF_INET, "192.168.0.0", &(saNet.sin_addr)) &&
      1 == inet_pton(AF_INET, "255.255.0.0", &(saMask.sin_addr)))
    ip_ranges.push_back({saNet.sin_addr.s_addr, saMask.sin_addr.s_addr});
  if (1 == inet_pton(AF_INET, "172.16.0.0", &(saNet.sin_addr)) &&
      1 == inet_pton(AF_INET, "255.240.0.0", &(saMask.sin_addr)))
    ip_ranges.push_back({saNet.sin_addr.s_addr, saMask.sin_addr.s_addr});
  if (1 == inet_pton(AF_INET, "10.0.0.0", &(saNet.sin_addr)) &&
      1 == inet_pton(AF_INET, "255.0.0.0", &(saMask.sin_addr)))
    ip_ranges.push_back({saNet.sin_addr.s_addr, saMask.sin_addr.s_addr});
  if (1 == inet_pton(AF_INET, "127.0.0.0", &(saNet.sin_addr)) &&
      1 == inet_pton(AF_INET, "255.0.0.0", &(saMask.sin_addr)))
    ip_ranges.push_back({saNet.sin_addr.s_addr, saMask.sin_addr.s_addr});
  if (1 == inet_pton(AF_INET, "169.254.0.0", &(saNet.sin_addr)) &&
      1 == inet_pton(AF_INET, "255.255.0.0", &(saMask.sin_addr)))
    ip_ranges.push_back({saNet.sin_addr.s_addr, saMask.sin_addr.s_addr});
...
bool isPrivateIP(uint32_t ip) {
  static const std::vector<std::pair<uint32_t, uint32_t>> ip_ranges =
      InitPrivateIPRanges();
  for (auto &ipRange : ip_ranges) {
    if ((ip & ipRange.second) ==
        (ipRange.first &
         ipRange.second)) // We can just use ipRange.first directly if it's
...
bool isLoopbackIP(uint32_t ip) { return ((uint8_t *)&ip)[0] == 0x7f; }

bool isMulticastIP(uint32_t ip) { return ((ip & 0xF0) == 0xE0); }
...
void getLocalMac(SceNetEtherAddr *addr) {
  ...
  if (PPSSPP_ID > 1) {
    ...
    mac[0] &= 0xfc;
  } else if (!ParseMacAddress(g_Config.sMACAddress, mac)) {
...
u_long getAvailToRecv(int sock, int udpBufferSize) {
  ...
  int err = -1;
  // Note: FIONREAD may have different behavior depends on the platform,
  // according to
  // https://stackoverflow.com/questions/9278189/how-do-i-get-amount-of-queued-data-for-udp-socket/9296481#9296481
#if defined(_WIN32)
  ...
  if (udpBufferSize > 0 && n > 0) {
    // TODO: May need to filter out packets from an IP that can't be translated
    // to MAC address
    // TODO: Cap number of bytes of full DGRAM message(s) up to buffer size, but
    // may cause Warriors Orochi 2 to get FPS drops
  }
  return n;
}
...
int getSockBufferSize(int sock, int opt) { // opt = SO_RCVBUF/SO_SNDBUF
  int n = PSP_ADHOC_PDP_MFS * 2; // 16384; // The value might be twice of the
                                 // value being set using setsockopt
  socklen_t m = sizeof(n);
  getsockopt(sock, SOL_SOCKET, opt, (char *)&n, &m);
  return (n);
}

int setSockBufferSize(int sock, int opt,
                      int size) { // opt = SO_RCVBUF/SO_SNDBUF
  int n = size;                   // 8192;
  switch (opt) {
  case SO_RCVBUF:
    n = std::max(size, 128);
    break; // FIXME: The minimum (doubled) value for SO_RCVBUF is 256 ?
           // (2048+MTU+padding on newer OS? TCP_SKB_MIN_TRUESIZE)
  case SO_SNDBUF:
    n = std::max(size, 1024);
    break; // FIXME: The minimum (doubled) value for SO_SNDBUF is 2048 ? (twice
           // the minimum of SO_RCVBUF on newer OS? TCP_SKB_MIN_TRUESIZE * 2)
  }
...
int setSockTimeout(
    int sock, int opt,
    unsigned long timeout_usec) { // opt = SO_SNDTIMEO/SO_RCVTIMEO
  if (timeout_usec > 0 && timeout_usec < minSocketTimeoutUS)
    timeout_usec =
        minSocketTimeoutUS; // Override timeout for high latency multiplayer
#if defined(_WIN32)
  unsigned long optval = timeout_usec / 1000UL;
  if (timeout_usec > 0 && optval == 0)
    optval = 1; // Since there are games that use 100 usec timeout, we should
                // set it to minimum value on Windows (1 msec) instead of using
                // 0 (0 = indefinitely timeout)
#elif defined(__APPLE__)
  struct timeval optval;
  optval.tv_sec = static_cast<long>(timeout_usec) / 1000000L;
  optval.tv_usec = static_cast<long>(timeout_usec) % 1000000L;
#else
  struct timeval optval = {static_cast<long>(timeout_usec) / 1000000L,
                           static_cast<long>(timeout_usec) % 1000000L};
#endif
...
int getSockError(int sock) {
  ...
  if (getsockopt(sock, SOL_SOCKET, SO_ERROR, (char *)&result, &result_len) <
      0) {
    result = socket_errno;
  }
  return result;
...
#define SIO_TCP_SET_ACK_FREQUENCY _WSAIOW(IOC_VENDOR, 23)
  ...
  DWORD retbytes = 0;
  WSAIoctl(tcpsock, SIO_TCP_SET_ACK_FREQUENCY, &freq, sizeof(freq), NULL, 0,
           &retbytes, NULL, NULL);
#endif
  // Disable Nagle Algo
  return setsockopt(tcpsock, IPPROTO_TCP, TCP_NODELAY, (char *)&opt,
                    sizeof(opt));
}
...
int setSockNoSIGPIPE(int sock, int flag) {
  ...
  // Note: Linux might have SO_NOSIGPIPE defined too, but using it on setsockopt
  // will result to EINVAL error
...
#endif
  return setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, (const char *)&opt,
                    sizeof(opt));
}

int setUDPConnReset(int udpsock, bool enabled) {
  // On Windows: Connection Reset error on UDP could cause a strange behavior
...
#define SIO_UDP_CONNRESET _WSAIOW(IOC_VENDOR, 12)
  ...
  return WSAIoctl(udpsock, SIO_UDP_CONNRESET, &bNewBehavior,
...
#define TCP_KEEPIDLE                                                           \
  TCP_KEEPALIVE // TCP_KEEPIDLE on Linux is equivalent to TCP_KEEPALIVE on macOS
#endif
...
#endif
#endif
int setSockKeepAlive(int sock, bool keepalive, const int keepinvl,
                     const int keepcnt, const int keepidle) {
...
int getNicknameCount(const char *nickname) {
  ...
  for (; peer != NULL; peer = peer->next) {
    // Match found
    if (peer->last_recv != 0 && strncmp((char *)&peer->nickname.data, nickname,
                                        ADHOCCTL_NICKNAME_LEN) == 0)
      count++;
  }
...
int initNetwork(SceNetAdhocctlAdhocId *adhoc_id) {
  ...
  // If Server is at localhost Try to Bind socket to specific adapter before
  // connecting to prevent 2nd instance being recognized as already existing
  // 127.0.0.1 by AdhocServer (may not works in WinXP/2003 for IPv4 due to "Weak
  // End System" model)
  if (isLoopbackIP(g_adhocServerIP.in.sin_addr.s_addr)) {
    int on = 1;
    // Not sure what is this SO_DONTROUTE supposed to fix, but i do remembered
    // there were issue related to multiple-instances without SO_DONTROUTE, but
    // forgot how to reproduce it :(
    setsockopt((int)metasocket, SOL_SOCKET, SO_DONTROUTE, (const char *)&on,
               sizeof(on));
    ...
    iResult =
        bind((int)metasocket, &g_localhostIP.addr, sizeof(g_localhostIP.addr));
    if (iResult == SOCKET_ERROR) {
      ERROR_LOG(Log::sceNet, "Bind to alternate localhost[%s] failed(%i).",
                ip2str(g_localhostIP.in.sin_addr).c_str(), iResult);
      g_OSD.Show(OSDType::MESSAGE_ERROR,
                 std::string(n->T("Failed to Bind Localhost IP")) + " " +
                     ip2str(g_localhostIP.in.sin_addr).c_str());
    }
  }

  // Default/Initial Network Parameters
  memset(&parameter, 0, sizeof(parameter));
  strncpy((char *)&parameter.nickname.data, g_Config.sNickName.c_str(),
          ADHOCCTL_NICKNAME_LEN);
  parameter.nickname.data[ADHOCCTL_NICKNAME_LEN - 1] = 0;
  parameter.channel = g_Config.iWlanAdhocChannel;
  // Assign a Valid Channel when connected to AP/Adhoc if it's Auto. JPCSP use
  // 11 as default for Auto (Commonly for Auto: 1, 6, 11)
  if (parameter.channel == PSP_SYSTEMPARAM_ADHOC_CHANNEL_AUTOMATIC)
    parameter.channel =
        defaultWlanChannel; // Faked Active channel to default channel
  ...
  // Don't need to connect if AdhocServer IP is the same with this instance
  // localhost IP and having AdhocServer disabled
  if (g_adhocServerIP.in.sin_addr.s_addr == g_localhostIP.in.sin_addr.s_addr &&
      !g_Config.bEnableAdhocServer)
    return SOCKET_ERROR;
  ...
  iResult =
      connect((int)metasocket, &g_adhocServerIP.addr, sizeof(g_adhocServerIP));
  ...
  if (iResult == SOCKET_ERROR && errorcode != EISCONN) {
    ...
    while (!done) {
      ...
      memset(&sin, 0, sinlen);
      // Ensure that the connection really established or not, since "select"
      // alone can't accurately detects it
      done &= (getpeername((int)metasocket, (struct sockaddr *)&sin, &sinlen) !=
               SOCKET_ERROR);
    ...
    if (!done) {
      ERROR_LOG(Log::sceNet,
                "Socket error (%i) when connecting to AdhocServer [%s/%s:%u]",
                errorcode, g_Config.sProAdhocServer.c_str(),
                ip2str(g_adhocServerIP.in.sin_addr).c_str(),
                ntohs(g_adhocServerIP.in.sin_port));
      g_OSD.Show(OSDType::MESSAGE_ERROR,
                 std::string(n->T("Failed to connect to Adhoc Server")) + " (" +
                     std::string(n->T("Error")) + ": " +
  ...
  getLocalMac(&addres);
  packet.mac = addres;
  strncpy((char *)&packet.name.data, g_Config.sNickName.c_str(),
          ADHOCCTL_NICKNAME_LEN);
  packet.name.data[ADHOCCTL_NICKNAME_LEN - 1] = 0;
  memcpy(packet.game.data, adhoc_id->data, ADHOCCTL_ADHOCID_LEN);

  IsSocketReady((int)metasocket, false, true, nullptr, adhocDefaultTimeout);
  DEBUG_LOG(Log::sceNet, "InitNetwork: Sending LOGIN OPCODE %d",
            packet.base.opcode);
  int sent =
      (int)send((int)metasocket, (char *)&packet, sizeof(packet), MSG_NOSIGNAL);
  if (sent > 0) {
...
u64 join32(u32 num1, u32 num2) { return (u64)num2 << 32 | num1; }
...
const char *AdhocCtlStateToString(int state) {
  ...
  case ADHOCCTL_STATE_GAMEMODE:
    return "GAMEMODE";
  case ADHOCCTL_STATE_DISCOVER:
    return "DISCOVER";
  case ADHOCCTL_STATE_WOL:
    return "WOL";
  default:
    return "(unk)";
  }
