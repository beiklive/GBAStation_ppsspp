...
static const int MPEG_DATA_STREAM =
    3; // Arbitrary user defined type. Can represent audio or video.
...
static const int mpegTimestampPerSecond =
    90000; // How many MPEG Timestamp units in a second.
static const int videoTimestampStep =
    3003; // Value based on pmfplayer (mpegTimestampPerSecond / 29.970 (fps)).
static const int audioTimestampStep =
    4180; // For audio play at 44100 Hz (2048 samples / 44100 *
          // mpegTimestampPerSecond == 4180)
static const int audioFirstTimestamp =
    90000; // The first MPEG audio AU has always this timestamp
...
static u32 pmp_videoSource =
    0; // pointer to the video source (SceMpegLLi structure)
static int pmp_nBlocks =
    0; // number of blocks received in the last sceMpegBasePESpacketCopy call
#ifdef USE_FFMPEG
static std::list<AVFrame *>
    pmp_queue; // list of pmp video frames have been decoded and will be played
#endif
static std::list<u32> pmp_ContextList;  // list of pmp media contexts
static bool pmp_oldStateLoaded = false; // for dostate

// Calculate the number of total packets added to the ringbuffer by calling the
// sceMpegRingbufferPut() once.
...
static int getMaxAheadTimestamp(const SceMpegRingBuffer &ringbuf) {
        return std::max(maxAheadTimestamp, 700 * ringbuf.packets);  // empiric
value from JPCSP, thanks!
}
*/

const u8 defaultMpegheader[2048] = {
    ,
...
MpegContext::MpegContext() { memcpy(mpegheader, defaultMpegheader, 2048); }
MpegContext::~MpegContext() { delete mediaengine; }
...
static MpegContext *getMpegCtx(u32 mpegAddr)
{
  ...
  {
    auto ringbuffer =
        PSPPointer<SceMpegRingBuffer>::Create(res->mpegRingbufferAddr);
...
const std::map<u32, MpegContext *> &__MpegGetContexts() { return g_mpegCtxs; }
...
static void AnalyzeMpeg(const u8 *buffer, u32 validSize, MpegContext *ctx)
{
  ...
  ctx->mpegFirstTimestamp =
      getMpegTimeStamp(buffer + PSMF_FIRST_TIMESTAMP_OFFSET);
  ...
  {
    WARN_LOG_REPORT(Log::Mpeg, "Unexpected mpeg first timestamp: %llx / %lld",
                    ctx->mpegFirstTimestamp, ctx->mpegFirstTimestamp);
  }
  ...
  if (!ctx->isAnalyzed && ctx->mediaengine && ctx->mpegStreamSize > 0 &&
      validSize >= ctx->mpegOffset)
  {
    // init mediaEngine
    auto ringbuffer =
        PSPPointer<SceMpegRingBuffer>::Create(ctx->mpegRingbufferAddr);
    if (ringbuffer.IsValid())
    {
      ctx->mediaengine->loadStream(buffer, ctx->mpegOffset,
                                   ringbuffer->packets *
    ...
    // When used with scePsmf, some applications attempt to use
    // sceMpegQueryStreamOffset and sceMpegQueryStreamSize, which forces a
    // packet overwrite in the Media Engine and in the MPEG ringbuffer. Mark the
    // current MPEG as analyzed to filter this, and restore it at sceMpegFinish.
    ctx->isAnalyzed = true;
  }
  ...
  *(u32_le *)(ctx->mpegheader + PSMF_STREAM_OFFSET_OFFSET) = 0x80000;
  ...
  INFO_LOG(Log::Mpeg, "Stream offset: %d, Stream size: 0x%X", ctx->mpegOffset,
           ctx->mpegStreamSize);
  INFO_LOG(Log::Mpeg, "First timestamp: %lld, Last timestamp: %lld",
           ctx->mpegFirstTimestamp, ctx->mpegLastTimestamp);
}
...
static u32 sceMpegInit()
{
  ...
  {
    INFO_LOG(Log::Mpeg, "sceMpegInit(), mpegLibVersion 0x%0x, mpegLibcrc %x",
             mpegLibVersion, mpegLibCrc);
  }
  isMpegInit = true;
...
static u32 sceMpegRingbufferConstruct(u32 ringbufferAddr, u32 numPackets,
                                      u32 data, u32 size, u32 callbackAddr,
                                      u32 callbackArg)
{
  if (!Memory::IsValidAddress(ringbufferAddr))
  {
    ERROR_LOG_REPORT(Log::Mpeg,
                     "sceMpegRingbufferConstruct(%08x, %i, %08x, %08x, %08x, "
                     "%08x): bad ringbuffer, should crash",
                     ringbufferAddr, numPackets, data, size, callbackAddr,
                     callbackArg);
  ...
  if (__MpegRingbufferQueryMemSize(numPackets) > size)
  {
    if (numPackets < 0x00100000)
    {
      ERROR_LOG_REPORT(Log::Mpeg,
                       "sceMpegRingbufferConstruct(%08x, %i, %08x, %08x, %08x, "
                       "%08x): too many packets for buffer",
                       ringbufferAddr, numPackets, data, size, callbackAddr,
                       callbackArg);
      return hleNoLog(SCE_MPEG_ERROR_NO_MEMORY);
    }
    ...
    {
      // The PSP's firmware allows some cases here, due to a bug in its
      // validation.
      ERROR_LOG_REPORT(Log::Mpeg,
                       "sceMpegRingbufferConstruct(%08x, %i, %08x, %08x, %08x, "
                       "%08x): too many packets for buffer, bogus size",
                       ringbufferAddr, numPackets, data, size, callbackAddr,
                       callbackArg);
    }
  }
...
static u32 sceMpegCreate(u32 mpegAddr, u32 dataPtr, u32 size,
                         u32 ringbufferAddr, u32 frameWidth, u32 mode,
                         u32 ddrTop)
{
  if (!Memory::IsValidAddress(mpegAddr))
  {
  ...
  if (ringbuffer.IsValid())
  {
    ...
    {
      ringbuffer->packetsAvail =
          ringbuffer->packets -
          (ringbuffer->dataUpperBound - ringbuffer->data) /
              ringbuffer->packetSize;
    }
  ...
  {
    WARN_LOG_REPORT(Log::HLE, "Replacing existing mpeg context at %08x",
                    mpegAddr);
  ...
  ctx->audioFrameCount = 0;
  ctx->videoPixelMode =
      GE_CMODE_32BIT_ABGR8888; // TODO: What's the actual default?
...
static int sceMpegAvcDecodeMode(u32 mpeg, u32 modeAddr)
{
  ...
  if (pixelMode >= GE_CMODE_16BIT_BGR5650 &&
      pixelMode <= GE_CMODE_32BIT_ABGR8888)
  {
    ctx->videoPixelMode = pixelMode;
  }
  else
  {
    ERROR_LOG(Log::Mpeg, "sceMpegAvcDecodeMode(%i, %i): unknown pixelMode ",
              mode, pixelMode);
  }
  return hleNoLog(0);
}
...
static int sceMpegQueryStreamOffset(u32 mpeg, u32 bufferAddr, u32 offsetAddr)
{
  if (!Memory::IsValidAddress(bufferAddr) ||
      !Memory::IsValidAddress(offsetAddr))
  {
    return hleLogWarning(Log::Mpeg, -1, "invalid addresses");
  }
  ...
  // Kinda destructive, no? Shouldn't this just do what sceMpegQueryStreamSize
  // does?
  AnalyzeMpeg(Memory::GetPointerWriteUnchecked(bufferAddr),
              Memory::ClampValidSizeAt(bufferAddr, 32768), ctx);
...
{
  if (!Memory::IsValidAddress(bufferAddr) ||
      !Memory::IsValidAddress(sizeAddr))
  {
    return hleLogWarning(Log::Mpeg, -1, "invalid addresses");
  }
...
static int sceMpegRegistStream(u32 mpeg, u32 streamType, u32 streamNum)
{
  ...
  case MPEG_DATA_STREAM:
    ...
    break;
  default:
    DEBUG_LOG(Log::Mpeg, "sceMpegRegistStream(%i) : unknown stream type",
              streamType);
    break;
  }
  // ...
...
static bool InitPmp(MpegContext *ctx)
{
  ...
  // set pmp video size. Better to get from pmp file directly if possible. Any
  // idea?
  ...
  mediaengine->m_bufSize = av_image_get_buffer_size(
      pmp_want_pix_fmt, pmp_CodecCtx->width, pmp_CodecCtx->height, 1);
#else
  mediaengine->m_bufSize = avpicture_get_size(
      pmp_want_pix_fmt, pmp_CodecCtx->width, pmp_CodecCtx->height);
#endif
...
// This class H264Frames is used for collecting small pieces of frames into
// larger frames for ffmpeg to decode Basically, this will avoid incomplete
// frame decoding issue and improve much better the video quality.
class H264Frames
{
public:
  int size;
  ...
  void add(const H264Frames *p) { add(p->stream, p->size); };
...
static bool decodePmpVideo(PSPPointer<SceMpegRingBuffer> ringbuffer,
                           u32 pmpctxAddr)
{
  ...
  if (Memory::IsValidAddress(pmp_videoSource))
  {
    ...
    av_image_fill_arrays(pFrameRGB->data, pFrameRGB->linesize,
                         mediaengine->m_buffer, pmp_want_pix_fmt,
                         pCodecCtx->width, pCodecCtx->height, 1);
#else
    avpicture_fill((AVPicture *)pFrameRGB, mediaengine->m_buffer,
                   pmp_want_pix_fmt, pCodecCtx->width, pCodecCtx->height);
#endif
    ...
    if (got_picture)
    {
      ...
      img_convert_ctx = sws_getContext(pCodecCtx->width, pCodecCtx->height,
                                       ...
                                       pCodecCtx->height, pmp_want_pix_fmt,
      ...
      if (bestPts != AV_NOPTS_VALUE)
        mediaengine->m_videopts =
            bestPts + ptsDuration - mediaengine->m_firstTimeStamp;
      else
...
static u32 sceMpegAvcDecode(u32 mpeg, u32 auAddr, u32 frameWidth,
                            u32 bufferAddr, u32 initAddr)
{
  MpegContext *ctx = getMpegCtx(mpeg);
  if (!ctx)
  {
  ...
  if (frameWidth == 0)
  { // wtf, go sudoku passes in 0xcccccccc
  ...
  DEBUG_LOG(Log::Mpeg, "video: bufferAddr = %08x, *buffer = %08x, *init = %08x",
            bufferAddr, buffer, init);
  ...
  {
    return hleDelayResult(hleLogWarning(Log::Mpeg,
                                        SCE_MPEG_ERROR_AVC_DECODE_FATAL,
                                        "mpeg buffer empty"),
                          "mpeg buffer empty", avcEmptyDelayMs);
  }

  s32 beforeAvail =
      ringbuffer->packets - ctx->mediaengine->getRemainSize() / 2048;
  ...
  if (ispmp)
  {
    ...
    while (pmp_queue.size() != 0)
    {
      ...
      int bufferSize = ctx->mediaengine->writeVideoImage(buffer, frameWidth,
                                                         ctx->videoPixelMode);
      gpu->PerformWriteFormattedFromMemory(buffer, bufferSize, frameWidth,
                                           (GEBufferFormat)ctx->videoPixelMode);
  ...
  {
    return hleDelayResult(hleLogDebug(Log::Mpeg, 0), "mpeg decode",
                          accumDelay + avcFirstDelayMs);
  }
  else
  {
    return hleDelayResult(hleLogDebug(Log::Mpeg, 0), "mpeg decode",
                          accumDelay + avcDecodeDelayMs);
  }
  // hleEatMicro(3300);
  // return hleDelayResult(0, "mpeg decode", 200);
}

static u32 sceMpegAvcDecodeStop(u32 mpeg, u32 frameWidth, u32 bufferAddr,
                                u32 statusAddr)
{
  if (!Memory::IsValidAddress(bufferAddr) ||
      !Memory::IsValidAddress(statusAddr))
  {
...
static u32 sceMpegUnRegistStream(u32 mpeg, int streamUid)
{
  ...
  case MPEG_DATA_STREAM:
    ...
    break;
  default:
    DEBUG_LOG(Log::Mpeg, "sceMpegUnRegistStream(%i) : unknown streamID ",
              streamUid);
    break;
  }
...
static u32 sceMpegAvcDecodeStopYCbCr(u32 mpeg, u32 bufferAddr, u32 statusAddr)
{
  ...
  ERROR_LOG(Log::Mpeg, "UNIMPL sceMpegAvcDecodeStopYCbCr(%08x, %08x, %08x)",
            mpeg, bufferAddr, statusAddr);
  Memory::Write_U32(0, statusAddr);
  return hleNoLog(0);
}

static int sceMpegAvcDecodeYCbCr(u32 mpeg, u32 auAddr, u32 bufferAddr,
                                 u32 initAddr)
{
  MpegContext *ctx = getMpegCtx(mpeg);
  if (!ctx)
...
static int sceMpegInitAu(u32 mpeg, u32 bufferAddr, u32 auPointer)
{
  ...
  if (bufferAddr >= 1 && bufferAddr <= (u32)MPEG_DATA_ES_BUFFERS &&
      ctx->esBuffers[bufferAddr - 1])
  {
    // This esbuffer has been allocated for Avc.
    // Default to 0, since we stuff the stream id in here.  Technically, we
    // shouldn't.
    // TODO: Do something better to track the AU data.  This used to be
    // bufferAddr.
    sceAu.esBuffer = 0;
...
{
  if (!Memory::IsValidAddress(esSizeAddr) ||
      !Memory::IsValidAddress(outSizeAddr))
  {
    return hleLogError(Log::Mpeg, -1, "invalid addresses");
  }
...
static int sceMpegRingbufferAvailableSize(u32 ringbufferAddr)
{
  ...
  if (!ringbuffer.IsValid())
  {
    return hleLogError(Log::Mpeg, SCE_KERNEL_ERROR_ILLEGAL_ADDRESS,
                       "invalid ringbuffer, should crash");
  }

  MpegContext *ctx = getMpegCtx(ringbuffer->mpeg);
  if (!ctx)
  {
    return hleLogError(Log::Mpeg, SCE_MPEG_ERROR_NOT_YET_INIT,
                       "bad mpeg handle");
  }
  ...
  {
    DEBUG_LOG(Log::Mpeg, "%i=sceMpegRingbufferAvailableSize(%08x)",
              ringbuffer->packets - ringbuffer->packetsAvail, ringbufferAddr);
    lastAvail = ringbuffer->packetsAvail;
  }
  else
  {
    VERBOSE_LOG(Log::Mpeg, "%i=sceMpegRingbufferAvailableSize(%08x)",
                ringbuffer->packets - ringbuffer->packetsAvail, ringbufferAddr);
  }
...
void PostPutAction::run(MipsCall &call)
{
  ...
  if (mpegLibVersion < 0x0105 && packetsAddedThisRound > 0)
  {
    ...
    uint32_t bufSize = Memory::ClampValidSizeAt(
        ringbuffer->data + readOffset * 2048, packetsAddedThisRound * 2048);
  ...
  if (ringbuffer->packetsRead == 0 && ctx->mediaengine &&
      packetsAddedThisRound > 0)
  {
    // init mediaEngine
    AnalyzeMpeg(ctx->mpegheader, 2048, ctx);
    ctx->mediaengine->loadStream(ctx->mpegheader, 2048,
                                 ringbuffer->packets * ringbuffer->packetSize);
  }
  if (packetsAddedThisRound > 0)
  {
    if (packetsAddedThisRound >
        ringbuffer->packets - ringbuffer->packetsAvail)
    {
      WARN_LOG(Log::Mpeg,
               "sceMpegRingbufferPut clamping packetsAdded old=%i new=%i",
    ...
    uint32_t dataSize = Memory::ClampValidSizeAt(
        ringbuffer->data + writeOffset * 2048, packetsAddedThisRound * 2048);
    int actuallyAdded =
        ctx->mediaengine == NULL
            ? 8
            : ctx->mediaengine->addStreamData(data, dataSize) / 2048;
    if (actuallyAdded != packetsAddedThisRound)
    {
      WARN_LOG_REPORT(Log::Mpeg,
                      "sceMpegRingbufferPut(): unable to enqueue all added "
                      "packets, going to overwrite some frames.");
    }
    ringbuffer->packetsRead += packetsAddedThisRound;
    ringbuffer->packetsWritePos += packetsAddedThisRound;
    ringbuffer->packetsAvail += packetsAddedThisRound;
  }
  DEBUG_LOG(Log::Mpeg, "packetAdded: %i packetsRead: %i packetsTotal: %i",
            packetsAddedThisRound, ringbuffer->packetsRead,
            ringbuffer->packets);
...
// Program signals that it has written data to the ringbuffer and gets a
// callback ?
static u32 sceMpegRingbufferPut(u32 ringbufferAddr, int numPackets,
                                int available)
{
  auto ringbuffer = PSPPointer<SceMpegRingBuffer>::Create(ringbufferAddr);
  if (!ringbuffer.IsValid())
  ...
  // Generally, program will call sceMpegRingbufferAvailableSize() before this
  // func. Seems still need to check actual available, Patapon 3 for example.
  numPackets =
      std::min(numPackets, ringbuffer->packets - ringbuffer->packetsAvail);
  if (numPackets <= 0)
  {
  ...
  if (!ctx)
  {
    return hleLogWarning(Log::Mpeg, -1, "bad mpeg handle %08x",
                         ringbuffer->mpeg);
  }
  ringbufferPutPacketsAdded = 0;
  // Execute callback function as a direct MipsCall, no blocking here so no
  // messing around with wait states etc
  if (ringbuffer->callback_addr != 0)
  {
    DEBUG_LOG(Log::Mpeg, "sceMpegRingbufferPut(%08x, %i, %i)", ringbufferAddr,
              numPackets, available);

    // Call this multiple times until we get numPackets.
    // Normally this would be if it did not read enough, but also if available >
    // packets. Should ultimately return the TOTAL number of returned packets.
    int writeOffset = ringbuffer->packetsWritePos % (s32)ringbuffer->packets;
    u32 packetsThisRound = 0;
    while (numPackets)
    {
      PostPutAction *action =
          (PostPutAction *)__KernelCreateAction(actionPostPut);
      action->setRingAddr(ringbufferAddr);

      packetsThisRound =
          std::min(numPackets, (s32)ringbuffer->packets - writeOffset);
      numPackets -= packetsThisRound;
      u32 args[3] = {(u32)ringbuffer->data + (u32)writeOffset * 2048,
                     packetsThisRound, (u32)ringbuffer->callback_args};
...
static int sceMpegGetAvcAu(u32 mpeg, u32 streamId, u32 auAddr, u32 attrAddr)
{
  ...
  if (PSP_CoreParameter().compat.flags().MpegAvcWarmUp)
  {
    ...
    {
      ctx->mpegwarmUp++;
      return hleLogDebug(Log::Mpeg, SCE_MPEG_ERROR_NO_DATA, "warming up (%d)",
                         ctx->mpegwarmUp);
    }
  }
  ...
  if (ringbuffer->packetsRead == 0 || ringbuffer->packetsAvail == 0)
  {
    ...
    return hleDelayResult(hleLogDebug(Log::Mpeg, SCE_MPEG_ERROR_NO_DATA),
  ...
  // esBuffer is the memory where this au data goes.  We don't write the data to
  // memory. Instead, let's abuse it to keep track of the stream number.
  avcAu.esBuffer = streamInfo->second.num;

  /*// Wait for audio if too much ahead
  if (ctx->atracRegistered && (ctx->mediaengine->getVideoTimeStamp() >
  ctx->mediaengine->getAudioTimeStamp() + getMaxAheadTimestamp(mpegRingbuffer)))
  {
          ERROR_LOG(Log::Mpeg, "sceMpegGetAvcAu - video too much ahead");
          // TODO: Does this really reschedule?
          return hleDelayResult(SCE_MPEG_ERROR_NO_DATA, "mpeg get avc",
  mpegDecodeErrorDelayMs);
  }*/
  ...
  {
    INFO_LOG(Log::Mpeg, "video end reach. pts: %i dts: %i", (int)avcAu.pts,
             (int)ctx->mediaengine->getLastTimeStamp());
    ringbuffer->packetsAvail = 0;
  ...
  // Jeanne d'Arc return 00000000 as attrAddr here and cause WriteToHardware
  // error
  ...
  // TODO: sceMpegGetAvcAu seems to modify esSize, and delay when it's > 1000 or
  // something. There's definitely more to it, but ultimately it seems games
  // should expect it to delay randomly.
...
static int sceMpegGetAtracAu(u32 mpeg, u32 streamId, u32 auAddr, u32 attrAddr)
{
  ...
  if (streamInfo == ctx->streamMap.end())
  {
    WARN_LOG_REPORT(Log::Mpeg, "sceMpegGetAtracAu: invalid audio stream %08x",
                    streamId);
  ...
  if (ringbuffer->packetsAvail == 0)
  {
    ...
    return hleDelayResult(hleLogDebug(Log::Mpeg, SCE_MPEG_ERROR_NO_DATA),
                          "mpeg get atrac", mpegDecodeErrorDelayMs);
  }
  ...
  if (ctx->mediaengine->IsVideoEnd())
  {
    INFO_LOG(Log::Mpeg, "video end reach. pts: %i dts: %i", (int)atracAu.pts,
             (int)ctx->mediaengine->getLastTimeStamp());
    ringbuffer->packetsAvail = 0;
  ...
  if (ctx->atracRegistered && ctx->mediaengine->IsNoAudioData() &&
      !ctx->endOfAudioReached)
  {
    WARN_LOG(Log::Mpeg, "Audio end reach. pts: %i dts: %i", (int)atracAu.pts,
             (int)ctx->mediaengine->getLastTimeStamp());
  ...
  // 3rd birthday return 00000000 as attrAddr here and cause WriteToHardware
  // error
  if (Memory::IsValidAddress(attrAddr))
  {
...
static u32 sceMpegChangeGetAuMode(u32 mpeg, int streamUid, int mode)
{
  ...
  {
    return hleLogError(Log::Mpeg, SCE_MPEG_ERROR_INVALID_VALUE,
                       "UNIMPL / unknown streamID");
  }
  else
  {
    StreamInfo &info = stream->second;
    DEBUG_LOG(Log::Mpeg,
              "UNIMPL sceMpegChangeGetAuMode(%08x, %i, %i): changing type=%d",
              mpeg, streamUid, mode, info.type);
    switch (info.type)
    {
    case MPEG_AVC_STREAM:
    ...
    default:
      ERROR_LOG(Log::Mpeg,
                "UNIMPL sceMpegChangeGetAuMode(%08x, %i, %i): unknown streamID",
                mpeg, streamUid, mode);
      break;
    }
  }
...
static u32 sceMpegChangeGetAvcAuMode(u32 mpeg, u32 stream_addr, int mode)
{
  ...
  ERROR_LOG_REPORT_ONCE(mpegChangeAvcAu, Log::Mpeg,
                        "UNIMPL sceMpegChangeGetAvcAuMode(%08x, %08x, %i)",
                        mpeg, stream_addr, mode);
  return 0;
}
...
static u32 sceMpegGetPcmAu(u32 mpeg, int streamUid, u32 auAddr, u32 attrAddr)
{
  ...
  {
    return hleLogWarning(Log::Mpeg, SCE_MPEG_ERROR_INVALID_ADDR,
                         "didn't get a fake stream, bad streamUid address");
  }
  ...
  ERROR_LOG_REPORT_ONCE(mpegPcmAu, Log::Mpeg,
                        "UNIMPL sceMpegGetPcmAu(%08x, %i, %08x, %08x)", mpeg,
                        streamUid, auAddr, attrAddr);
  return hleNoLog(0);
}
...
{
  if (!Memory::IsValidAddress(sourceAddr) ||
      !Memory::IsValidAddress(YCbCrAddr))
  {
...
static u32 sceMpegAtracDecode(u32 mpeg, u32 auAddr, u32 bufferAddr, int init)
{
  ...
  return hleDelayResult(hleLogDebug(Log::Mpeg, 0), "mpeg atrac decode",
                        atracDecodeDelayMs);
  // hleEatMicro(4000);
...
static u32 sceMpegAvcCsc(u32 mpeg, u32 sourceAddr, u32 rangeAddr,
                         int frameWidth, u32 destAddr)
{
  if (!Memory::IsValidAddress(sourceAddr) ||
      !Memory::IsValidAddress(rangeAddr) || !Memory::IsValidAddress(destAddr))
  {
    return hleLogError(Log::Mpeg, -1, "invalid addresses");
  }
  ...
  {
    WARN_LOG(Log::Mpeg,
             "sceMpegAvcCsc(%08x, %08x, %08x, %i, %08x) returning "
             "ERROR_INVALID_VALUE",
             mpeg, sourceAddr, rangeAddr, frameWidth, destAddr);
    return SCE_KERNEL_ERROR_INVALID_VALUE;
  }

  int destSize = ctx->mediaengine->writeVideoImageWithRange(
      destAddr, frameWidth, ctx->videoPixelMode, x, y, width, height);
  gpu->PerformWriteFormattedFromMemory(destAddr, destSize, frameWidth,
                                       (GEBufferFormat)ctx->videoPixelMode);
  ...
  // If do not use DelayResult,Wil cause flickering in Dengeki no Pilot: Tenkuu
  // no Kizuna https://github.com/hrydgard/ppsspp/issues/7549

  return hleDelayResult(hleLogDebug(Log::Mpeg, 0), "mpeg avc csc",
                        avcCscDelayMs);
}
...
static u32 sceMpegAvcInitYCbCr(u32 mpeg, int mode, int width, int height,
                               u32 ycbcr_addr)
{
  ...
  WARN_LOG_ONCE(sceMpegAvcInitYCbCr, Log::Mpeg,
                "UNIMPL sceMpegAvcInitYCbCr(%08x, %i, %i, %i, %08x)", mpeg,
                mode, width, height, ycbcr_addr);
  return hleNoLog(0);
}

static int sceMpegAvcQueryYCbCrSize(u32 mpeg, u32 mode, u32 width, u32 height,
                                    u32 resultAddr)
{
  if ((width & 15) != 0 || (height & 15) != 0 || height > 272 || width > 480)
  {
    return hleLogError(Log::Mpeg, SCE_MPEG_ERROR_INVALID_VALUE,
                       "sceMpegAvcQueryYCbCrSize: bad w/h %i x %i", width,
                       height);
  }
...
static u32 sceMpegQueryUserdataEsSize(u32 mpeg, u32 esSizeAddr,
                                      u32 outSizeAddr)
{
  if (!Memory::IsValidAddress(esSizeAddr) ||
      !Memory::IsValidAddress(outSizeAddr))
  {
    ERROR_LOG(Log::Mpeg,
              "sceMpegQueryUserdataEsSize(%08x, %08x, %08x): invalid addresses",
              mpeg, esSizeAddr, outSizeAddr);
    return -1;
  }
  ...
  {
    WARN_LOG(Log::Mpeg,
             "sceMpegQueryUserdataEsSize(%08x, %08x, %08x): bad mpeg handle",
             mpeg, esSizeAddr, outSizeAddr);
    return -1;
  }
...
static u32 sceMpegAvcResourceInit(u32 mpeg)
{
  ...
  // In a real implementation, this would use sceKernelCreateFpl and
  // sceKernelAllocateFpl For PPSSPP, we'll simulate the allocation
...
{
  // see http://en.wikipedia.org/wiki/Yuv#Y.27UV444_to_RGB888_conversion for
  // more information.
  u8 r = (abgr >> 0) & 0xFF;
  u8 g = (abgr >> 8) & 0xFF;
  u8 b = (abgr >> 16) & 0xFF;
...
static int __MpegAvcConvertToYuv420(const void *data, u32 bufferOutputAddr,
                                    int width, int height)
{
  ...
  int sizeCb = sizeY >> 2;
  u8 *Y =
      Memory::GetPointerWriteRange(bufferOutputAddr, sizeY + sizeCb + sizeCb);
  u8 *Cb = Y + sizeY;
  u8 *Cr = Cb + sizeCb;

  if (!Y)
    return hleLogError(Log::Mpeg, 0,
                       "Bad output buffer pointer for yuv conv: %08x",
                       bufferOutputAddr);
...
static int sceMpegAvcConvertToYuv420(u32 mpeg, u32 bufferOutputAddr,
                                     u32 bufferAddr, int unknown2)
{
  ...
  if (ctx->mediaengine->m_buffer == 0)
    return hleLogWarning(Log::Mpeg, SCE_MPEG_ERROR_AVC_INVALID_VALUE,
                         "m_buffer is zero");
...
static int sceMpegGetUserdataAu(u32 mpeg, u32 streamUid, u32 auAddr,
                                u32 resultAddr)
{
  MpegContext *ctx = getMpegCtx(mpeg);
  if (!ctx)
  ...
  // We currently can't demux userdata so this seems like the best thing to
  // return in the meantime.. Then we probably shouldn't do the above writes?
  // but it works...
...
static u32 sceMpegNextAvcRpAu(u32 mpeg, u32 streamUid)
{
  ...
  ERROR_LOG_REPORT(Log::Mpeg, "UNIMPL sceMpegNextAvcRpAu(%08x, %08x)", mpeg,
                   streamUid);
  return 0;
}
...
const HLEFunction sceMpeg[] = {
    {0XE1CE83A7, &WrapI_UUUU<sceMpegGetAtracAu>, "sceMpegGetAtracAu", 'i',
     "xxxx"},
    ...
    {0XC132E22F, &WrapU_V<sceMpegQueryMemSize>, "sceMpegQueryMemSize", 'x', "",
     HLE_CLEAR_STACK_BYTES, 0x18},
    {0X21FF80E4, &WrapI_UUU<sceMpegQueryStreamOffset>,
     "sceMpegQueryStreamOffset", 'i', "xxx", HLE_CLEAR_STACK_BYTES, 0x18},
    {0X611E9E11, &WrapU_UU<sceMpegQueryStreamSize>, "sceMpegQueryStreamSize",
     'x', "xx", HLE_CLEAR_STACK_BYTES, 0x8},
    {0X42560F23, &WrapI_UUU<sceMpegRegistStream>, "sceMpegRegistStream", 'i',
     "xxx", HLE_CLEAR_STACK_BYTES, 0x48},
    {0X591A4AA2, &WrapU_UI<sceMpegUnRegistStream>, "sceMpegUnRegistStream", 'x',
     "xi", HLE_CLEAR_STACK_BYTES, 0x18},
    {0X707B7629, &WrapU_U<sceMpegFlushAllStream>, "sceMpegFlushAllStream", 'x',
     "x"},
    {0X500F0429, &WrapU_UI<sceMpegFlushStream>, "sceMpegFlushStream", 'x',
     "xi"},
    {0XA780CF7E, &WrapI_U<sceMpegMallocAvcEsBuf>, "sceMpegMallocAvcEsBuf", 'i',
     "x"},
    {0XCEB870B1, &WrapI_UI<sceMpegFreeAvcEsBuf>, "sceMpegFreeAvcEsBuf", 'i',
     "xi"},
    {0X167AFD9E, &WrapI_UUU<sceMpegInitAu>, "sceMpegInitAu", 'i', "xxx"},
    {0X682A619B, &WrapU_V<sceMpegInit>, "sceMpegInit", 'x', "",
     HLE_CLEAR_STACK_BYTES, 0x48},
    {0X606A4649, &WrapI_U<sceMpegDelete>, "sceMpegDelete", 'i', "x",
     HLE_CLEAR_STACK_BYTES, 0x18},
    {0X874624D6, &WrapU_V<sceMpegFinish>, "sceMpegFinish", 'x', "",
     HLE_CLEAR_STACK_BYTES, 0x18},
    {0X800C44DF, &WrapU_UUUI<sceMpegAtracDecode>, "sceMpegAtracDecode", 'x',
     "xxxi"},
    {0X0E3C2E9D, &WrapU_UUUUU<sceMpegAvcDecode>, "sceMpegAvcDecode", 'x',
     "xxxxx"},
    {0X740FCCD1, &WrapU_UUUU<sceMpegAvcDecodeStop>, "sceMpegAvcDecodeStop", 'x',
     "xxxx"},
    {0X4571CC64, &WrapU_U<sceMpegAvcDecodeFlush>, "sceMpegAvcDecodeFlush", 'x',
     "x"},
    {0X0F6C18D7, &WrapI_UU<sceMpegAvcDecodeDetail>, "sceMpegAvcDecodeDetail",
     'i', "xx"},
    {0XA11C7026, &WrapI_UU<sceMpegAvcDecodeMode>, "sceMpegAvcDecodeMode", 'i',
     "xx"},
    {0X37295ED8, &WrapU_UUUUUU<sceMpegRingbufferConstruct>,
     "sceMpegRingbufferConstruct", 'x', "xxxxxx"},
    {0X13407F13, &WrapU_U<sceMpegRingbufferDestruct>,
     "sceMpegRingbufferDestruct", 'x', "x"},
    {0XB240A59E, &WrapU_UII<sceMpegRingbufferPut>, "sceMpegRingbufferPut", 'x',
     "xxx"},
    {0XB5F6DC87, &WrapI_U<sceMpegRingbufferAvailableSize>,
     "sceMpegRingbufferAvailableSize", 'i', "x"},
    {0XD7A29F46, &WrapU_I<sceMpegRingbufferQueryMemSize>,
     "sceMpegRingbufferQueryMemSize", 'x', "i"},
    {0X769BEBB6, &WrapI_U<sceMpegRingbufferQueryPackNum>,
     "sceMpegRingbufferQueryPackNum", 'i', "x"},
    {0X211A057C, &WrapI_UUUUU<sceMpegAvcQueryYCbCrSize>,
     "sceMpegAvcQueryYCbCrSize", 'i', "xxxxx"},
    {0XF0EB1125, &WrapI_UUUU<sceMpegAvcDecodeYCbCr>, "sceMpegAvcDecodeYCbCr",
     'i', "xxxx"},
    {0XF2930C9C, &WrapU_UUU<sceMpegAvcDecodeStopYCbCr>,
     "sceMpegAvcDecodeStopYCbCr", 'x', "xxx"},
    {0X67179B1B, &WrapU_UIIIU<sceMpegAvcInitYCbCr>, "sceMpegAvcInitYCbCr", 'x',
     "xiiix"},
    {0X0558B075, &WrapU_UUU<sceMpegAvcCopyYCbCr>, "sceMpegAvcCopyYCbCr", 'x',
     "xxx"},
    ...
    {0XC02CF6B5, &WrapI_UUU<sceMpegQueryPcmEsSize>, "sceMpegQueryPcmEsSize",
     'i', "xxx"},
    {0XC45C99CC, &WrapU_UUU<sceMpegQueryUserdataEsSize>,
     "sceMpegQueryUserdataEsSize", 'x', "xxx"},
    {0X234586AE, &WrapU_UUI<sceMpegChangeGetAvcAuMode>,
     "sceMpegChangeGetAvcAuMode", 'x', "xxi"},
    {0X63B9536A, &WrapU_U<sceMpegAvcResourceGetAvcDecTopAddr>,
     "sceMpegAvcResourceGetAvcDecTopAddr", 'x', "x"},
    {0X8160A2FE, &WrapU_U<sceMpegAvcResourceFinish>, "sceMpegAvcResourceFinish",
     'x', "x"},
    {0XAF26BB01, &WrapU_U<sceMpegAvcResourceGetAvcEsBuf>,
     "sceMpegAvcResourceGetAvcEsBuf", 'x', "x"},
    {0XFCBDB5AD, &WrapU_U<sceMpegAvcResourceInit>, "sceMpegAvcResourceInit",
     'x', "x"},
    {0XF5E7EA31, &WrapI_UUUI<sceMpegAvcConvertToYuv420>,
     "sceMpegAvcConvertToYuv420", 'x', "xxxi"},
    {0X01977054, &WrapI_UUUU<sceMpegGetUserdataAu>, "sceMpegGetUserdataAu", 'i',
     "xxxx"},
    {0X3C37A7A6, &WrapU_UU<sceMpegNextAvcRpAu>, "sceMpegNextAvcRpAu", 'x',
     "xx"},
    {0X11F95CF1, &WrapU_U<sceMpegGetAvcNalAu>, "sceMpegGetAvcNalAu", 'x', "x"},
    {0XAB0E9556, &WrapU_U<sceMpegAvcDecodeDetailIndex>,
     "sceMpegAvcDecodeDetailIndex", 'x', "x"},
...
static u32 sceMpegBasePESpacketCopy(u32 p)
{
  ...
  DEBUG_LOG(Log::Mpeg, "sceMpegBasePESpacketCopy(%08x), received %d block(s)",
            pmp_videoSource, pmp_nBlocks);
  return 0;
}

const HLEFunction sceMpegbase[] = {
    {0XBEA18F91, &WrapU_U<sceMpegBasePESpacketCopy>, "sceMpegBasePESpacketCopy",
     'x', "x"},
