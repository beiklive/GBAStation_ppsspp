...
void ffmpeg_logger(void *, int level, const char *format, va_list va_args) {
  ...
  if (level <= AV_LOG_PANIC) {
    ...
    if (startsWith(tmp, "No accelerated colorspace") ||
        startsWith(tmp, "SEI type 1 size 40 truncated at 36")) {
...
int MediaEngine::MpegReadbuffer(void *opaque, uint8_t *buf, int buf_size) {
  ...
  if (mpeg->m_mpegheaderReadPos < mpeg->m_mpegheaderSize) {
    size =
        std::min(buf_size, mpeg->m_mpegheaderSize - mpeg->m_mpegheaderReadPos);
...
bool MediaEngine::SetupStreams() {
  ...
  if (numStreams <= 0 || numStreams > 8) {
    ...
    WARN_LOG_REPORT(Log::ME,
                    "Could not setup streams, unexpected stream count: %d",
                    numStreams);
    return false;
  }
...
bool MediaEngine::openContext(bool keepReadPos) {
  ...
  m_pIOContext = avio_alloc_context(tempbuf, m_bufSize, 0, (void *)this,
                                    &MpegReadbuffer, nullptr, nullptr);
  m_pFormatCtx->pb = m_pIOContext;

  // Open video file
  AVDictionary *open_opt = nullptr;
  av_dict_set_int(&open_opt, "probesize", m_mpegheaderSize, 0);
  if (avformat_open_input((AVFormatContext **)&m_pFormatCtx, nullptr, nullptr,
                          &open_opt) != 0) {
  ...
  if (!SetupStreams() ||
      PSP_CoreParameter().compat.flags().UseFFMPEGFindStreamInfo) {
    // Fallback to old behavior.  Reads too much and corrupts when game doesn't
    // read fast enough. SetupStreams sometimes work for newer FFmpeg 3.1+ now,
    // but sometimes framerate is missing.
    WARN_LOG_REPORT_ONCE(setupStreams, Log::ME,
  ...
  if (PSP_CoreParameter().compat.flags().UseFFMPEGFindStreamInfo &&
      usedFFMPEGFindStreamInfo) {
    m_mpegheaderReadPos++;
...
bool MediaEngine::loadStream(const u8 *buffer, int readSize,
                             int RingbufferSize) {
  closeMedia();
...
bool MediaEngine::setVideoStream(int streamNum, bool force) {
  ...
  if (m_pFormatCtx && m_pCodecCtxs.find(streamNum) == m_pCodecCtxs.end()) {
    ...
    if (!pCodec) {
      WARN_LOG_REPORT(Log::ME, "Could not find decoder for %d",
                      (int)stream->codecpar->codec_id);
      return false;
    }
    AVCodecContext *m_pCodecCtx = avcodec_alloc_context3(pCodec);
    int paramResult =
        avcodec_parameters_to_context(m_pCodecCtx, stream->codecpar);
    if (paramResult < 0) {
      WARN_LOG_REPORT(Log::ME, "Failed to prepare context parameters: %08x",
                      paramResult);
      return false;
    }
#else
    ...
    m_pCodecCtx->flags |=
        AV_CODEC_FLAG_OUTPUT_CORRUPT | AV_CODEC_FLAG_LOW_DELAY;

    AVDictionary *opt = nullptr;
    // Allow ffmpeg to use any number of threads it wants.  Without this, it
    // doesn't use threads.
...
bool MediaEngine::setVideoDim(int width, int height) {
  ...
  int numBytes = av_image_get_buffer_size((AVPixelFormat)m_sws_fmt, m_desWidth,
                                          m_desHeight, 1);
#else
  int numBytes =
      avpicture_get_size((AVPixelFormat)m_sws_fmt, m_desWidth, m_desHeight);
#endif
  m_buffer = (u8 *)av_malloc(numBytes * sizeof(uint8_t));

  // Assign appropriate parts of buffer to image planes in m_pFrameRGB
#if LIBAVCODEC_VERSION_INT >= AV_VERSION_INT(57, 12, 100)
  av_image_fill_arrays(m_pFrameRGB->data, m_pFrameRGB->linesize, m_buffer,
                       (AVPixelFormat)m_sws_fmt, m_desWidth, m_desHeight, 1);
#else
  avpicture_fill((AVPicture *)m_pFrameRGB, m_buffer, (AVPixelFormat)m_sws_fmt,
                 m_desWidth, m_desHeight);
#endif
#endif // USE_FFMPEG
  return true;
}
...
void MediaEngine::updateSwsFormat(int videoPixelMode) {
  ...
  AVCodecContext *m_pCodecCtx =
      codecIter == m_pCodecCtxs.end() ? 0 : codecIter->second;
  ...
  if (swsDesired != m_sws_fmt && m_pCodecCtx != 0) {
    ...
    m_sws_ctx = sws_getCachedContext(
        m_sws_ctx, m_pCodecCtx->width, m_pCodecCtx->height,
        m_pCodecCtx->pix_fmt, m_desWidth, m_desHeight, (AVPixelFormat)m_sws_fmt,
    ...
    if (sws_getColorspaceDetails(m_sws_ctx, &inv_coefficients, &srcRange,
                                 &coefficients, &dstRange, &brightness,
                                 &contrast, &saturation) != -1) {
      srcRange = 0;
      dstRange = 0;
      sws_setColorspaceDetails(m_sws_ctx, inv_coefficients, srcRange,
                               coefficients, dstRange, brightness, contrast,
                               saturation);
    }
  }
#endif
...
bool MediaEngine::stepVideo(int videoPixelMode, bool skipFrame) {
  ...
  while (!bGetFrame) {
    ...
    // Even if we've read all frames, some may have been re-ordered frames at
    // the end. Still need to decode those, so keep calling
    // avcodec_decode_video2() / avcodec_receive_frame().
    if (dataEnd || packet.stream_index == m_videoStream) {
      // avcodec_decode_video2() / avcodec_send_packet() gives us the re-ordered
      // frames with a NULL packet.
#if LIBAVCODEC_VERSION_INT >= AV_VERSION_INT(57, 12, 100)
      if (dataEnd)
      ...
      if (frameFinished) {
        ...
        if (m_pFrameRGB && !skipFrame) {
          ...
          // TODO: Technically we could set this to frameWidth instead of
          // m_desWidth for better perf. Update the linesize for the new format
          // too.  We started with the largest size, so it should fit.
          m_pFrameRGB->linesize[0] =
              getPixelFormatBytes(videoPixelMode) * m_desWidth;

          sws_scale(m_sws_ctx, m_pFrame->data, m_pFrame->linesize, 0,
                    m_pCodecCtx->height, m_pFrameRGB->data,
...
#endif
        if (ptsDuration == 0) {
          if (m_lastPts == bestPts - m_firstTimeStamp ||
      ...
      if (result <= 0 && dataEnd) {
        // Sometimes, m_readSize is less than m_streamSize at the end, but not
        // by much. This is kinda a hack, but the ringbuffer would have to be
        // prematurely empty too.
...
// Some games depend on this, for example Sword Art Online (doesn't clear A's
// from buffer.)
inline void writeVideoLineRGBA(void *destp, const void *srcp, int width) {
  ...
  while (count >= 8) {
    __m128i pixels1 =
        _mm_and_si128(_mm_loadu_si128((const __m128i *)src), mask);
    __m128i pixels2 =
        _mm_and_si128(_mm_loadu_si128((const __m128i *)src + 1), mask);
...
int MediaEngine::writeVideoImage(u32 bufferPtr, int frameWidth,
                                 int videoPixelMode) {
  int videoLineSize = 0;
  ...
  if (!Memory::IsValidRange(bufferPtr, videoImageSize) || frameWidth > 2048) {
    ...
    ERROR_LOG_REPORT(Log::ME, "Ignoring invalid video decode address %08x/%x",
  ...
  bool swizzle = Memory::IsVRAMAddress(bufferPtr) &&
                 (bufferPtr & 0x00200000) == 0x00200000;
  if (swizzle) {
  ...
  default:
    ERROR_LOG_REPORT(Log::ME, "Unsupported video pixel format %d",
                     videoPixelMode);
    break;
  }
...
int MediaEngine::writeVideoImageWithRange(u32 bufferPtr, int frameWidth,
                                          int videoPixelMode, int xpos,
  ...
  NotifyMemInfo(MemBlockFlags::WRITE, bufferPtr, videoImageSize,
                "VideoDecodeRange");
...
int MediaEngine::getNextAudioFrame(u8 **buf, int *headerCode1,
                                   int *headerCode2) {
...
int MediaEngine::getAudioSamples(u32 bufferPtr) {
  ...
  if (buffer == nullptr) {
    ERROR_LOG_REPORT(
        Log::ME, "Ignoring bad audio decode address %08x during video playback",
        bufferPtr);
  }
  if (!m_demux) {
    return 0;
  ...
  if (m_audioContext != nullptr) {
    if (headerCode1 == 0x24) {
      // This means mono audio - tell the decoder to expect it before the first
      // frame. Note that it will always send us back stereo audio.
      m_audioContext->SetChannels(1);
    }

    int inbytesConsumed = 0;
    if (!m_audioContext->Decode(audioFrame, frameSize, &inbytesConsumed, 2,
                                buffer, &outSamples)) {
      ERROR_LOG(Log::ME, "Audio (%s) decode failed during video playback",
                GetCodecName(m_audioType));
    }
    int outBytes = outSamples * sizeof(int16_t) * 2;

    NotifyMemInfo(MemBlockFlags::WRITE, bufferPtr, outBytes,
                  "VideoDecodeAudio");
  }
...
bool MediaEngine::IsActuallyPlayingAudio() { return getAudioTimeStamp() >= 0; }

s64 MediaEngine::getVideoTimeStamp() { return m_videopts; }
