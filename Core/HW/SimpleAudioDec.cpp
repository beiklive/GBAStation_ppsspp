...
public:
  ...
  bool Decode(const uint8_t *inbuf, int inbytes, int *inbytesConsumed,
    ...
    // It's because minimp3 is a bit more sensitive than ffmpeg - if you give it
    // a buffer that's larger than the frame size, it'll check that there's a
    // second matching frame before accepting. But in our case we only get one
    // frame, but we do not know the size. So this might need some modifications
    // in minimp3.
    mp3dec_frame_info_t info{};
    int samplesWritten = mp3dec_decode_frame(&mp3_, inbuf, inbytes,
                                             (mp3d_sample_t *)temp_, &info);
...
// Based on
// http://ffmpeg.org/doxygen/trunk/doc_2examples_2decoding_encoding_8c-example.html#_a13
class FFmpegAudioDecoder : public AudioDecoder {
public:
  FFmpegAudioDecoder(PSPAudioType audioType, int sampleRateHz = 44100,
                     int channels = 2);
  ~FFmpegAudioDecoder();
...
AudioDecoder *CreateAudioDecoder(PSPAudioType audioType, int sampleRateHz,
  ...
  //   * Doesn't accept data where there's only one valid frame if the buffer is
  //   bigger.
  //     This prevents sceMp3LowLevelDecode from working, since nothing passes
  //     us the frame size.
  //
  // case PSP_CODEC_MP3:
...
FFmpegAudioDecoder::FFmpegAudioDecoder(PSPAudioType audioType, int sampleRateHz,
                                       int channels)
  ...
  if (!audioCodecId) {
    ERROR_LOG(Log::ME,
              "This version of FFMPEG does not support Audio codec type: %08x. "
  ...
  if (!codec_) {
    ...
    ERROR_LOG(Log::ME,
              "This version of FFMPEG does not support AV_CODEC_ctx for audio "
              "(%s). Update your submodule.",
              GetCodecName(audioType));
    return;
  }
  ...
  if (!codecCtx_) {
    ERROR_LOG(Log::ME,
              "Found a decoder for audio codec ID %08x but failed to allocate "
              "a codec context. Strange.",
              audioCodecId);
    return;
  }
...
bool FFmpegAudioDecoder::OpenCodec(int block_align) {
  ...
  if (!codec_ || !codecCtx_) {
    ERROR_LOG(Log::ME,
              "Codec context not allocated for some reason. This is bad.");
    return false;
  }
  // Some versions of FFmpeg require this set.  May be set in SetExtraData(),
  // but optional. When decoding, we decode by packet, so we know the size.
...
#else
  // Future versions may add other things to free, but avcodec_free_context
  // didn't exist yet here.
...
bool FFmpegAudioDecoder::Decode(const uint8_t *inbuf, int inbytes,
                                int *inbytesConsumed, int outputChannels,
                                int16_t *outbuf, int *outSamples) {
#ifdef USE_FFMPEG
  if (!codecOpen_) {
    OpenCodec(inbytes);
    if (!codecOpen_) {
  ...
  if (inbytes != 0) {
    ...
    if (err < 0) {
      ERROR_LOG(Log::ME,
                "Error sending audio frame to decoder (%d bytes): %d (%08x)",
                inbytes, err, err);
      return false;
    }
  }
  ...
  if (len < 0) {
    ERROR_LOG(Log::ME, "Error decoding Audio frame (%i bytes): %i (%08x)",
              inbytes, len, len);
    return false;
  }
  ...
  if (got_frame) {
    // Initializing the sample rate convert. We will use it to convert float
    // output into int.
    _dbg_assert_(outputChannels == 2);
#if LIBAVUTIL_VERSION_MAJOR >= 59
    AVChannelLayout wanted_channel_layout =
        AV_CHANNEL_LAYOUT_STEREO; // we want stereo output layout
    const AVChannelLayout &dec_channel_layout =
        frame_->ch_layout; // decoded channel layout
#else
    int64_t wanted_channel_layout =
        AV_CH_LAYOUT_STEREO; // we want stereo output layout
    int64_t dec_channel_layout =
        frame_->channel_layout; // decoded channel layout
#endif
...
#if LIBAVUTIL_VERSION_MAJOR >= 59
      swr_alloc_set_opts2(&swrCtx_, &wanted_channel_layout, AV_SAMPLE_FMT_S16,
                          outputSampleRate, &dec_channel_layout,
                          codecCtx_->sample_fmt, inputSampleRate, 0, NULL);
#else
      swrCtx_ = swr_alloc_set_opts(
          swrCtx_, wanted_channel_layout, AV_SAMPLE_FMT_S16, outputSampleRate,
          dec_channel_layout, codecCtx_->sample_fmt, inputSampleRate, 0, NULL);
#endif
    ...
    if (outbuf != nullptr) {
      swrRet =
          swr_convert(swrCtx_, (uint8_t **)&outbuf, frame_->nb_samples,
                      (const u8 **)frame_->extended_data, frame_->nb_samples);
    }
    if (swrRet < 0) {
    ...
    // Save outbuf into pcm audio, you can uncomment this line to save and check
    // the decoded audio into pcm file. SaveAudio("dump.pcm", outbuf,
    // *outbytes);
  }
  return true;
#else
...
u32 AuCtx::AuDecode(u32 pcmAddr) {
  ...
  if (!sourcebuff.empty()) {
    ...
    int outSamples = 0;
    decoder->Decode(&sourcebuff[nextSync], (int)sourcebuff.size() - nextSync,
                    &inbytesConsumed, 2, (int16_t *)outbuf, &outSamples);
    ...
    if (outpcmbufsize == 0) {
      ...
      // (the available buff size is now used to know if we can read again from
      // file and how many to read)
  ...
  if (end && LoopNum != 0) {
    // When looping, start the sum back off at zero and reset readPos to the
    // start.
    SumDecodedSamples = 0;
    readPos = startPos;
    if (LoopNum > 0)
...
int AuCtx::AuStreamBytesNeeded() {
  if (decoder->GetAudioType() == PSP_CODEC_MP3) {
    // The endPos and readPos are not considered, except when you've read to the
    // end.
    if (readPos >= endPos)
      return 0;
...
u32 AuCtx::AuNotifyAddStreamData(int size) {
  ...
  if (Memory::IsValidRange(AuBuf, size)) {
    ...
    Memory::MemcpyUnchecked(&sourcebuff[sourcebuff.size() - size],
                            AuBuf + offset, size);
  }
...
u32 AuCtx::AuResetPlayPositionByFrame(int frame) {
  // Note: this doesn't correctly handle padding or slot size, but the PSP
  // doesn't either.
  uint32_t bytesPerSecond = (MaxOutputSample / 8) * BitRate * 1000;
  readPos = startPos + (frame * bytesPerSecond) / SamplingRate;
  // Not sure why, but it seems to consistently seek 1 before, maybe in case
  // it's off slightly.
  if (frame != 0)
    readPos -= 1;
