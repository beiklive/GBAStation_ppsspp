...
static const u32 FILE_LIST_TOTAL_SIZE =
    sizeof(SaveSFOFileListEntry) * FILE_LIST_COUNT_MAX;
...
void SetStringFromSFO(ParamSFOData &sfoFile, const char *name, char *str,
                      int strLength) {
  truncate_cpy(str, strLength, sfoFile.GetValueString(name));
}

bool ReadPSPFile(const std::string &filename, u8 **data, s64 dataSize,
                 s64 *readSize) {
...
bool WritePSPFile(const std::string &filename, const u8 *data,
                  SceSize dataSize) {
  int handle = pspFileSystem.OpenFile(
      filename,
      (FileAccess)(FILEACCESS_WRITE | FILEACCESS_CREATE | FILEACCESS_TRUNCATE));
  if (handle < 0)
    return false;
...
PSPFileInfo FileFromListing(const std::vector<PSPFileInfo> &listing,
...
int align16(int address) { return (address + 15) & ~15; }
...
void SavedataParam::Init() {
  ...
  int handle = pspFileSystem.OpenFile(
      savePath + ".nomedia", (FileAccess)(FILEACCESS_CREATE | FILEACCESS_WRITE),
      0);
  if (handle >= 0) {
    pspFileSystem.CloseFile(handle);
  } else {
    INFO_LOG(
        Log::IO,
        "Failed to create .nomedia file (might be ok if it already exists)");
  }
#endif
}

std::string SavedataParam::GetSaveDirName(const SceUtilitySavedataParam *param,
                                          int saveId) const {
  if (!param) {
    return "";
  }
...
std::string SavedataParam::GetSaveDir(const SceUtilitySavedataParam *param,
                                      const std::string &saveDirName) const {
  if (!param) {
    return "";
  }
...
std::string SavedataParam::GetSaveFilePath(const SceUtilitySavedataParam *param,
...
std::string
SavedataParam::GetGameName(const SceUtilitySavedataParam *param) const {
  return FixedToString(param->gameName, ARRAY_SIZE(param->gameName));
}

std::string
SavedataParam::GetSaveName(const SceUtilitySavedataParam *param) const {
  const std::string saveName =
      FixedToString(param->saveName, ARRAY_SIZE(param->saveName));
  if (saveName == "<>")
    return "";
  return saveName;
}
...
std::string
SavedataParam::GetFileName(const SceUtilitySavedataParam *param) const {
...
bool SavedataParam::Delete(SceUtilitySavedataParam *param, int saveId) {
  ...
  if (!strnlen(param->gameName, sizeof(param->gameName)) &&
      param->mode != SCE_UTILITY_SAVEDATA_TYPE_LISTALLDELETE) {
    ERROR_LOG(Log::sceUtility,
              "Bad param with gameName empty - cannot delete save directory");
    return false;
  }
  ...
  if (dirPath.size() == 0) {
    ERROR_LOG(Log::sceUtility,
              "GetSaveFilePath (%.*s) returned empty - cannot delete save "
              "directory. Might already be deleted?",
              (int)sizeof(param->gameName), param->gameName);
    return false;
  }
...
int SavedataParam::DeleteData(SceUtilitySavedataParam *param) {
  ...
  if (!subFolder.size()) {
    ERROR_LOG(Log::sceUtility, "Bad subfolder, ignoring delete of %s",
              filePath.c_str());
    return 0;
  }
  ...
  if (sfoFile) {
    ...
    u32 fileListSize = 0;
    SaveSFOFileListEntry *fileList =
        (SaveSFOFileListEntry *)sfoFile->GetValueData("SAVEDATA_FILE_LIST",
                                                      &fileListSize);
    size_t fileListCount = fileListSize / sizeof(SaveSFOFileListEntry);
    bool changed = false;
    for (size_t i = 0; i < fileListCount; ++i) {
      if (strncmp(fileList[i].filename, fileName.c_str(),
                  sizeof(fileList[i].filename)) != 0)
        continue;
    ...
    if (changed) {
      ...
      sfoFile->SetValue("SAVEDATA_FILE_LIST", updatedList.get(), fileListSize,
                        (int)FILE_LIST_TOTAL_SIZE);
...
int SavedataParam::Save(SceUtilitySavedataParam *param,
                        const std::string &saveDirName, bool secureMode) {
  if (!param) {
    return SCE_UTILITY_SAVEDATA_ERROR_SAVE_MS_NOSPACE;
  }
  if (param->dataSize > param->dataBufSize) {
    ERROR_LOG_REPORT(Log::sceUtility, "Savedata buffer overflow: %d / %d",
  ...
  auto validateSize = [](const PspUtilitySavedataFileData &data) {
    if (data.buf.IsValid() && data.bufSize < data.size) {
      ERROR_LOG_REPORT(Log::sceUtility,
                       "Savedata subdata buffer overflow: %d / %d", data.size,
                       data.bufSize);
      return false;
    }
    return true;
  };
  if (!validateSize(param->icon0FileData) ||
      !validateSize(param->icon1FileData) ||
      !validateSize(param->pic1FileData) ||
      !validateSize(param->snd0FileData)) {
    return SCE_UTILITY_SAVEDATA_ERROR_RW_BAD_PARAMS;
  }

  if (param->secureVersion > 3) {
    ERROR_LOG_REPORT(Log::sceUtility, "Savedata version requested on save: %d",
                     param->secureVersion);
    ...
    if (param->secureVersion != 1 && !HasKey(param) && secureMode) {
      ERROR_LOG_REPORT(Log::sceUtility,
                       "Savedata version with missing key on save: %d",
                       param->secureVersion);
      return SCE_UTILITY_SAVEDATA_ERROR_SAVE_PARAM;
    }
    INFO_LOG(Log::sceUtility, "Savedata version requested on save: %d",
             param->secureVersion);
  }
  ...
  if (!pspFileSystem.GetFileInfo(dirPath).exists) {
    if (!pspFileSystem.MkDir(dirPath)) {
      ...
      g_OSD.Show(OSDType::MESSAGE_ERROR,
                 err->T("Unable to write savedata, disk may be full"));
    }
  }
  ...
  if (param->dataBuf.IsValid() && g_Config.bEncryptSave && secureMode) {
    ...
    if (aligned_len != cryptedSize) {
      WARN_LOG(Log::sceUtility, "cryptedSize unaligned: %d (%d)", cryptedSize,
               cryptedSize & 15);
    }
    ...
    if (EncryptData(decryptMode, cryptedData, &cryptedSize, &aligned_len,
                    cryptedHash, (hasKey ? param->key : 0)) != 0) {
      auto err = GetI18NCategory(I18NCat::ERRORS);
      g_OSD.Show(
          OSDType::MESSAGE_WARNING,
          err->T("Save encryption failed. This save won't work on real PSP"),
          6.0f);
      ERROR_LOG(Log::sceUtility,
                "Save encryption failed. This save won't work on real PSP");
      delete[] cryptedData;
      cryptedData = 0;
    }
  }
  ...
  bool subWrite = param->mode == SCE_UTILITY_SAVEDATA_TYPE_WRITEDATASECURE ||
                  param->mode == SCE_UTILITY_SAVEDATA_TYPE_WRITEDATA;
  bool wasCrypted = GetSaveCryptMode(param, saveDirName) != 0;

  // Update values. NOTE! #18430 made this conditional on !subWrite, but this is
  // not correct, as it causes #18687. So now we do a hacky trick and just check
  // for a valid title before we proceed with updating the sfoFile.
  ...
  // For each file, 13 bytes for filename, 16 bytes for file hash (0 in PPSSPP),
  // 3 byte for padding
  u32 tmpDataSize = 0;
  SaveSFOFileListEntry *tmpDataOrig =
      (SaveSFOFileListEntry *)sfoFile->GetValueData("SAVEDATA_FILE_LIST",
                                                    &tmpDataSize);
  SaveSFOFileListEntry *updatedList =
      new SaveSFOFileListEntry[FILE_LIST_COUNT_MAX];
  if (tmpDataSize != 0)
    memcpy(updatedList, tmpDataOrig,
           std::min(tmpDataSize, FILE_LIST_TOTAL_SIZE));
  ...
  if (secureMode && param->dataBuf.IsValid()) {
    ...
    for (auto entry = updatedList; entry < updatedList + FILE_LIST_COUNT_MAX;
         ++entry) {
      if (entry->filename[0] != '\0') {
        if (strncmp(entry->filename, saveFilename.c_str(),
                    sizeof(entry->filename)) != 0)
          continue;
      }
  ...
  sfoFile->SetValue("SAVEDATA_FILE_LIST", (u8 *)updatedList,
                    FILE_LIST_TOTAL_SIZE, (int)FILE_LIST_TOTAL_SIZE);
  delete[] updatedList;

  // Init param with 0. This will be used to detect crypted save or not on
  // loading
  u8 zeroes[128]{};
  ...
  if (param->dataBuf.IsValid()) // Can launch save without save data in mode 13
  {
    ...
    INFO_LOG(Log::sceUtility, "Saving file with size %u in %s", saveSize,
             filePath.c_str());
  ...
  if (param->icon0FileData.buf.IsValid()) {
    ...
    WritePSPFile(icon0path, param->icon0FileData.buf,
                 param->icon0FileData.size);
  }
  // SAVE ICON1
  if (param->icon1FileData.buf.IsValid()) {
    std::string icon1path = dirPath + "/" + ICON1_FILENAME;
    WritePSPFile(icon1path, param->icon1FileData.buf,
                 param->icon1FileData.size);
  }
  // SAVE PIC1
...
int SavedataParam::Load(SceUtilitySavedataParam *param,
                        const std::string &saveDirName, int saveId,
                        bool secureMode) {
  if (!param) {
    return SCE_UTILITY_SAVEDATA_ERROR_LOAD_NO_DATA;
  }

  bool isRWMode = param->mode == SCE_UTILITY_SAVEDATA_TYPE_READDATA ||
                  param->mode == SCE_UTILITY_SAVEDATA_TYPE_READDATASECURE;
  ...
  if (!pspFileSystem.GetFileInfo(dirPath).exists) {
    return isRWMode ? SCE_UTILITY_SAVEDATA_ERROR_RW_NO_DATA
                    : SCE_UTILITY_SAVEDATA_ERROR_LOAD_NO_DATA;
  }

  if (!fileName.empty() && !pspFileSystem.GetFileInfo(filePath).exists) {
    return isRWMode ? SCE_UTILITY_SAVEDATA_ERROR_RW_FILE_NOT_FOUND
                    : SCE_UTILITY_SAVEDATA_ERROR_LOAD_FILE_NOT_FOUND;
  }

  // If it wasn't zero, force to zero before loading and especially in case of
  // error. This isn't reset if the path doesn't even exist.
  param->dataSize = 0;
  ...
  if (!LoadSFO(param, dirPath)) {
    WARN_LOG(Log::sceUtility, "Load: Failed to load SFO from %s",
             dirPath.c_str());
    return isRWMode ? SCE_UTILITY_SAVEDATA_ERROR_RW_DATA_BROKEN
                    : SCE_UTILITY_SAVEDATA_ERROR_LOAD_DATA_BROKEN;
  }

  // Don't know what it is, but PSP always respond this and this unlock some
  // game
  param->bind = 1021;

  // Load other files, seems these are required by some games, e.g. Fushigi no
  // Dungeon Fuurai no Shiren 4 Plus.
...
int SavedataParam::LoadSaveData(SceUtilitySavedataParam *param,
                                const std::string &saveDirName,
                                const std::string &dirPath, bool secureMode) {
  if (param->secureVersion > 3) {
    ERROR_LOG_REPORT(Log::sceUtility, "Savedata version requested: %d",
                     param->secureVersion);
    return SCE_UTILITY_SAVEDATA_ERROR_LOAD_PARAM;
  } else if (param->secureVersion != 0) {
    if (param->secureVersion != 1 && !HasKey(param) && secureMode) {
      ERROR_LOG_REPORT(Log::sceUtility, "Savedata version with missing key: %d",
                       param->secureVersion);
      return SCE_UTILITY_SAVEDATA_ERROR_LOAD_PARAM;
    }
    WARN_LOG_REPORT(Log::sceUtility, "Savedata version requested: %d",
                    param->secureVersion);
  }
  ...
  s64 readSize;
  INFO_LOG(Log::sceUtility, "Loading file with size %u in %s",
           param->dataBufSize, filePath.c_str());
  u8 *saveData = nullptr;
  int saveSize = -1;
  ...
  if (isCrypted) {
    ...
    loadedSize = LoadCryptedSave(param, param->dataBuf, saveData, saveSize,
                                 prevCryptMode,
                                 hasExpectedHash ? hash : nullptr, saveDone);
    // TODO: Should return SCE_UTILITY_SAVEDATA_ERROR_LOAD_DATA_BROKEN here if
    // !saveDone.
  }
  if (!saveDone) {
  ...
  if (loadedSize != 0 && (loadedSize & 0x80000000) == 0) {
    ...
    NotifyMemInfo(MemBlockFlags::WRITE, param->dataBuf.ptr, loadedSize,
                  tag.c_str(), tag.size());
  }
...
int SavedataParam::DetermineCryptMode(
    const SceUtilitySavedataParam *param) const {
  int decryptMode = 1;
  if (param->secureVersion == 1) {
    ...
    decryptMode =
        GetSDKMainVersion(sceKernelGetCompiledSdkVersion()) >= 4 ? 5 : 1;
  } else if (HasKey(param)) {
    // TODO: This should ignore HasKey(), which would trigger errors.  Not doing
    // that yet to play it safe.
    decryptMode =
        GetSDKMainVersion(sceKernelGetCompiledSdkVersion()) >= 4 ? 5 : 3;
  }
  return decryptMode;
}

u32 SavedataParam::LoadCryptedSave(SceUtilitySavedataParam *param, u8 *data,
                                   const u8 *saveData, int &saveSize,
                                   int prevCryptMode, const u8 *expectedHash,
                                   bool &saveDone) {
  ...
  if (decryptMode != prevCryptMode) {
    if (prevCryptMode == 1 && param->key[0] == 0) {
      ...
      WARN_LOG(Log::sceUtility,
               "Savedata loading with hashmode %d instead of detected %d",
               prevCryptMode, decryptMode);
      ...
      if (!g_Config.bEncryptSave) {
        ...
        g_OSD.Show(OSDType::MESSAGE_WARNING,
                   di->T("When you save, it will load on a PSP, but not an "
                         "older PPSSPP"),
                   6.0f);
        g_OSD.Show(OSDType::MESSAGE_WARNING, di->T("Old savedata detected"),
                   6.0f);
      }
    } else {
      if (decryptMode == 5 && prevCryptMode == 3) {
        WARN_LOG(
            Log::sceUtility,
            "Savedata loading with detected hashmode %d instead of file's %d",
            decryptMode, prevCryptMode);
      } else {
        WARN_LOG_REPORT(
            Log::sceUtility,
      ...
      auto di = GetI18NCategory(I18NCat::DIALOG);
      g_OSD.Show(OSDType::MESSAGE_WARNING,
                 di->T("When you save, it will not work on outdated PSP "
                       "Firmware anymore"),
                 6.0f);
      g_OSD.Show(OSDType::MESSAGE_WARNING, di->T("Old savedata detected"),
                 6.0f);
  ...
  int err = DecryptData(decryptMode, data_base, &saveSize, &align_len,
                        hasKey ? cryptKey : nullptr, expectedHash);
  // Perhaps the file had the wrong mode....
  if (err != 0 && detectedMode != decryptMode) {
    resetData(detectedMode);
    err = DecryptData(detectedMode, data_base, &saveSize, &align_len,
                      hasKey ? cryptKey : nullptr, expectedHash);
  }
  ...
  if (err != 0 && expectedHash != nullptr) {
    ...
    resetData(decryptMode);
    err = DecryptData(decryptMode, data_base, &saveSize, &align_len,
                      hasKey ? cryptKey : nullptr, nullptr);
  }

  u32 sz = 0;
  if (err == 0) {
    if (param->dataBuf.IsValid()) {
      if ((u32)saveSize > param->dataBufSize ||
          !Memory::IsValidRange(param->dataBuf.ptr, saveSize)) {
...
u32 SavedataParam::LoadNotCryptedSave(SceUtilitySavedataParam *param, u8 *data,
                                      u8 *saveData, int &saveSize) {
  if (param->dataBuf.IsValid()) {
    if ((u32)saveSize > param->dataBufSize ||
        !Memory::IsValidRange(param->dataBuf.ptr, saveSize)) {
...
bool SavedataParam::LoadSFO(SceUtilitySavedataParam *param,
                            const std::string &dirPath) {
  std::string sfopath = dirPath + "/" + SFO_FILENAME;
  std::shared_ptr<ParamSFOData> sfoFile = LoadCachedSFO(sfopath);
  if (sfoFile) {
    // copy back info in request
    strncpy(param->sfoParam.title, sfoFile->GetValueString("TITLE").c_str(),
            128);
    strncpy(param->sfoParam.savedataTitle,
            sfoFile->GetValueString("SAVEDATA_TITLE").c_str(), 128);
    strncpy(param->sfoParam.detail,
            sfoFile->GetValueString("SAVEDATA_DETAIL").c_str(), 1024);
...
std::vector<SaveSFOFileListEntry>
SavedataParam::GetSFOEntries(const std::string &dirPath) {
  ...
  SaveSFOFileListEntry *sfoFileList =
      (SaveSFOFileListEntry *)sfoFile->GetValueData("SAVEDATA_FILE_LIST",
                                                    &sfoFileListSize);
  const u32 count =
      std::min((u32)FILE_LIST_COUNT_MAX,
               sfoFileListSize / (u32)sizeof(SaveSFOFileListEntry));
...
std::set<std::string>
SavedataParam::GetSecureFileNames(const std::string &dirPath) {
...
bool SavedataParam::GetExpectedHash(const std::string &dirPath,
                                    const std::string &filename, u8 hash[16]) {
  auto entries = GetSFOEntries(dirPath);

  for (const auto &entry : entries) {
    if (strncmp(entry.filename, filename.c_str(), sizeof(entry.filename)) ==
        0) {
...
void SavedataParam::LoadFile(const std::string &dirPath,
                             const std::string &filename,
                             PspUtilitySavedataFileData *fileData) {
  std::string filePath = dirPath + "/" + filename;
  ...
  if (ReadPSPFile(filePath, &buf, size, &readSize)) {
    ...
    NotifyMemInfo(MemBlockFlags::WRITE, fileData->buf.ptr, fileData->size,
                  tag.c_str(), tag.size());
    INFO_LOG(Log::sceUtility, "Loaded subfile %s (size: %d bytes) into %08x",
             filePath.c_str(), fileData->size, fileData->buf.ptr);
  } else {
    WARN_LOG(Log::sceUtility, "Failed to load subfile %s into %08x",
             filePath.c_str(), fileData->buf.ptr);
  }
}

// Note: The work is done in-place, hence the memmove etc.
int SavedataParam::EncryptData(unsigned int mode, unsigned char *data,
                               int *dataLen, int *alignedLen,
                               unsigned char *hash, const u8 *cryptkey) {
  pspChnnlsvContext1 ctx1{};
  pspChnnlsvContext2 ctx2{};

  INFO_LOG(Log::sceUtility, "EncryptData(mode=%d, *dataLen=%d, *alignedLen=%d)",
...
// Note: The work is done in-place, hence the memmove etc.
int SavedataParam::DecryptData(unsigned int mode, unsigned char *data,
                               int *dataLen, int *alignedLen,
                               const u8 *cryptkey, const u8 *expectedHash) {
  pspChnnlsvContext1 ctx1{};
  pspChnnlsvContext2 ctx2{};
...
// Requires sfoData to be padded with zeroes to the next 16-byte boundary (due
// to BuildHash)
int SavedataParam::UpdateHash(u8 *sfoData, int sfoSize, int sfoDataParamsOffset,
                              int encryptmode) {
  ...
  if ((ret = BuildHash(filehash, sfoData, sfoSize, alignedLen, firstHashMode,
                       0)) < 0) {
    // Not sure about "2"
    return ret - 400;
  }
  ...
  *(sfoData + sfoDataParamsOffset) |= 0x01;
  ...
  if (encryptmode & 6) {
    ...
    *(sfoData + sfoDataParamsOffset) |= (encryptmode & 6) << 4;
    ...
    if ((ret = BuildHash(filehash, sfoData, sfoSize, alignedLen, secondHashMode,
                         0)) < 0) {
      return ret - 500;
    }
...
int SavedataParam::BuildHash(uint8_t *output, const uint8_t *data,
                             unsigned int len, unsigned int alignedLen,
                             int mode, const uint8_t *cryptkey) {
  pspChnnlsvContext1 ctx1;
...
inline std::string FmtPspTime(const ScePspDateTime &dt) {
  return StringFromFormat("%04d-%02d-%02d %02d:%02d:%02d.%06d", dt.year,
...
int SavedataParam::GetSizes(SceUtilitySavedataParam *param) {
  ...
  if (param->msFree.IsValid()) {
    ...
    strncpy(param->msFree->freeSpaceStr, spaceTxt.c_str(),
            sizeof(param->msFree->freeSpaceStr));
    NotifyMemInfo(MemBlockFlags::WRITE, param->msFree.ptr,
                  sizeof(SceUtilitySavedataMsFreeInfo), "SavedataGetSizes");
  }
  if (param->msData.IsValid()) {
    const SceUtilitySavedataMsDataInfo *msData = param->msData;
    const std::string gameName(
        msData->gameName, strnlen(msData->gameName, sizeof(msData->gameName)));
    const std::string saveName(
        msData->saveName, strnlen(msData->saveName, sizeof(msData->saveName)));
    // TODO: How should <> be handled?
    std::string path =
        GetSaveFilePath(param, gameName + (saveName == "<>" ? "" : saveName));
    ...
    if (listingExists) {
      ...
      for (auto &item : listing) {
        param->msData->info.usedClusters +=
            (item.size + (u32)MemoryStick_SectorSize() - 1) /
            (u32)MemoryStick_SectorSize();
      }

      // The usedSpaceKB value is definitely based on clusters, not bytes or
      // even KB. Fieldrunners expects 736 KB, even though the files add up to
      // ~600 KB.
      int total_size =
          param->msData->info.usedClusters * (u32)MemoryStick_SectorSize();
      param->msData->info.usedSpaceKB = total_size / 0x400;
      std::string spaceTxt = SavedataParam::GetSpaceText(total_size, true);
      strncpy(param->msData->info.usedSpaceStr, spaceTxt.c_str(),
              sizeof(param->msData->info.usedSpaceStr));

      // TODO: What does this mean, then?  Seems to be the same.
      param->msData->info.usedSpace32KB = param->msData->info.usedSpaceKB;
      strncpy(param->msData->info.usedSpace32Str, spaceTxt.c_str(),
              sizeof(param->msData->info.usedSpace32Str));
    } else {
      param->msData->info.usedClusters = 0;
      param->msData->info.usedSpaceKB = 0;
      strncpy(param->msData->info.usedSpaceStr, "",
              sizeof(param->msData->info.usedSpaceStr));
      param->msData->info.usedSpace32KB = 0;
      strncpy(param->msData->info.usedSpace32Str, "",
              sizeof(param->msData->info.usedSpace32Str));
      ret = SCE_UTILITY_SAVEDATA_ERROR_SIZES_NO_DATA;
    }
    NotifyMemInfo(MemBlockFlags::WRITE, param->msData.ptr,
                  sizeof(SceUtilitySavedataMsDataInfo), "SavedataGetSizes");
  }
  if (param->utilityData.IsValid()) {
    ...
    param->utilityData->usedClusters =
        total_size / (u32)MemoryStick_SectorSize();
    param->utilityData->usedSpaceKB = total_size / 0x400;
    std::string spaceTxt = SavedataParam::GetSpaceText(total_size, true);
    memset(param->utilityData->usedSpaceStr, 0,
           sizeof(param->utilityData->usedSpaceStr));
    strncpy(param->utilityData->usedSpaceStr, spaceTxt.c_str(),
            sizeof(param->utilityData->usedSpaceStr));
    ...
    memset(param->utilityData->usedSpace32Str, 0,
           sizeof(param->utilityData->usedSpace32Str));
    strncpy(param->utilityData->usedSpace32Str, spaceTxt32.c_str(),
            sizeof(param->utilityData->usedSpace32Str));

    INFO_LOG(Log::sceUtility,
             "GetSize: usedSpaceKB: %d (str: %s) (clusters: %d)",
             param->utilityData->usedSpaceKB, spaceTxt.c_str(),
             param->utilityData->usedClusters);
    INFO_LOG(Log::sceUtility, "GetSize: usedSpace32KB: %d (str32: %s)",
             param->utilityData->usedSpace32KB, spaceTxt32.c_str());

    NotifyMemInfo(MemBlockFlags::WRITE, param->utilityData.ptr,
                  sizeof(SceUtilitySavedataUsedDataInfo), "SavedataGetSizes");
  }
  return ret;
}
...
bool SavedataParam::GetList(SceUtilitySavedataParam *param) {
  ...
  if (param->idList.IsValid()) {
    ...
    // TODO: Here we can filter by prefix - only the savename in param is likely
    // to be a regex.
    ...
    for (size_t i = 0; i < allDir.size() && validDir.size() < maxFileCount;
         i++) {
    ...
    for (size_t i = 0; i < validDir.size(); ++i) {
      ...
      sfoFile = pspFileSystem.GetFileInfo(savePath + validDir[i].name + "/" +
                                          SFO_FILENAME);
    ...
    if (GenericLogEnabled(Log::sceUtility, LogLevel::LINFO)) {
      INFO_LOG(Log::sceUtility, "LIST (searchstring=%s): %d files (max: %d)",
               searchString.c_str(), param->idList->resultCount, maxFileCount);
      for (int i = 0; i < validDir.size(); i++) {
        INFO_LOG(Log::sceUtility,
                 "%s: mode %08x, ctime: %s, atime: %s, mtime: %s",
                 entries[i].name, entries[i].st_mode,
                 FmtPspTime(entries[i].st_ctime).c_str(),
                 FmtPspTime(entries[i].st_atime).c_str(),
                 FmtPspTime(entries[i].st_mtime).c_str());
      }
    }
    NotifyMemInfo(MemBlockFlags::WRITE, param->idList.ptr,
                  sizeof(SceUtilitySavedataIdListInfo), "SavedataGetList");
    NotifyMemInfo(MemBlockFlags::WRITE, param->idList->entries.ptr,
                  (uint32_t)validDir.size() *
                      sizeof(SceUtilitySavedataIdListEntry),
                  "SavedataGetList");
  }
  return true;
}
...
int SavedataParam::GetFilesList(SceUtilitySavedataParam *param,
  ...
  if (!param->fileList.IsValid()) {
    ERROR_LOG_REPORT(Log::sceUtility,
                     "SavedataParam::GetFilesList(): bad fileList address %08x",
                     param->fileList.ptr);
    // Should crash.
    return -1;
  }
  ...
  if (fileList->secureEntries.IsValid() && fileList->maxSecureEntries > 99) {
    ERROR_LOG_REPORT(
        Log::sceUtility,
        "SavedataParam::GetFilesList(): too many secure entries, %d",
        fileList->maxSecureEntries);
    return SCE_UTILITY_SAVEDATA_ERROR_RW_BAD_PARAMS;
  }
  if (fileList->normalEntries.IsValid() && fileList->maxNormalEntries > 8192) {
    ERROR_LOG_REPORT(
        Log::sceUtility,
        "SavedataParam::GetFilesList(): too many normal entries, %d",
        fileList->maxNormalEntries);
    return SCE_UTILITY_SAVEDATA_ERROR_RW_BAD_PARAMS;
  }
  if (sceKernelGetCompiledSdkVersion() >= 0x02060000) {
    if (fileList->systemEntries.IsValid() && fileList->maxSystemEntries > 5) {
      ERROR_LOG_REPORT(
          Log::sceUtility,
          "SavedataParam::GetFilesList(): too many system entries, %d",
          fileList->maxSystemEntries);
      return SCE_UTILITY_SAVEDATA_ERROR_RW_BAD_PARAMS;
    }
  }
  ...
  if (!dirPathExists) {
    DEBUG_LOG(Log::sceUtility,
              "SavedataParam::GetFilesList(): directory %s does not exist",
              dirPath.c_str());
    return SCE_UTILITY_SAVEDATA_ERROR_RW_NO_DATA;
  }
  ...
  // We need PARAM.SFO's SAVEDATA_FILE_LIST to determine which entries are
  // secure.
  ...
  param->bind = 1021;
  // This should be set around the same time as the file data.  This runs on a
  // thread, so set immediately.
  auto requestPtr = PSPPointer<SceUtilitySavedataParam>::Create(requestAddr);
  requestPtr->bind = 1021;

  // Does not list directories, nor recurse into them, and ignores files not ALL
  // UPPERCASE.
  ...
  for (auto file = files.begin(), end = files.end(); file != end; ++file) {
    ...
    // TODO: What are the exact rules?  It definitely skips lowercase, and
    // allows FILE or FILE.EXT.
    if (file->name.find_first_of("abcdefghijklmnopqrstuvwxyz") !=
        file->name.npos) {
      DEBUG_LOG(
          Log::sceUtility,
          "SavedataParam::GetFilesList(): skipping file %s with lowercase",
          file->name.c_str());
      continue;
    }

    bool isSystemFile = file->name == ICON0_FILENAME ||
                        file->name == ICON1_FILENAME ||
                        file->name == PIC1_FILENAME;
    isSystemFile = isSystemFile || file->name == SND0_FILENAME ||
                   file->name == SFO_FILENAME;

    SceUtilitySavedataFileListEntry *entry = NULL;
    int sizeOffset = 0;
    if (isSystemFile) {
      if (fileList->systemEntries.IsValid() &&
          fileList->resultNumSystemEntries < fileList->maxSystemEntries) {
        entry = &fileList->systemEntries[fileList->resultNumSystemEntries++];
      }
    } else if (secureFilenames.find(file->name) != secureFilenames.end()) {
      if (fileList->secureEntries.IsValid() &&
          fileList->resultNumSecureEntries < fileList->maxSecureEntries) {
      ...
      if (fileList->normalEntries.IsValid() &&
          fileList->resultNumNormalEntries < fileList->maxNormalEntries) {
  ...
  if (GenericLogEnabled(Log::sceUtility, LogLevel::LINFO)) {
    INFO_LOG(Log::sceUtility, "FILES: %d files listed (+ %d system, %d secure)",
             fileList->resultNumNormalEntries, fileList->resultNumSystemEntries,
             fileList->resultNumSecureEntries);
    if (fileList->normalEntries.IsValid()) {
      for (int i = 0; i < (int)fileList->resultNumNormalEntries; i++) {
        const SceUtilitySavedataFileListEntry &info =
            fileList->normalEntries[i];
        INFO_LOG(Log::sceUtility,
                 "%s: mode %08x, ctime: %s, atime: %s, mtime: %s", info.name,
                 info.st_mode, FmtPspTime(info.st_ctime).c_str(),
                 FmtPspTime(info.st_atime).c_str(),
                 FmtPspTime(info.st_mtime).c_str());
      }
    } else if (fileList->resultNumNormalEntries > 0) {
      WARN_LOG(Log::sceUtility, "Invalid normalEntries pointer (%d entries)",
               fileList->resultNumNormalEntries);
    }
    // TODO: Log system and secure entries?
  }

  NotifyMemInfo(MemBlockFlags::WRITE, fileList.ptr,
                sizeof(SceUtilitySavedataFileListInfo), "SavedataGetFilesList");
  if (fileList->resultNumSystemEntries != 0)
    NotifyMemInfo(MemBlockFlags::WRITE, fileList->systemEntries.ptr,
                  fileList->resultNumSystemEntries *
                      sizeof(SceUtilitySavedataFileListEntry),
                  "SavedataGetFilesList");
  if (fileList->resultNumSecureEntries != 0)
    NotifyMemInfo(MemBlockFlags::WRITE, fileList->secureEntries.ptr,
                  fileList->resultNumSecureEntries *
                      sizeof(SceUtilitySavedataFileListEntry),
                  "SavedataGetFilesList");
  if (fileList->resultNumNormalEntries != 0)
    NotifyMemInfo(MemBlockFlags::WRITE, fileList->normalEntries.ptr,
                  fileList->resultNumNormalEntries *
                      sizeof(SceUtilitySavedataFileListEntry),
                  "SavedataGetFilesList");
...
bool SavedataParam::GetSize(SceUtilitySavedataParam *param) {
  ...
  const std::string saveDir =
      savePath + GetGameName(param) + GetSaveName(param);
  bool exists = false;
  ...
  if (param->sizeInfo.IsValid()) {
    ...
    INFO_LOG(Log::sceUtility,
             "SectorSize: %d FreeSectors: %d FreeKB: %d neededKb: %d "
             "overwriteKb: %d",
             param->sizeInfo->sectorSize, param->sizeInfo->freeSectors,
             param->sizeInfo->freeKB, param->sizeInfo->neededKB,
             param->sizeInfo->overwriteKB);

    NotifyMemInfo(MemBlockFlags::WRITE, param->sizeInfo.ptr,
                  sizeof(PspUtilitySavedataSizeInfo), "SavedataGetSize");
  }
  ...
  if (saveDataList) {
    for (int i = 0; i < saveNameListDataCount; i++) {
      if (saveDataList[i].texture != NULL &&
          (!noSaveIcon || saveDataList[i].texture != noSaveIcon->texture))
...
int SavedataParam::SetPspParam(SceUtilitySavedataParam *param) {
  ...
  if (param->mode == SCE_UTILITY_SAVEDATA_TYPE_LISTALLDELETE) {
    ...
    for (auto save : allSaves) {
      if (save.type != FILETYPE_DIRECTORY || save.name == "." ||
          save.name == "..")
        continue;
  ...
  if (param->saveNameList.IsValid()) {
    ...
    if (saveDataListCount > 0 && WouldHaveMultiSaveName(param)) {
      ...
      for (int i = 0; i < saveDataListCount; i++) {
        ...
        if (strncmp(saveNameListData[i], "<>",
                    ARRAY_SIZE(saveNameListData[i])) == 0) {
          ...
          for (auto it = allSaves.begin(); it != allSaves.end(); ++it) {
            if (it->name.compare(0, gameName.length(), gameName) == 0) {
              ...
              if (IsInSaveDataList(
                      saveName, realCount)) // Already in SaveDataList, skip...
                continue;
        ...
        const std::string thisSaveName =
            FixedToString(saveNameListData[i], ARRAY_SIZE(saveNameListData[i]));
        ...
        if (found) {
          ...
          if (info.exists) {
            ...
            INFO_LOG(Log::sceUtility, "Save data exists: %s = %s",
                     thisSaveName.c_str(), fileDataDir.c_str());
            realCount++;
          } else {
            found = false;
          }
        ...
        if (!found) { // NOTE: May be changed above, can't merge with the
                      // expression
          if (listEmptyFile) {
            ClearFileInfo(saveDataList[realCount], thisSaveName);
            DEBUG_LOG(Log::sceUtility, "Listing missing save data: %s = %s",
                      thisSaveName.c_str(), fileDataDir.c_str());
            realCount++;
          } else {
            INFO_LOG(Log::sceUtility, "Save data not found: %s = %s",
                     thisSaveName.c_str(), fileDataDir.c_str());
          }
        }
      }
  ...
  if (!hasMultipleFileName) {
    ...
    if (info.exists) {
      ...
      INFO_LOG(Log::sceUtility, "Save data exists: %s = %s",
               GetSaveName(param).c_str(), fileDataDir.c_str());
...
void SavedataParam::SetFileInfo(SaveFileInfo &saveInfo, PSPFileInfo &info,
                                const std::string &saveName,
                                const std::string &savrDir) {
  ...
  std::string saveDir =
      savrDir.empty() ? GetGameName(pspParam) + saveName : savrDir;
  ...
  // TODO : If icon0 don't exist, need to use icon1 which is a moving icon. Also
  // play sound
  std::string iconPath = savePath + saveDir + "/" + ICON0_FILENAME;

  if (!ignoreTextures_) {
    saveInfo.texture = new PPGeImage(iconPath);
  }
  ...
  if (sfoFile) {
    ...
    SetStringFromSFO(*sfoFile, "SAVEDATA_TITLE", saveInfo.saveTitle,
                     sizeof(saveInfo.saveTitle));
    SetStringFromSFO(*sfoFile, "SAVEDATA_DETAIL", saveInfo.saveDetail,
                     sizeof(saveInfo.saveDetail));
  } else {
...
void SavedataParam::ClearFileInfo(SaveFileInfo &saveInfo,
                                  const std::string &saveName) {
  saveInfo.size = 0;
  saveInfo.saveName = saveName;
  saveInfo.idx = 0;
  ...
  if (GetPspParam()->newData.IsValid() &&
      GetPspParam()->newData->buf.IsValid()) {
    ...
    if (!noSaveIcon) {
      ...
      if (Memory::IsValidRange(newData->buf.ptr, newData->size)) {
        noSaveIcon->texture =
            new PPGeImage(newData->buf.ptr, (SceSize)newData->size);
      }
    }
    saveInfo.texture = noSaveIcon->texture;
  } else if ((u32)GetPspParam()->mode == SCE_UTILITY_SAVEDATA_TYPE_SAVE &&
             GetPspParam()->icon0FileData.buf.IsValid()) {
    const PspUtilitySavedataFileData &icon0FileData =
        GetPspParam()->icon0FileData;
    saveInfo.texture =
        new PPGeImage(icon0FileData.buf.ptr, (SceSize)icon0FileData.size);
  }
}
...
PSPFileInfo SavedataParam::GetSaveInfo(const std::string &saveDir) {
  ...
  if (info.exists) {
    ...
    for (auto file : allFiles) {
      if (file.type == FILETYPE_DIRECTORY || file.name == "." ||
          file.name == "..")
        continue;
...
SceUtilitySavedataParam *SavedataParam::GetPspParam() { return pspParam; }

const SceUtilitySavedataParam *SavedataParam::GetPspParam() const {
  return pspParam;
}

int SavedataParam::GetFilenameCount() { return saveNameListDataCount; }
...
int SavedataParam::GetSelectedSave() {
  // The slot # of the same save on LOAD/SAVE lists can dismatch so this isn't
  // right anyhow
  return selectedSave < saveNameListDataCount ? selectedSave : 0;
}

void SavedataParam::SetSelectedSave(int idx) { selectedSave = idx; }

int SavedataParam::GetFirstListSave() { return 0; }

int SavedataParam::GetLastListSave() { return saveNameListDataCount - 1; }
...
bool SavedataParam::WouldHaveMultiSaveName(
...
std::shared_ptr<ParamSFOData>
SavedataParam::LoadCachedSFO(const std::string &path, bool orCreate) {
  std::lock_guard<std::mutex> guard(cacheLock_);
...
int SavedataParam::GetSaveCryptMode(const SceUtilitySavedataParam *param,
                                    const std::string &saveDirName) {
  std::string dirPath = GetSaveFilePath(param, GetSaveDir(param, saveDirName));
  std::string sfopath = dirPath + "/" + SFO_FILENAME;
  std::shared_ptr<ParamSFOData> sfoFile = LoadCachedSFO(sfopath);
  if (sfoFile) {
    ...
    u32 tmpDataSize = 0;
    const u8 *tmpDataOrig =
        sfoFile->GetValueData("SAVEDATA_PARAMS", &tmpDataSize);
    ...
    default:
      ...
      ERROR_LOG_REPORT(Log::sceUtility,
                       "Unexpected SAVEDATA_PARAMS hash flag: %02x",
                       tmpDataOrig[0]);
      return 1;
    }
  }
