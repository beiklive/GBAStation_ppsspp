...
void PPGeSetDrawContext(Draw::DrawContext *draw) { g_draw = draw; }

// Overwrite the current text lines buffer so it can be drawn later.
void PPGePrepareText(std::string_view text, float x, float y, PPGeAlign align,
                     float scale, float lineHeightScale,
                     int WrapType = PPGE_LINE_NONE, int wrapWidth = 0);

// These functions must be called between PPGeBegin and PPGeEnd.

// Draw currently buffered text using the state from PPGeGetTextBoundingBox()
// call. Clears the buffer and state when done.
...
static void Vertex(float x, float y, float u, float v, int tw, int th,
...
static void EndVertexDataAndDraw(int prim) {
  ...
  if (vertexCount != 0) {
    NotifyMemInfo(MemBlockFlags::WRITE, vertexStart, dataWritePtr - vertexStart,
                  "PPGe Vertex");
...
bool PPGeIsFontTextureAddress(u32 addr) { return addr == atlasPtr; }
...
void __PPGeSetupListArgs() {
  ...
  if (listArgs.IsValid()) {
    ...
    if (savedContextPtr == 0)
      savedContextPtr =
          __PPGeDoAlloc(savedContextSize, false, "PPGe Saved Context");
...
void __PPGeInit() {
  ...
  bool loadedZIM =
      !skipZIM && LoadZIM("ppge_atlas.zim", width, height, &flags, imageData);
  if (!skipZIM && !loadedZIM) {
    ERROR_LOG(Log::sceGe, "Failed to load ppge_atlas.zim.\n\nPlace it in the "
                          "directory \"assets\" under your PPSSPP "
  ...
  u32 atlasSize =
      height[0] * width[0] / 2; // it's a 4-bit paletted texture in ram
  atlasWidth = width[0];
  ...
  __PPGeSetupListArgs();
  atlasPtr = atlasSize == 0
                 ? 0
                 : __PPGeDoAlloc(atlasSize, false, "PPGe Atlas Texture");
  palette = __PPGeDoAlloc(paletteSize, false, "PPGe Texture Palette");

  // Generate 16-greyscale palette. All PPGe graphics are greyscale so we can
  // use a tiny paletted texture.
  for (int i = 0; i < 16; i++) {
    int val = i;
    palette[i] = (val << 12) | 0xFFF;
  }
  NotifyMemInfo(MemBlockFlags::WRITE, palette.ptr, 16 * sizeof(u16_le),
                "PPGe Palette");

  const u32_le *imagePtr = (u32_le *)imageData[0];
  u8 *ramPtr = atlasPtr == 0
                   ? nullptr
                   : (u8 *)Memory::GetPointerRange(atlasPtr, atlasSize);
  ...
  INFO_LOG(Log::sceGe,
           "PPGe drawing library initialized. DL: %08x Data: %08x Atlas: %08x "
           "(%i) Args: %08x",
...
void __PPGeDoState(PointerWrap &p) {
  ...
  // If the atlas the save state was created with differs from the current one,
  // reload.
  ...
  if (s >= 4) {
    ...
    if (atlasPtr != 0) {
      savedHash = XXH3_64bits(
          Memory::GetPointerRange(atlasPtr, atlasWidth * atlasHeight / 2),
  ...
  if (s >= 3) {
    ...
    case PointerWrap::MODE_READ:
      ...
      for (uint32_t i = 0; i < sz; ++i) {
        // We only care about the pointers, so we can free them.  We'll decimate
        // right away.
        PPGeTextDrawerCacheKey key{StringFromFormat("__savestate__%d", i), -1,
                                   -1};
...
void PPGeBegin() {
  ...
  if (g_RemasterMode) {
    WriteCmd(GE_CMD_VERTEXTYPE, GE_VTYPE_TC_FLOAT | GE_VTYPE_COL_8888 |
                                    GE_VTYPE_POS_FLOAT | GE_VTYPE_THROUGH);
  } else {
    WriteCmd(GE_CMD_VERTEXTYPE, GE_VTYPE_TC_16BIT | GE_VTYPE_COL_8888 |
                                    GE_VTYPE_POS_FLOAT | GE_VTYPE_THROUGH);
  }
}
...
void PPGeEnd() {
  ...
  if (dataWritePtr > dataPtr) {
    ...
    NotifyMemInfo(MemBlockFlags::WRITE, dlPtr, dlWritePtr - dlPtr,
                  "PPGe ListCmds");
    u32 list = hleCall(sceGe_user, u32, sceGeListEnQueue, dlPtr, dlWritePtr, -1,
                       listArgs.ptr);
...
void PPGeScissorReset() { PPGeScissor(0, 0, 480, 272); }

static const AtlasChar *PPGeGetChar(const AtlasFont &atlasfont,
                                    unsigned int cval) {
...
static AtlasTextMetrics BreakLines(std::string_view text,
                                   const AtlasFont &atlasfont, float x, float y,
                                   PPGeAlign align, float scale,
                                   float lineHeightScale, int wrapType,
                                   float wrapWidth, bool dryRun) {
    ...
    while (!utf.end()) {
      ...
      while (!utfWord.end() && !finished) {
        ...
        case 0xFF1F: // FULLWIDTH QUESTION MARK
          // Count this character (punctuation is so clingy), but then we're
          // done.
          ++numChars;
        ...
        default: {
          ...
          isCJK = isCJK ||
                  (cval >= 0x2E80 && cval <= 0x2FFF); // Kangxi Radicals etc.
#if 0
...
#else
          isCJK =
              isCJK || (cval >= 0x3040 && cval <= 0x4DB5); // Above collapsed
#endif
          isCJK = isCJK ||
                  (cval >= 0x4E00 && cval <= 0x9FBB); // CJK Unified Ideographs
          isCJK =
              isCJK || (cval >= 0xAC00 && cval <= 0xD7AF); // Hangul Syllables
          isCJK = isCJK || (cval >= 0xF900 &&
                            cval <= 0xFAD9); // CJK Compatibility Ideographs
          isCJK =
              isCJK || (cval >= 0x20000 &&
                        cval <= 0x2A6D6); // CJK Unified Ideographs Extension B
          isCJK = isCJK || (cval >= 0x2F800 &&
                            cval <= 0x2FA1D); // CJK Compatibility Supplement
          if (isCJK) {
            if (numChars > 0) {
              utfWord = utfPrev;
              finished = true;
      ...
      if (wrapType > 0) {
        if (lineWidth + nextWidth > wrapWidth || skipRest) {
          if (wrapType & PPGE_LINE_WRAP_WORD) {
            // TODO: Should check if we have had at least one other word
            // instead.
            if (lineWidth > 0) {
              ++numLines;
              break;
...
static bool HasTextDrawer() {
  ...
  if (textDrawer) {
    ...
    textDrawer->SetOrCreateFont(
        FontStyle(FontFamily::SansSerif, 18, FontStyleFlags::Default));
  }
...
void PPGeMeasureText(float *w, float *h, std::string_view text, float scale,
                     int WrapType, int wrapWidth) {
  ...
  if (HasTextDrawer()) {
    ...
    textDrawer->MeasureStringRect(s, wrapWidth <= 0 ? 480.0f : wrapWidth, &mw,
                                  &mh, dtalign);
  ...
  AtlasTextMetrics metrics =
      BreakLines(s.c_str(), atlasfont, 0, 0, PPGeAlign::BOX_TOP, scale, scale,
                 WrapType, wrapWidth, true);
  if (w)
    *w = metrics.maxWidth;
  if (h)
    *h = metrics.lineHeight * metrics.numLines;
}

void PPGePrepareText(std::string_view text, float x, float y, PPGeAlign align,
                     float scale, float lineHeightScale, int WrapType,
                     int wrapWidth) {
  const AtlasFont &atlasfont = g_ppge_atlas.fonts[0];
  if (!g_ppge_atlas.IsMetadataLoaded() || g_ppge_atlas.num_fonts < 1) {
    return;
  }
  char_lines_metrics = BreakLines(text, atlasfont, x, y, align, scale,
                                  lineHeightScale, WrapType, wrapWidth, false);
}
...
void PPGeDrawCurrentText(u32 color) {
  ...
  bool useTextureWindow =
      g_Config.bSoftwareRendering && (atlasWidth > 512 || atlasHeight > 512);
  ...
  if (dlPtr) {
    ...
    for (auto i = char_lines.begin(); i != char_lines.end(); ++i) {
      for (auto j = i->begin(); j != i->end(); ++j) {
        ...
        if (useTextureWindow &&
            (wantedPosX != texturePosX || wantedPosY != texturePosY)) {
...
static PPGeTextDrawerImage PPGeGetTextImage(std::string_view text,
  ...
  PPGeTextDrawerCacheKey key{std::string(text), tdalign,
                             maxWidth / style.scale};
  ...
  if (cacheItem != textDrawerImages.end()) {
    ...
    textDrawer->DrawStringBitmapRect(bitmapData, im.entry,
                                     Draw::DataFormat::R8_UNORM,
                                     cleaned.c_str(), b, tdalign, false);
    ...
    if (im.ptr) {
      ...
      memset(ramPtr + im.entry.bmHeight * bufwBytes, 0,
             bufwBytes + sz - origSz);
    }
...
static void PPGeDrawTextImage(const PPGeTextDrawerImage &im, float x, float y,
                              const PPGeStyle &style) {
  if (!im.ptr) {
    return;
  }

  // Sometimes our texture can be larger than 512 wide, which means we need to
  // tile it.
  ...
  float tile2u1 =
      hasTile2 ? (float)(im.entry.width - 512) / (1 << tile2wp2) : 0.0f;
  ...
  if (style.hasShadow) {
    ...
    for (float dy = 0.0f; dy <= 2.0f; dy += 1.0f) {
      for (float dx = 0.0f; dx <= 1.0f; dx += 0.5f) {
        ...
        Vertex(x + dx + tile1w, y + dy + h, tile1u1, v1, 1 << tile1wp2,
               1 << hp2, shadowColor);
      }
    }
    ...
    if (hasTile2) {
      ...
      for (float dy = 0.0f; dy <= 2.0f; dy += 1.0f) {
        for (float dx = 0.0f; dx <= 1.0f; dx += 0.5f) {
          ...
          Vertex(x + tile2x + dx, y + dy, 0, 0, 1 << tile2wp2, 1 << hp2,
                 shadowColor);
          Vertex(x + tile2x + dx + tile2w, y + dy + h, tile2u1, v1,
                 1 << tile2wp2, 1 << hp2, shadowColor);
        }
      }
  ...
  if (hasTile2) {
    ...
    Vertex(x + tile2x + tile2w, y + h, tile2u1, v1, 1 << tile2wp2, 1 << hp2,
           style.color);
  }
  EndVertexDataAndDraw(GE_PRIM_RECTANGLES);
...
void PPGeDrawText(std::string_view text, float x, float y,
                  const PPGeStyle &style) {
  ...
  if (style.hasShadow) {
    ...
    PPGePrepareText(text, x + 1, y + 2, style.align, style.scale, style.scale,
                    PPGE_LINE_USE_ELLIPSIS);
    PPGeDrawCurrentText(style.shadowColor);
  }

  PPGePrepareText(text, x, y, style.align, style.scale, style.scale,
                  PPGE_LINE_USE_ELLIPSIS);
...
void PPGeDrawTextWrapped(std::string_view text, float x, float y,
                         float wrapWidth, float wrapHeight,
                         const PPGeStyle &style) {
  std::string s = PPGeSanitizeText(text);
  if (wrapHeight != 0.0f) {
  ...
  if (HasTextDrawer()) {
    ...
    int tdalign = 0;
    textDrawer->SetFontScale(style.scale, style.scale);
    textDrawer->MeasureStringRect(s, b.w, &actualWidth, &actualHeight,
                                  tdalign | FLAG_WRAP_TEXT);
    ...
    PPGeTextDrawerImage im = PPGeGetTextImage(
        s, adjustedStyle, wrapWidth <= 0 ? 480.0f - x : wrapWidth, true);
    if (im.ptr) {
  ...
  PPGePrepareText(s, x + sx, y + sy, style.align, style.scale, style.scale,
                  PPGE_LINE_USE_ELLIPSIS | PPGE_LINE_WRAP_WORD, wrapWidth);

  float scale = style.scale;
  float lineHeightScale = style.scale;
  float actualHeight =
      char_lines_metrics.lineHeight * char_lines_metrics.numLines;
  if (wrapHeight != 0.0f && actualHeight > wrapHeight) {
    if (actualHeight > wrapHeight * maxScaleDown) {
      float maxLines =
          floor(wrapHeight * maxScaleDown / char_lines_metrics.lineHeight);
    ...
    // Try to keep the font as large as possible, so reduce the line height
    // some.
    scale = reduced * 1.15f;
    lineHeightScale = reduced;
    PPGePrepareText(s, x + sx, y + sy, style.align, scale, lineHeightScale,
                    PPGE_LINE_USE_ELLIPSIS | PPGE_LINE_WRAP_WORD, wrapWidth);
  }
  if (style.hasShadow) {
    // This doesn't have the nicer shadow because it's so many verts.
    PPGeDrawCurrentText(style.shadowColor);
    PPGePrepareText(s, x, y, style.align, scale, lineHeightScale,
                    PPGE_LINE_USE_ELLIPSIS | PPGE_LINE_WRAP_WORD, wrapWidth);
  }
  PPGeDrawCurrentText(style.color);
}
...
void PPGeDraw4Patch(ImageID atlasImage, float x, float y, float w, float h,
                    u32 color) {
  if (!dlPtr)
    return;
...
void PPGeDrawImage(ImageID atlasImage, float x, float y,
                   const PPGeStyle &style) {
  if (!dlPtr)
    return;
  ...
  if (style.hasShadow) {
    for (float dy = 0.0f; dy <= 2.0f; dy += 1.0f) {
      for (float dx = 0.0f; dx <= 1.0f; dx += 0.5f) {
        if (dx == 0.0f && dy == 0.0f)
          continue;
        Vertex(x + dx, y + dy, img->u1, img->v1, atlasWidth, atlasHeight,
               alphaMul(style.shadowColor, 0.35f));
        Vertex(x + dx + w, y + dy + h, img->u2, img->v2, atlasWidth,
               atlasHeight, alphaMul(style.shadowColor, 0.35f));
      }
    }
  }
...
void PPGeDrawImage(ImageID atlasImage, float x, float y, float w, float h,
                   const PPGeStyle &style) {
  if (!dlPtr)
    return;
...
void PPGeDrawImage(float x, float y, float w, float h, float u1, float v1,
                   float u2, float v2, int tw, int th,
                   const PPGeImageStyle &style) {
  if (!dlPtr)
    return;
  BeginVertexData();
...
void PPGeSetDefaultTexture() {
  ...
  WriteCmd(GE_CMD_TEXFUNC,
           (0 << 16) | (1 << 8) |
               0); // RGBA texture reads, modulate, no color doubling
...
void PPGeDisableTexture() { WriteCmd(GE_CMD_TEXTUREMAPENABLE, 0); }

std::vector<PPGeImage *> PPGeImage::loadedTextures_;

constexpr size_t MAX_VALID_IMAGE_SIZE = 16 * 1024 * 1024;

PPGeImage::PPGeImage(std::string_view pspFilename) : filename_(pspFilename) {}
...
PPGeImage::PPGeImage(u32 pngPointer, size_t pngSize)
  ...
  if (!Memory::IsValidRange(this->png_, (u32)this->size_)) {
    WARN_LOG(Log::sceGe,
             "Created PPGeImage from invalid memory range %08x (%08x bytes). "
             "Will not be drawn.",
             this->png_, (int)this->size_);
  }
}

PPGeImage::~PPGeImage() { Free(); }
...
bool PPGeImage::Load() {
  ...
  if (filename_.empty()) {
    ...
    if (!srcPtr) {
      ERROR_LOG(Log::sceGe,
                "Trying to load PPGeImage from invalid range: %08x, %08x bytes",
                png_, (int)size_);
      return false;
    }
    ...
    // Fallback: Try reading directly from host storage if it's an ms0:/ path.
    // pspFileSystem sometimes fails on some platforms/configurations for simple
    // file reads.
    if (filename_.length() > 5 && filename_.substr(0, 5) == "ms0:/") {
      Path hostPath = g_Config.memStickDirectory / filename_.substr(5);

      std::string data;
      if (File::ReadBinaryFileToString(hostPath, &data)) {
        pngData.resize(data.size());
        memcpy(pngData.data(), data.data(), data.size());
        successRead = true;
      }

      if (!successRead && filename_.length() > 9 &&
          filename_.substr(5, 4) == "PSP/") {
        // If that failed, try stripping "PSP/" if present (ms0:/PSP/A ->
        // root/A) This matches STRIP_PSP behavior.
        Path hostPathStripped =
            g_Config.memStickDirectory / filename_.substr(9);
        if (File::ReadBinaryFileToString(hostPathStripped, &data)) {
          pngData.resize(data.size());
          memcpy(pngData.data(), data.data(), data.size());
          successRead = true;
        }
    ...
    if (!successRead) {
      if (pspFileSystem.ReadEntireFile(filename_, pngData) < 0) {
        WARN_LOG(Log::sceGe, "PPGeImage cannot load file %s",
                 filename_.c_str());
        loadFailed_ = true;
        return false;
      }
    ...
    success = pngLoadPtr((const unsigned char *)&pngData[0], pngData.size(),
                         &width_, &height_, &textureData);
  }
  if (!success) {
    WARN_LOG(Log::sceGe, "Bad PPGeImage - not a valid png (path: %s)",
             filename_.c_str());
    loadFailed_ = true;
    return false;
  }
  ...
  if (texture_ == 0) {
    free(textureData);
    WARN_LOG(Log::sceGe,
             "Bad PPGeImage - unable to allocate space for texture");
...
void PPGeImage::Free() {
  if (texture_ != 0) {
    ...
    texture_ = 0;
    loadedTextures_.erase(
        std::remove(loadedTextures_.begin(), loadedTextures_.end(), this),
        loadedTextures_.end());
    loadFailed_ = false;
  }
}
