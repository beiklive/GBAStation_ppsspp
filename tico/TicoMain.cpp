/// @file TicoMain.cpp
/// @brief Entry point for tico-integrated PPSSPP NRO
...
bool InitWindow()
{
    ...
    g_window = SDL_CreateWindow("ppsspp",
...
void HandleInput()
{
    ...
    if (g_core)
    {
        ...
        for (int p = 0; p < numControllers; p++)
        {
            ...
            // PSP mapping from Switch layout
...
int main(int argc, char *argv[])
{
    ...
    // 2. Socket initialization removed to prevent 10s timeout hang on boot without wifi
    ...
    // Pin main thread to core 0 — this runs JIT, GPU, and retro_run()
    // Prevents cache thrashing from OS scheduler bouncing across cores
    svcSetThreadCoreMask(CUR_THREAD_HANDLE, 0, (1ULL << 0));

    // Boost main thread priority from default 0x2C to 0x20
    // Lower value = higher priority on Switch
    svcSetThreadPriority(CUR_THREAD_HANDLE, 0x20);

    // Boost CPU clock to 1428 MHz for smooth emulation
    // Uses clkrst on firmware ≥8.0.0, pcv on older firmware (same as sys-clk)
    {
        Result rc;
        if (hosversionAtLeast(8,0,0))
        {
            ...
            if (R_SUCCEEDED(rc))
            {
                ...
                if (R_SUCCEEDED(rc))
                {
                    rc = clkrstSetClockRate(&session, 1326000000);
                    if (R_SUCCEEDED(rc))
                        LOG_INFO("HOME", "CPU clock set to 1326 MHz via clkrst");
                    else
                        LOG_WARN("HOME", "clkrstSetClockRate failed: 0x%x", rc);
                    clkrstCloseSession(&session);
                }
                else
                {
                    LOG_WARN("HOME", "clkrstOpenSession failed: 0x%x", rc);
                }
                // Don't call clkrstExit() — keep the clock rate active for the lifetime of the process
            }
            else
            {
                LOG_WARN("HOME", "clkrstInitialize failed: 0x%x", rc);
            }
        }
        else
        {
            ...
            if (R_SUCCEEDED(rc))
            {
                rc = pcvSetClockRate(PcvModule_CpuBus, 1326000000);
                if (R_SUCCEEDED(rc))
                    LOG_INFO("HOME", "CPU clock set to 1326 MHz via pcv");
                else
                    LOG_WARN("HOME", "pcvSetClockRate failed: 0x%x", rc);
                // Don't call pcvExit() — keep the clock rate active
            }
            else
            {
                LOG_WARN("HOME", "pcvInitialize failed: 0x%x", rc);
            }
        }
    }

    LOG_INFO("HOME", "Main thread optimized (Core 0, Priority 0x20, CPU 1428MHz)");
#endif

    LOG_INFO("HOME", "ppsspp starting...");
    ...
    if (argc > 1)
    {
        // Find first arg that isn't ticoSetup
        for (int i = 1; i < argc; ++i)
        {
            if (strcmp(argv[i], "ticoSetup") != 0)
            {
                // Assume it's the ROM path if it's not the flag
                // (Very basic parsing, might want to improve if there are more flags)
                romPath = argv[i];
                romArgFound = true;
    ...
    // Search args for a title? Or just use filename.
    // Simplified for now, just filename logic unless specific arg structure known.
    // Assuming arg[2] was title in previous code, but index shifts with ticoSetup.
    // Let's rely on standard logic for now.

    // Logic to extract clean title (remove regions like (USA), [v1.0] etc.)
