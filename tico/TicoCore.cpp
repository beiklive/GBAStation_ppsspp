/// @file TicoCore.cpp
/// @brief Simplified libretro frontend for PPSSPP with tico overlay
/// Based on SwanStation's TicoCore.cpp but adapted for PPSSPP

#include "TicoCore.h"
#include "TicoConfig.h"
#include "PpssppConfig.h"
#include <json.hpp>
#include <SDL.h>
#include <SDL_mixer.h>
#include <cstring>
...
{
    // PPSSPP manages its own save data internally via MemStick
    // No external SRAM loading needed
    LOG_CORE("LoadSRAM: PPSSPP manages saves internally via MemStick");
}

void TicoCore::SaveSRAM()
{
    // PPSSPP manages its own save data internally via MemStick
    LOG_CORE("SaveSRAM: PPSSPP manages saves internally via MemStick");
}

// Include libretro API
#include "libretro.h"

// Forward declarations for PPSSPP core functions (standard libretro API)
extern "C"
{
...
bool TicoCore::Init()
{
    ...
    // Load trophy sound effect (Mix_OpenAudio is called in InitWindow)
#ifdef __SWITCH__
...
bool TicoCore::LoadGame(const std::string &path)
{
    ...
    // PPSSPP libretro core ignores `gameInfo.data` and loads directly from `gameInfo.path`.
    // Do NOT read the ROM into RAM! It causes a massive boot delay and memory waste.
    ...
    // Set controller - PPSSPP uses standard JOYPAD (with analog)
...
void TicoCore::LoadState(const std::string &path)
{
    ...
    if (success)
    {
        ...
        // Run one frame to force display update
...
bool TicoCore::HandleEnvironment(unsigned cmd, void *data)
{
    ...
    case RETRO_ENVIRONMENT_GET_VARIABLE:
    {
        ...
        // Fixed performance defaults — never configurable, always forced
        static const char* s_jit = "JIT";
        ...
        if (strcmp(var->key, "ppsspp_software_rendering") == 0) {
            var->value = s_disabled;
            return true;
        }
        if (strcmp(var->key, "ppsspp_force_lag_sync") == 0) {
            var->value = s_disabled;
            return true;
        }
        ...
        // Handle ppsspp_ options via PpssppConfig
        if (strncmp(var->key, "ppsspp_", 7) == 0)
        {
            ...
            // Network-related — always disabled
            if (strncmp(var->key, "ppsspp_enable_wlan", 18) == 0 ||
                strncmp(var->key, "ppsspp_wlan_channel", 19) == 0 ||
                strncmp(var->key, "ppsspp_enable_builtin_pro_ad_hoc_server", 39) == 0 ||
                strncmp(var->key, "ppsspp_change_pro_ad_hoc_server_address", 39) == 0 ||
                strncmp(var->key, "ppsspp_enable_upnp", 18) == 0 ||
                strncmp(var->key, "ppsspp_upnp_use_original_port", 29) == 0 ||
                strncmp(var->key, "ppsspp_port_offset", 18) == 0 ||
                strncmp(var->key, "ppsspp_minimum_timeout", 22) == 0 ||
                strncmp(var->key, "ppsspp_forced_first_connect", 27) == 0 ||
                strncmp(var->key, "ppsspp_change_mac_address", 25) == 0 ||
                strncmp(var->key, "ppsspp_pro_ad_hoc_server_address", 32) == 0)
            {
                var->value = s_disabled;
                return true;
            }
                                   ...
                                   unsigned height, size_t pitch)
{
    ...
    // During PPSSPP boot, video_cb fires with GPU init garbage.
    // Skip the first few frames before marking the texture as valid.
    if (!m_hasValidFrame)
    {
        m_bootFrameCount++;
        // PPSSPP typically takes 5-10 video_cb calls during boot before
        // producing a real game frame. Skip them all to prevent glitch.
        if (m_bootFrameCount > 8)
        {
            m_hasValidFrame = true;
            LOG_CORE("First valid frame detected after %d boot frames", m_bootFrameCount);
        }
    }
}
...
void TicoCore::LoadConfig()
{
    ...
    // Load PPSSPP config
    PpssppConfig::Instance().Load();

    // Copy options from PpssppConfig into local map for GET_VARIABLE
    const auto& opts = PpssppConfig::Instance().GetOptionsMap();
    for (const auto& pair : opts) {
        m_configOptions[pair.first] = pair.second;
    }

    m_configLoaded = true;
    LOG_CORE("Loaded %lu options from PpssppConfig", m_configOptions.size());
}
...
{
    uint32_t console_id = 41; // PSP default
