...
public:
  ...
  void Log(Level level, const char *category, uint32_t flags,
    ...
    if (!m_file) {
      m_file = fopen("sdmc:/tico/debug/ppsspp.txt", "a");
    }
#endif
    if (!m_file)
      return;
...
#define LOG_DEBUG_FLAGS(flags, cat, ...) do { } while (0)
#define LOG_INFO_FLAGS(flags, cat, ...) do { } while (0)
#define LOG_WARN_FLAGS(flags, cat, ...) do { } while (0)
#define LOG_ERROR_FLAGS(flags, cat, ...) do { } while (0)
#else
#define LOG_DEBUG(cat, ...) \
  do { \
    if (Logger::Instance().IsCategoryEnabled(cat) && \
        Logger::Instance().ShouldLog(Logger::Level::DEBUG)) { \
      Logger::Instance().Log(Logger::Level::DEBUG, cat, Logger::None, __VA_ARGS__); \
    } \
  } while (0)
...
#define LOG_INFO(cat, ...) \
  do { \
    if (Logger::Instance().IsCategoryEnabled(cat) && \
      ...
      Logger::Instance().Log(Logger::Level::INFO, cat, Logger::None, __VA_ARGS__); \
    } \
  } while (0)
...
#define LOG_WARN(cat, ...) \
  do { \
    if (Logger::Instance().IsCategoryEnabled(cat) && \
      ...
      Logger::Instance().Log(Logger::Level::WARNING, cat, Logger::None, __VA_ARGS__); \
    } \
  } while (0)
...
#define LOG_ERROR(cat, ...) \
  do { \
    if (Logger::Instance().IsCategoryEnabled(cat) && \
      ...
      Logger::Instance().Log(Logger::Level::ERROR, cat, Logger::None, __VA_ARGS__); \
    } \
  } while (0)
