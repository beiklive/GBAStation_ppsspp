...
public:
    ...
    static constexpr size_t QUEUE_THRESHOLD = 32768;       // ~185ms latency target (SDL_QueueAudio) to absorb jitter
    ...
    void PushSample(int16_t left, int16_t right)
    {
        ...
        if (TicoConfig::USE_SDLQUEUEAUDIO)
        {
            // NEVER block the main thread. If we have too much audio queued, drop it
            // to prevent stalling the emulator and missing VSync deadlines.
            if (SDL_GetQueuedAudioSize(m_deviceId) > QUEUE_THRESHOLD)
            {
                return;
            }
        ...
        {
            if (m_buffer.GetAvailableWrite() < 2)
            {
                return; // Drop sample to prevent stall
            }
            int16_t samples[2] = {left, right};
    ...
    size_t PushSamples(const int16_t *data, size_t frames)
    {
        ...
        if (TicoConfig::USE_SDLQUEUEAUDIO)
        {
            // Drop audio batch instead of stalling. A stall of 1ms here can cause
            // eglSwapBuffers to miss the Vblank, creating a 33.3ms total frame delay,
            // which causes a massive audio underrun (desync/crackling).
            if (SDL_GetQueuedAudioSize(m_deviceId) > QUEUE_THRESHOLD)
            {
                return frames; // Pretend we wrote it
            }
            SDL_QueueAudio(m_deviceId, data, samplesNeeded * sizeof(int16_t));
        }
        else
        {
            if (m_buffer.GetAvailableWrite() < samplesNeeded)
            {
                return frames; // Pretend we wrote it
            }
...
private:
    ...
    static void AudioCallback(void *userdata, uint8_t *stream, int len)
    {
        ...
        size_t samplesNeeded = len / sizeof(int16_t);
        ...
        if (toRead > 0)
        {
            std::vector<int16_t> tempBuf(toRead);
            size_t read = self->m_buffer.Read(tempBuf.data(), toRead);

            // Feed to resampler or directly to stream
            if (self->m_resampler)
            {
                SDL_AudioStreamPut(self->m_resampler, tempBuf.data(), read * sizeof(int16_t));
            }
            else
            {
                // No resampling needed - copy directly
                size_t bytesToCopy = std::min(read * sizeof(int16_t), static_cast<size_t>(len));
                memcpy(stream, tempBuf.data(), bytesToCopy);
