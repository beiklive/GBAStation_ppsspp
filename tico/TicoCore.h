...
/// @brief Simplified libretro core wrapper for PPSSPP
class TicoCore
{
public:
    TicoCore();
    ...
    unsigned int GetFrameTextureID() const { return m_hasValidFrame ? m_frameTexture : 0; }
...
private:
    ...
    bool m_hasValidFrame = false; // Suppresses rendering until a real game frame is produced
    int m_bootFrameCount = 0;     // Counts video_cb calls during boot to skip garbage frames
    ...
    int m_frameWidth = 480;
    int m_frameHeight = 272;
    int m_fboWidth = 0;
    int m_fboHeight = 0;
    float m_aspectRatio = 480.0f / 272.0f;
    double m_fps = 60.0;
