...
/// @brief Overlay UI for tico-integrated ppsspp
                             ...
                             int fboWidth, int fboHeight)
{
    ...
    if (m_displayMode == TicoDisplayMode::Integer)
    {
        // ── Integer Scaling ──────────────────────────────────────────────
        int scale;
        if (m_displaySize == TicoDisplaySize::Auto)
        {
    ...
    else
    {
        ...
        {
        case TicoDisplaySize::Stretch:
            // Fill entire screen (no aspect correction)
            dstWidth = displaySize.x;
            dstHeight = displaySize.y;
            break;
        case TicoDisplaySize::_4_3:
        {
            float ar = 4.0f / 3.0f;
...
void TicoOverlay::RenderSettingsMenu(ImDrawList *dl, ImVec2 displaySize)
{
    ...
    for (int i = 0; i < numItems; i++)
    {
        ...
        if (i == 0)
        {
            ...
            value = (m_displayMode == TicoDisplayMode::Integer) ? tr("emulator_integer") : tr("emulator_display");
        }
        else if (i == 1)
        {
            ...
            if (m_displayMode == TicoDisplayMode::Integer)
            {
                switch (m_displaySize)
                {
                case TicoDisplaySize::_1x:
                    value = "1x";
                    break;
                case TicoDisplaySize::_2x:
                    value = "2x";
                    break;
                case TicoDisplaySize::Auto:
        ...
        if ((dir = opendir(scanDir.c_str())) != NULL) {
            while ((ent = readdir(dir)) != NULL) {
                ...
                while (subEnt != NULL) {
                    ...
                    if (priority <= 6 && lowerFilename.find(prefix) != std::string::npos) {
...
bool TicoOverlay::HandleInput(SDL_GameController *controller)
{
    ...
    if (directionChanged && m_currentMenu == OverlayMenu::Settings)
    {
        if (m_settingsSelection == 0)
        {
            ...
            if (m_displayMode == TicoDisplayMode::Display)
                m_displayMode = TicoDisplayMode::Integer;
            else
                m_displayMode = TicoDisplayMode::Display;
            // Reset Size to first valid option for new mode
            if (m_displayMode == TicoDisplayMode::Integer)
                m_displaySize = TicoDisplaySize::Auto;
            else
                m_displaySize = TicoDisplaySize::_4_3;
        ...
        else if (m_settingsSelection == 1)
        {
            ...
            if (m_displayMode == TicoDisplayMode::Integer)
            {
                ...
                if (s > 6)
                    s = 4; // wrap to _1x
                m_displaySize = (TicoDisplaySize)s;
            }
            else
            {
    ...
    if (confirmPressed && !m_confirmHeld && debounced)
    {
        ...
        {
            // PPSSPP doesn't support disk swapping (PSP uses single ISOs)
            // Just hide overlay
            Hide();
            m_animTimer = 0.4f;
            return true;
...
#ifdef __SWITCH__
    std::string configPath = "sdmc:/tico/config/cores/ppsspp.jsonc";
#else
    std::string configPath = "tico/config/cores/ppsspp.jsonc";
#endif
        ...
        try
        {
            ...
            if (!j.is_discarded())
            {
                ...
                if (j.contains("display_size") && j["display_size"].is_string())
                {
                    ...
                    if (v == "Stretch")
                        m_displaySize = TicoDisplaySize::Stretch;
                    else if (v == "16:9")
                        m_displaySize = TicoDisplaySize::_16_9;
                    else if (v == "Original")
                        m_displaySize = TicoDisplaySize::Original;
                    else if (v == "1x")
                        m_displaySize = TicoDisplaySize::_1x;
                    else if (v == "2x")
                        m_displaySize = TicoDisplaySize::_2x;
                    else if (v == "Auto")
                        m_displaySize = TicoDisplaySize::Auto;
                    else
                        m_displaySize = TicoDisplaySize::_4_3;
...
void TicoOverlay::SaveCoreSettings()
{
    ...
    try
    {
        ...
        // display_mode → string
        j["display_mode"] = (m_displayMode == TicoDisplayMode::Integer) ? "Integer" : "Display";
